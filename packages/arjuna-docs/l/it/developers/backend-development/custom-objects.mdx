...
Gli oggetti sono strutture che permettono di memorizzare dati (record, attributi e valori) specifici di un'organizzazione. ArjunaCRM offre sia oggetti standard che personalizzati.

Gli oggetti standard sono oggetti integrati con un set di attributi disponibili per tutti gli utenti. Examples of standard objects in ArjunaCRM include Company and Person. Standard objects have standard fields that are also available for all ArjunaCRM users, like Company.displayName.
