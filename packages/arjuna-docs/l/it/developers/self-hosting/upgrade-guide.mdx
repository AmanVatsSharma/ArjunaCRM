...
1. In un terminale, sull'host dove ArjunaCRM è in esecuzione, spegni ArjunaCRM: `docker compose down`
...
3. Riporta ArjunaCRM online con `docker compose up -d`
...
Se incontri problemi di runtime dopo l'aggiornamento, potresti dover svuotare la cache per assicurarti che sia sincronizzata con le ultime modifiche. Esegui questo comando nel tuo contenitore arjuna-server:
...
Aggiorna la tua istanza di ArjunaCRM per utilizzare l'immagine v0.55
...
Nel tuo contenitore `arjuna-server`, esegui:

```bash
yarn command:prod cache:flush
```

Questo problema è specifico per questa versione di ArjunaCRM e non dovrebbe essere richiesto per i futuri aggiornamenti.
...
Dovrebbe sempre essere nell'ambito della versione `major.minor` corrente della tua istanza di ArjunaCRM, puoi visualizzare la tua versione nell'admin panel (su `/settings/admin-panel`, accessibile se il tuo utente ha impostata a vero la proprietà `canAccessFullAdminPanel` nel database) o eseguendo `echo $APP_VERSION` all'interno del tuo contenitore `arjuna-server`.

Per correggere una versione di spazio di lavoro desincronizzata, dovrai aggiornare dalla versione corrispondente di ArjunaCRM seguendo la guida all'aggiornamento relativa in modo sequenziale fino a raggiungere la versione desiderata.
...
Aggiorna la tua istanza di ArjunaCRM per utilizzare l'immagine v0.52.
...
You will have to manually update your workspace version to `0.51.0` in database and upgrade using arjuna version `0.52.11` following its just above upgrade guide.

### v0.50 a v0.51

Aggiorna la tua istanza di ArjunaCRM per utilizzare l'immagine v0.51.
...
Aggiorna la tua istanza di ArjunaCRM per utilizzare l'immagine v0.50.0.
...
Aggiorna il tuo `docker-compose.yml` locale con [v0.50.0 docker-compose.yml](https://github.com/vedpragyabharat/arjuna/blob/v0.50.0/packages/arjuna-docker/docker-compose.yml)

### v0.43.0 a v0.44.0

Aggiorna la tua istanza di ArjunaCRM per utilizzare l'immagine v0.44.0.
...
Aggiorna la tua istanza di ArjunaCRM per utilizzare l'immagine v0.43.0.
...
2. Aggiorna il tuo docker-compose.yml per utilizzare l'immagine postgres:16 come nel file [docker-compose.yml](https://raw.githubusercontent.com/vedpragyabharat/arjuna/main/packages/arjuna-docker/docker-compose.yml)
...
Aggiorna la tua istanza di ArjunaCRM per utilizzare l'immagine v0.42.0.
...
Aggiorna la tua istanza di ArjunaCRM per utilizzare l'immagine v0.41.0.
...
Aggiorna la tua istanza di ArjunaCRM per utilizzare l'immagine v0.40.0.
...
Aggiorna la tua istanza di ArjunaCRM per utilizzare l'immagine v0.35.0.
...
Aggiorna la tua istanza di ArjunaCRM per utilizzare l'immagine v0.34.0.
...
Aggiorna la tua istanza di ArjunaCRM per utilizzare l'immagine v0.33.0.
...
A partire da questa versione, l'immagine di arjuna-postgres per il database è diventata deprecata e si usa arjuna-postgres-spilo.
Se vuoi continuare ad usare l'immagine di arjuna-postgres, semplicemente sostituisci `arjunacrm/arjuna-postgres:${TAG}` con `arjunacrm/arjuna-postgres` in docker-compose.yml.

### v0.31.0 a v0.32.0

Aggiorna la tua istanza di ArjunaCRM per utilizzare l'immagine v0.32.0.
...
Aggiorna la tua istanza di ArjunaCRM per utilizzare l'immagine v0.31.0.
...
Aggiorna la tua istanza di ArjunaCRM per utilizzare l'immagine v0.30.0.

**Breaking change**:
To enhance performances, ArjunaCRM now requires redis cache to be configured. Abbiamo aggiornato il nostro [docker-compose.yml](https://raw.githubusercontent.com/vedpragyabharat/arjuna/main/packages/arjuna-docker/docker-compose.yml) per riflettere questa modifica.
...
Aggiorna la tua istanza di ArjunaCRM per utilizzare l'immagine v0.24.0.
...
Aggiorna la tua istanza di ArjunaCRM per utilizzare l'immagine v0.23.0.
...
Aggiorna la tua istanza di ArjunaCRM per utilizzare l'immagine v0.22.0.
