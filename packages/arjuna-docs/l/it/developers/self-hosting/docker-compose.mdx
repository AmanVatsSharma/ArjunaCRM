...
<Warning>
Docker containers are for production hosting or self-hosting, for the contribution please check the [Local Setup](https://docs.vedpragya.com/l/it/developers/local-setup).
</Warning>

## Panoramica

Questa guida fornisce istruzioni passo passo per installare e configurare l'applicazione ArjunaCRM usando Docker Compose. L'obiettivo è rendere il processo semplice ed evitare gli errori comuni che potrebbero compromettere il tuo setup.
...
Consulta i documenti [Configurazione delle Variabili di Ambiente](https://docs.vedpragya.com/l/it/developers/self-hosting/setup) per configurazioni avanzate. Tutte le variabili di ambiente devono essere dichiarate nel file docker-compose.yml a livello di server e/o di worker a seconda della variabile.
...
Installa l'ultima versione stabile di ArjunaCRM con un unico comando:
...
Se ospiti arjunaCRM sul tuo computer, apri il browser e naviga a [http://localhost:3000](http://localhost:3000).
...
### Esponi ArjunaCRM per Accesso Esterno

Per impostazione predefinita, ArjunaCRM gira su `localhost` alla porta `3000`. Per accedervi tramite un dominio esterno o indirizzo IP devi configurare il `SERVER_URL` nel tuo file `.env`.
...
Raccomandiamo fortemente di configurare ArjunaCRM dietro un proxy inverso con terminazione SSL per una sicurezza e funzionalità ottimali.
...
Se riscontri problemi, controlla [Risoluzione dei problemi](https://docs.vedpragya.com/l/it/developers/self-hosting/troubleshooting) per le soluzioni.
