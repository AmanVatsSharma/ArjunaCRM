...
Se hai un'istanza di ArjunaCRM esistente con dati di produzione, **NON** seguire questi passaggi poiché elimineranno permanentemente il tuo database!

Durante l'installazione di ArjunaCRM per la prima volta, potresti voler cambiare la password del database predefinita.
...
Durante l'installazione di ArjunaCRM, devi fornire il tuo database postgres con gli schemi, le estensioni e gli utenti corretti.
...
#### Cannot find module 'arjuna-emails' or its corresponding type declarations.

You have to build the package `arjuna-emails` before running the initialization of the database with `npx nx run arjuna-emails:build`

#### Pacchetto arjuna-x mancante

Assicurati di eseguire yarn nella directory principale e poi esegui `npx nx server:dev arjuna-server`. Se ancora non funziona prova a costruire manualmente il pacchetto mancante.
...
#### Durante l'esecuzione di `npx nx start` o `npx nx start arjuna-front`, viene generato un errore di memoria insufficiente

In `packages/arjuna-front/.env` decommentare `VITE_DISABLE_TYPESCRIPT_CHECKER=true` e `VITE_DISABLE_ESLINT_CHECKER=true` per disabilitare i controlli in background riducendo così la quantità di RAM necessaria.

**Se non funziona:**  
Esegui solo i servizi di cui hai bisogno, invece di `npx nx start`. Ad esempio, se lavori sul server, esegui solo `npx nx worker arjuna-server`

**Se non funziona:**  
Se hai provato a eseguire solo `npx nx run arjuna-server:start` su WSL e sta fallendo con l'errore di memoria seguente:
...
**Se non funziona:**  
Indaga quali processi stanno utilizzando la maggior parte della RAM del tuo computer. In ArjunaCRM, abbiamo notato che alcune estensioni di VScode stavano occupando molta RAM quindi le disabilitiamo temporaneamente.
...
- Apri la tua app ArjunaCRM, naviga su `/settings` e abilita il toggle **Avanzate** in basso a sinistra dello schermo.
- Crea un nuovo webhook.
- Incolla **Your Unique Webhook URL** nel campo **Endpoint Url** in ArjunaCRM. Imposta i **Filtri** su `Companies` e `Created`.
<div style={{textAlign: 'center'}}>
...
Commenta il plugin checker in `packages/arjuna-ui/vite-config.ts` come mostrato nell'esempio sotto
...
Se stai eseguendo ArjunaCRM dietro un reverse proxy e stai riscontrando problemi di connessione:
...
2. **Controlla le impostazioni del reverse proxy:**
   ...
   - Conferma che il tuo reverse proxy sta inoltrando correttamente le richieste al server ArjunaCRM.
   - Assicurati che le intestazioni come `X-Forwarded-For` e `X-Forwarded-Proto` siano impostate correttamente.

3. **Riavvia i servizi:**

   Dopo aver apportato modifiche, riavvia sia il reverse proxy che i container ArjunaCRM.
...
- Supporto comunitario:
  ...
  Contatta la [comunità ArjunaCRM](https://github.com/vedpragyabharat/arjuna/issues) o [canali di supporto](https://discord.gg/cx5n4Jzs57) per assistenza.