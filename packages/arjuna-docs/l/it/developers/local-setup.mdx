...
description: "La guida per i collaboratori (o sviluppatori curiosi) che vogliono eseguire ArjunaCRM localmente."
...
Prima di poter installare e usare ArjunaCRM, assicurati di installare quanto segue sul tuo computer:
...
ArjunaCRM richiede una cache redis per offrire le migliori prestazioni
...
Usa variabili d'ambiente o file `.env` per configurare il tuo progetto. Maggiori informazioni [qui](https://docs.vedpragya.com/l/it/developers/self-hosting/setup)
...
Per costruire il server ArjunaCRM e popolare alcuni dati nel tuo database, esegui il seguente comando:
...
## Passo 8: Utilizza ArjunaCRM

**Frontend**

Il frontend di ArjunaCRM sarà in esecuzione su [http://localhost:3001](http://localhost:3001).
Puoi accedere utilizzando l'account demo predefinito: `tim@apple.dev` (password: `tim@apple.dev`)

**Backend**

- Il server di ArjunaCRM sarà attivo e funzionante su [http://localhost:3000](http://localhost:3000)
