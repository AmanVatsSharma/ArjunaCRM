...
1. En una terminal, en el host donde ArjunaCRM está funcionando, apague ArjunaCRM: `docker compose down`
...
3. Vuelva a conectar ArjunaCRM con `docker compose up -d`
...
Si encuentra problemas de ejecución después de actualizar, es posible que deba vaciar su caché para asegurar que esté sincronizado con los cambios más recientes. Ejecute este comando en su contenedor del servidor de arjuna:
...
Actualice su instancia de ArjunaCRM para usar la imagen v0.55
...
En su contenedor `arjuna-server`, ejecute:

```bash
yarn command:prod cache:flush
```

Este problema es específico de esta versión de ArjunaCRM y no debería ser necesario para futuras actualizaciones.
...
Siempre debería estar dentro del rango de la versión `major.minor` actual de su instancia de ArjunaCRM; puede ver la versión de su instancia en el panel de administración (en `/settings/admin-panel`, accesible si su usuario tiene la propiedad `canAccessFullAdminPanel` establecida en verdadero en la base de datos) o ejecutando `echo $APP_VERSION` en su contenedor `arjuna-server`.

Para corregir una versión de espacio de trabajo desincronizada, tendrá que actualizar desde la correspondiente versión de arjuna siguiendo la guía de actualización relacionada secuencialmente, y así sucesivamente hasta alcanzar la versión deseada.
...
Actualice su instancia de ArjunaCRM para usar la imagen v0.52
...
Tendrá que actualizar manualmente la versión de su espacio de trabajo a `0.51.0` en la base de datos y actualizar usando la versión arjuna `0.52.11` siguiendo su guía de actualización justo arriba.

### v0.50 a v0.51

Actualice su instancia de ArjunaCRM para usar la imagen v0.51
...
Actualice su instancia de ArjunaCRM para usar la imagen v0.50.0
...
Actualice su `docker-compose.yml` local con el [docker-compose.yml de v0.50.0](https://github.com/vedpragyabharat/arjuna/blob/v0.50.0/packages/arjuna-docker/docker-compose.yml)

### v0.43.0 a v0.44.0

Actualice su instancia de ArjunaCRM para usar la imagen v0.44.0
...
Actualice su instancia de ArjunaCRM para usar la imagen v0.43.0
...
2. Actualice su docker-compose.yml para usar la imagen de postgres:16 como en el archivo [docker-compose.yml](https://raw.githubusercontent.com/vedpragyabharat/arjuna/main/packages/arjuna-docker/docker-compose.yml).
...
Actualice su instancia de ArjunaCRM para usar la imagen v0.42.0
...
Actualice su instancia de ArjunaCRM para usar la imagen v0.41.0
...
Actualice su instancia de ArjunaCRM para usar la imagen v0.40.0
...
Actualice su instancia de ArjunaCRM para usar la imagen v0.35.0
...
Actualice su instancia de ArjunaCRM para usar la imagen v0.34.0
...
Actualice su instancia de ArjunaCRM para usar la imagen v0.33.0
...
A partir de esta versión, la imagen arjuna-postgres para DB quedó obsoleta y ahora se usa arjuna-postgres-spilo.
Si desea seguir usando la imagen arjuna-postgres, simplemente reemplace `arjunacrm/arjuna-postgres:${TAG}` con `arjunacrm/arjuna-postgres` en docker-compose.yml.

### v0.31.0 a v0.32.0

Actualice su instancia de ArjunaCRM para usar la imagen v0.32.0
...
Actualice su instancia de ArjunaCRM para usar la imagen v0.31.0
...
Actualice su instancia de ArjunaCRM para usar la imagen v0.30.0

**Breaking change**:
To enhance performances, ArjunaCRM now requires redis cache to be configured. Hemos actualizado nuestro [docker-compose.yml](https://raw.githubusercontent.com/vedpragyabharat/arjuna/main/packages/arjuna-docker/docker-compose.yml) para reflejar esto.
...
Actualice su instancia de ArjunaCRM para usar la imagen v0.24.0
...
Actualice su instancia de ArjunaCRM para usar la imagen v0.23.0
...
Actualice su instancia de ArjunaCRM para usar la imagen v0.22.0
