...
Si tiene una instancia de ArjunaCRM existente con datos de producción, **NO** siga estos pasos ya que borrarán permanentemente su base de datos.

Al instalar ArjunaCRM por primera vez, es posible que desee cambiar la contraseña predeterminada de la base de datos.
...
Durante la instalación de ArjunaCRM, debe aprovisionar su base de datos postgres con los esquemas, extensiones y usuarios correctos.
...
#### No se puede encontrar el módulo 'arjuna-emails' o sus declaraciones de tipo correspondientes.

Tienes que construir el paquete `arjuna-emails` antes de ejecutar la inicialización de la base de datos con `npx nx run arjuna-emails:build`

#### Missing arjuna-x package

Asegúrese de ejecutar yarn en el directorio raíz y luego ejecute `npx nx server:dev arjuna-server`. Si esto aún no funciona, intente construir el paquete faltante manualmente.
...
#### While running `npx nx start` or `npx nx start arjuna-front`, Out of memory error is thrown

En `packages/arjuna-front/.env` descomente `VITE_DISABLE_TYPESCRIPT_CHECKER=true` y `VITE_DISABLE_ESLINT_CHECKER=true` para deshabilitar las comprobaciones en segundo plano y así reducir la cantidad de RAM necesaria.

**Si no funciona:**  
Ejecute solo los servicios que necesite, en lugar de `npx nx start`. Por ejemplo, si trabaja en el servidor, ejecute solo `npx nx worker arjuna-server`

**Si no funciona:**  
Si intentó ejecutar solo `npx nx run arjuna-server:start` en WSL y falla con el siguiente error de memoria:
...
**Si no funciona:**  
Investigue qué procesos están ocupando la mayor parte de la RAM de su máquina. En ArjunaCRM, notamos que algunas extensiones de VScode estaban ocupando mucha RAM, por lo que las desactivamos temporalmente.
...
- Abra la aplicación ArjunaCRM, navegue a `/settings` y active el interruptor **Avanzado** en la parte inferior izquierda de la pantalla.
- Cree un nuevo webhook.
- Pegue **Su URL de Webhook Única** en el campo **Url de EndPoint** en ArjunaCRM. Establezca los **Filtros** en `Companies` y `Created`.
<div style={{textAlign: 'center'}}>
...
Comente el plugin checker en `packages/arjuna-ui/vite-config.ts` como en el ejemplo a continuación
...
Si está ejecutando ArjunaCRM detrás de un proxy inverso y experimenta problemas de conexión:
...
2. **Verifique la configuración del Proxy Reverso:**
   ...
   - Confirme que su proxy reverso está reenviando correctamente las solicitudes al servidor de ArjunaCRM.
...
3. **Reinicie los Servicios:**
   ...
   Después de hacer cambios, reinicie tanto el proxy inverso como los contenedores de ArjunaCRM.
...
- Soporte Comunitario:
  ...
  Póngase en contacto con la [comunidad de ArjunaCRM](https://github.com/vedpragyabharat/arjuna/issues) o [los canales de soporte](https://discord.gg/cx5n4Jzs57) para obtener asistencia.