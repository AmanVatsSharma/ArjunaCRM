...
1. Em um terminal, no host onde ArjunaCRM está sendo executado, desligue o ArjunaCRM: `docker compose down`
...
3. Reative o ArjunaCRM com `docker compose up -d`
...
Se você encontrar problemas de tempo de execução após a atualização, pode ser necessário limpar seu cache para garantir que esteja sincronizado com as alterações mais recentes. Execute este comando no seu contêiner do servidor ArjunaCRM:
...
Atualize sua instância ArjunaCRM para usar a imagem v0.55
...
No seu contêiner `arjuna-server`, execute:

```bash
yarn command:prod cache:flush
```

Este problema é específico desta versão do ArjunaCRM e não deve ser necessário para futuras atualizações.
...
Ela deve estar sempre no alcance da versão `major.minor` da sua instância atual do ArjunaCRM, você pode visualizar a versão da sua instância no painel de administração (em `/settings/admin-panel`, acessível se sua propriedade de usuário tiver `canAccessFullAdminPanel` definida como verdadeira no banco de dados) ou executando `echo $APP_VERSION` no seu contêiner `arjuna-server`.

Para corrigir uma versão de espaço de trabalho dessincronizada, você terá que atualizar a partir da versão correspondente do ArjunaCRM seguindo o guia de atualização relacionado sequencialmente e assim por diante até atingir a versão desejada.
...
Atualize sua instância ArjunaCRM para usar a imagem v0.52
...
Você terá que atualizar manualmente a versão do seu espaço de trabalho para `0.51.0` no banco de dados e atualizar usando a versão arjuna `0.52.11` seguindo seu guia de atualização logo acima.

### v0.50 para v0.51

Atualize sua instância ArjunaCRM para usar a imagem v0.51
...
Atualize sua instância ArjunaCRM para usar a imagem v0.50.0
...
Por favor, atualize seu `docker-compose.yml` local com [v0.50.0 docker-compose.yml](https://github.com/vedpragyabharat/arjuna/blob/v0.50.0/packages/arjuna-docker/docker-compose.yml)

### v0.43.0 para v0.44.0

Atualize sua instância ArjunaCRM para usar a imagem v0.44.0
...
Atualize sua instância ArjunaCRM para usar a imagem v0.43.0
...
2. Atualize seu docker-compose.yml para usar a imagem postgres:16 como no arquivo [docker-compose.yml](https://raw.githubusercontent.com/vedpragyabharat/arjuna/main/packages/arjuna-docker/docker-compose.yml).
...
Atualize sua instância ArjunaCRM para usar a imagem v0.42.0
...
Atualize sua instância ArjunaCRM para usar a imagem v0.41.0
...
Atualize sua instância ArjunaCRM para usar a imagem v0.40.0
...
Atualize sua instância ArjunaCRM para usar a imagem v0.35.0
...
Atualize sua instância ArjunaCRM para usar a imagem v0.34.0
...
Atualize sua instância ArjunaCRM para usar a imagem v0.33.0
...
A partir desta versão, a imagem arjuna-postgres para DB tornou-se obsoleta e arjuna-postgres-spilo é usado em seu lugar.
Se desejar continuar usando a imagem arjuna-postgres, basta substituir `arjunacrm/arjuna-postgres:${TAG}` por `arjunacrm/arjuna-postgres` no docker-compose.yml.

### v0.31.0 para v0.32.0

Atualize sua instância ArjunaCRM para usar a imagem v0.32.0
...
Atualize sua instância ArjunaCRM para usar a imagem v0.31.0
...
Atualize sua instância ArjunaCRM para usar a imagem v0.30.0

**Breaking change**:
To enhance performances, ArjunaCRM now requires redis cache to be configured. Atualizamos nosso [docker-compose.yml](https://raw.githubusercontent.com/vedpragyabharat/arjuna/main/packages/arjuna-docker/docker-compose.yml) para refletir isso.
...
Atualize sua instância ArjunaCRM para usar a imagem v0.24.0
...
Atualize sua instância ArjunaCRM para usar a imagem v0.23.0
...
Atualize sua instância ArjunaCRM para usar a imagem v0.22.0
