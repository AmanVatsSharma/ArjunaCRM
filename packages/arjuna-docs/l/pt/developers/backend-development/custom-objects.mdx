...
Objetos são estruturas que permitem armazenar dados (registros, atributos e valores) específicos de uma organização. O ArjunaCRM fornece tanto objetos padrão quanto personalizados.

Objetos padrão são objetos embutidos com um conjunto de atributos disponíveis para todos os usuários. Exemplos de objetos padrão no ArjunaCRM incluem Empresa e Pessoa. Objetos padrão possuem campos padrão que também estão disponíveis para todos os usuários do ArjunaCRM, como Company.displayName.
