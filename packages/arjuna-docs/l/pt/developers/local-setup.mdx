...
description: "O guia para contribuidores (ou desenvolvedores curiosos) que desejam executar o ArjunaCRM localmente."
...
Antes de instalar e usar o ArjunaCRM, certifique-se de instalar o seguinte em seu computador:
...
ArjunaCRM requer um cache redis para fornecer o melhor desempenho.
...
Use variáveis de ambiente ou arquivos `.env` para configurar seu projeto. Mais informações [aqui](https://docs.vedpragya.com/l/pt/developers/self-hosting/setup)
...
Para compilar o servidor ArjunaCRM e inserir alguns dados em seu banco de dados, execute o seguinte comando:
...
O frontend do ArjunaCRM estará rodando em [http://localhost:3001](http://localhost:3001).
Você pode acessar com a conta demo padrão: `tim@apple.dev` (senha: `tim@apple.dev`)

**Backend**

- O servidor do ArjunaCRM estará operando em [http://localhost:3000](http://localhost:3000)
