---
title: Glossar
info: "Machen Sie sich mit grundlegenden Begriffen vertraut, die in ArjunaCRM verwendet werden."
image: /images/user-guide/glossary/glossary.png
sectionInfo: Terminologieressourcen und Gemeinschaftsinformationen
---

<Frame>
  <img src="/images/user-guide/glossary/glossary.png" alt="Header" />
</Frame>

## API

API (Application Programming Interface) ermöglicht es Ihnen, ArjunaCRM mit anderen Softwaresystemen zu verbinden und benutzerdefinierte Integrationen zu erstellen.

## Befehlsmenü

Das Befehlsmenü ist eine Schnellzugriffsoberfläche (geöffnet mit `Cmd + K` auf Mac und `Ctrl + K` auf Windows), die es Ihnen ermöglicht, Aktionen durchzuführen, Datensätze zu erstellen und Ihren Arbeitsbereich effizient zu navigieren.

## Company & People

Das CRM verfügt über zwei grundlegende Arten von Datensätzen:

- A `Company` represents a business or organization.
- `People` represent your company's current and prospective customers or clients.

## Benutzerdefinierte Felder

Benutzerdefinierte Felder sind Datenfelder, die Sie erstellen, um Informationen zu erfassen, die speziell auf die Bedürfnisse und Prozesse Ihres Unternehmens zugeschnitten sind.

## Datenmodell

Ein Datenmodell ist die Struktur, die definiert, wie Informationen in Ihrem CRM organisiert sind, einschließlich der vorhandenen Objekte, deren Eigenschaften (Felder) und wie sie zueinander in Beziehung stehen.

## Favoriten

Favoriten sind Datensätze, die Sie für den Schnellzugriff markiert haben und in Ihrer Seitenleiste erscheinen, für den schnellen Zugriff auf wichtige Daten.

## Feld

Ein Feld bezeichnet einen bestimmten Bereich, in dem bestimmte Daten für eine Entität gespeichert werden.

## Integration

Integrationen sind integrierte Werkzeuge, die es ermöglichen, ArjunaCRM mit anderen Software oder Systemen zu verknüpfen.

## Kanban

Ein `Kanban` ist eine visuelle Möglichkeit, Ihre Geschäftsprozesse mit Karten und Spalten zu verfolgen. Jede Spalte stellt eine Phase in Ihrem Prozess dar (zum Beispiel: neu, laufend, gewonnen, verloren), und Sie verschieben Datensätze durch diese Phasen, während sie fortschreiten.

## Objekt

An Object is a data structure that represents a specific type of entity in your CRM (like People, Companies, or Opportunities). Objekte können standardmäßig (eingebaut) oder benutzerdefiniert (von Ihnen erstellt) sein.

## Opportunities

Opportunities in ArjunaCRM CRM are potential deals or sales with accounts or contacts.

## Datensatz

Ein Datensatz zeigt eine Instanz eines Objekts an, wie ein bestimmtes Konto oder einen bestimmten Kontakt.

## Relationsfelder

Beziehungsfelder schaffen Verbindungen zwischen verschiedenen Objekten und ermöglichen es, Datensätze miteinander zu verknüpfen (wie die Verbindung einer Person zu einem Unternehmen).

## Standardfelder

Standardfelder sind vorgefertigte Datenfelder, die standardmäßig mit Objekten geliefert werden und in allen Arbeitsbereichen gebräuchliche Funktionen bieten.

## Aufgaben

Tasks in ArjunaCRM CRM are assigned activities relating to contacts, accounts, or opportunities.

## Ansichten

Sie können die Anzeige Ihrer Datensätze mit Ansichten anpassen, indem Sie verschiedene Filter, Layouts und Sortierungsoptionen für jede Ansicht festlegen.

## Webhooks

Webhooks sind automatisierte Nachrichten, die von ArjunaCRM an andere Anwendungen gesendet werden, wenn bestimmte Ereignisse eintreten, und ermöglichen die Synchronisierung von Daten in Echtzeit.

## Workflows

Workflows sind automatisierte Prozesse, die basierend auf bestimmten Bedingungen Aktionen auslösen und Ihnen helfen, sich wiederholende Aufgaben und Geschäftsprozesse zu automatisieren.

## Arbeitsbereich

Ein `Arbeitsbereich` repräsentiert typischerweise ein Unternehmen, das ArjunaCRM verwendet. Er enthält alle Datensätze und Daten, die Sie und Ihre Teammitglieder zu ArjunaCRM hinzufügen.  
Er hat einen eindeutigen Domainnamen, der typischerweise der Domainname ist, den Ihr Unternehmen für Mitarbeiter-E-Mail-Adressen verwendet.

## Arbeitsbereichsmitglieder

Arbeitsbereichsmitglieder sind die ArjunaCRM-Benutzer aus Ihrem Team, die Zugriff auf Ihren Arbeitsbereich haben. Sie können als Eigentümer oder Beauftragte für Datensätze zugewiesen werden.

