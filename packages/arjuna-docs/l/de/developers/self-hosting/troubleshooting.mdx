...
Wenn Sie bereits eine bestehende ArjunaCRM-Instanz mit Produktionsdaten haben, **NICHT** diesen Schritten folgen, da diese permanent Ihre Datenbank löschen!

Bei der ersten Installation von ArjunaCRM möchten Sie möglicherweise das Standard-Datenbankpasswort ändern.
...
Während der ArjunaCRM-Installation müssen Sie Ihre Postgres-Datenbank mit den richtigen Schemata, Erweiterungen und Benutzern bereitstellen.
...
#### Modul 'arjuna-emails' oder seine entsprechenden Typdeklarationen nicht gefunden.

Sie müssen das Paket `arjuna-emails` erstellen, bevor Sie die Initialisierung der Datenbank mit `npx nx run arjuna-emails:build` ausführen.

#### Fehlendes ArjunaCRM-x Paket

Stellen Sie sicher, dass Sie `yarn` im Root-Verzeichnis ausführen und dann `npx nx server:dev arjuna-server` ausführen. Wenn das immer noch nicht funktioniert, versuchen Sie, das fehlende Paket manuell zu erstellen.
...
#### Beim Ausführen von `npx nx start` oder `npx nx start arjuna-front` wird ein Speicherfehler angezeigt

Kommentieren Sie in `packages/arjuna-front/.env` `VITE_DISABLE_TYPESCRIPT_CHECKER=true` und `VITE_DISABLE_ESLINT_CHECKER=true` aus, um Hintergrundprüfungen zu deaktivieren und den RAM-Bedarf zu reduzieren.

**Wenn es nicht funktioniert:**  
Führen Sie stattdessen nur die Services aus, die Sie benötigen, anstatt `npx nx start`. Wenn Sie zum Beispiel am Server arbeiten, führen Sie nur `npx nx worker arjuna-server` aus.

**Wenn es nicht funktioniert:**  
Wenn Sie versucht haben, nur `npx nx run arjuna-server:start` auf WSL zu starten und es mit dem untenstehenden Speicherfehler fehlschlägt:
...
**Wenn es nicht funktioniert:**  
Untersuchen Sie, welche Prozesse den größten Teil Ihres RAMs beanspruchen. Bei ArjunaCRM haben wir festgestellt, dass einige vscode-Erweiterungen viel RAM verwenden, daher deaktivieren wir sie vorübergehend.
...
- Öffnen Sie Ihre ArjunaCRM-App, navigieren Sie zu `/settings`, und aktivieren Sie den **Erweitert**-Schalter unten links auf dem Bildschirm.
- Erstellen Sie einen neuen Webhook.
- Fügen Sie **Ihre einzigartige Webhook-URL** in das Feld **Endpoint-Url** in ArjunaCRM ein. Stellen Sie die **Filter** auf `Companies` und `Created` ein.
<div style={{textAlign: 'center'}}>
...
Kommentieren Sie das Checker-Plugin in `packages/arjuna-ui/vite-config.ts` wie im folgenden Beispiel aus
...
Wenn Sie ArjunaCRM hinter einem Reverse-Proxy ausführen und Verbindungsprobleme haben:
...
2. **Reverse-Proxy-Einstellungen prüfen:**
   ...
   - Stellen Sie sicher, dass Ihr Reverse-Proxy Anfragen korrekt an den ArjunaCRM-Server weiterleitet.
   - Stellen Sie sicher, dass Header wie `X-Forwarded-For` und `X-Forwarded-Proto` korrekt gesetzt sind.

3. **Dienste neu starten:**

   Starten Sie nach Änderungen sowohl den Reverse-Proxy als auch die ArjunaCRM-Container neu.
...
- Unterstützung aus der Community:
  ...
  Wenden Sie sich an die [ArjunaCRM Community](https://github.com/vedpragyabharat/arjuna/issues) oder [Support-Kanäle](https://discord.gg/cx5n4Jzs57) für Unterstützung.