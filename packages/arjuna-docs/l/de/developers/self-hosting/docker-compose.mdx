...
<Warning>
Docker containers are for production hosting or self-hosting, for the contribution please check the [Local Setup](https://docs.vedpragya.com/l/de/developers/local-setup).
</Warning>

## Übersicht

This guide provides step-by-step instructions to install and configure the ArjunaCRM application using Docker Compose. Das Ziel ist, den Prozess übersichtlich zu gestalten und häufige Fallstricke zu vermeiden, die Ihre Einrichtung unterbrechen könnten.
...
See docs [Setup Environment Variables](https://docs.vedpragya.com/l/de/developers/self-hosting/setup) for advanced configuration. All environment variables must be declared in the docker-compose.yml file at the server and / or worker level depending on the variable.
...
Installieren Sie die neueste stabile Version von ArjunaCRM mit einem einzigen Befehl:
...
If you host arjunaCRM on your own computer, open your browser and navigate to [http://localhost:3000](http://localhost:3000).
...
### ArjunaCRM für externen Zugriff freigeben

Standardmäßig läuft ArjunaCRM auf `localhost` an Port `3000`. Um über eine externe Domain oder IP-Adresse darauf zuzugreifen, müssen Sie die `SERVER_URL` in Ihrer `.env`-Datei konfigurieren.
...
SSL (HTTPS) ist erforderlich, damit bestimmte Browserfunktionen ordnungsgemäß funktionieren. Obwohl diese Funktionen möglicherweise während der lokalen Entwicklung funktionieren (da Browser localhost anders behandeln), ist eine ordnungsgemäße SSL-Einrichtung erforderlich, wenn ArjunaCRM auf einer regulären Domain gehostet wird.
...
Wir empfehlen dringend, ArjunaCRM hinter einem Reverse-Proxy mit SSL-Beendigung für optimale Sicherheit und Funktionalität zu konfigurieren.
...
Sollten Sie auf ein Problem stoßen, schauen Sie sich die [Fehlerbehebung](https://docs.vedpragya.com/l/de/developers/self-hosting/troubleshooting) für Lösungen an.
