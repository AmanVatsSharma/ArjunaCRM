...
1. Schalten Sie ArjunaCRM in einem Terminal auf dem Host, auf dem ArjunaCRM läuft, mit `docker compose down` aus.
...
3. Schalten Sie ArjunaCRM mit `docker compose up -d` wieder ein.
...
Wenn Sie nach dem Upgrade auf Laufzeitprobleme stoßen, müssen Sie möglicherweise Ihren Cache leeren, um sicherzustellen, dass er mit den neuesten Änderungen synchronisiert ist. Führen Sie diesen Befehl in Ihrem ArjunaCRM-Server-Container aus:
...
Aktualisieren Sie Ihre ArjunaCRM-Instanz, um das v0.55-Image zu verwenden.
...
Führen Sie dies in Ihrem `arjuna-server`-Container aus:

```bash
yarn command:prod cache:flush
```

Dieses Problem ist spezifisch für diese ArjunaCRM-Version und sollte bei zukünftigen Upgrades nicht erforderlich sein.
...
Es sollte immer im Bereich Ihrer aktuellen ArjunaCRM-Instanz `major.minor`-Version liegen, Sie können Ihre Instanzversion im Admin-Panel (unter `/settings/admin-panel`, zugänglich, wenn Ihr Benutzer die Eigenschaft `canAccessFullAdminPanel` in der Datenbank auf true gesetzt hat) oder durch Ausführen von `echo $APP_VERSION` in Ihrem `arjuna-server`-Container anzeigen.

Um eine desynchronisierte Arbeitsbereichsversion zu korrigieren, müssen Sie von der entsprechenden ArjunaCRM-Version aus aktualisieren, indem Sie die zugehörige Upgrade-Anleitung der Reihe nach befolgen, bis Sie die gewünschte Version erreichen.
...
Aktualisieren Sie Ihre ArjunaCRM-Instanz, um das v0.52-Image zu verwenden.
...
Sie müssen Ihre Arbeitsbereichsversion manuell auf `0.51.0` in der Datenbank aktualisieren und mit der ArjunaCRM-Version `0.52.11` gemäß der obigen Upgrade-Anleitung aktualisieren.

### v0.50 bis v0.51

Aktualisieren Sie Ihre ArjunaCRM-Instanz, um das v0.51-Image zu verwenden.
...
Aktualisieren Sie Ihre ArjunaCRM-Instanz, um das v0.50.0-Image zu verwenden.
...
Bitte aktualisieren Sie Ihre lokale `docker-compose.yml` mit der [docker-compose.yml v0.50.0](https://github.com/vedpragyabharat/arjuna/blob/v0.50.0/packages/arjuna-docker/docker-compose.yml)

### v0.43.0 bis v0.44.0

Aktualisieren Sie Ihre ArjunaCRM-Instanz, um das v0.44.0-Image zu verwenden.
...
Aktualisieren Sie Ihre ArjunaCRM-Instanz, um das v0.43.0-Image zu verwenden.
...
2. Aktualisieren Sie Ihre docker-compose.yml, um das postgres:16-Image gemäß der [docker-compose.yml](https://raw.githubusercontent.com/vedpragyabharat/arjuna/main/packages/arjuna-docker/docker-compose.yml) zu verwenden.
...
Aktualisieren Sie Ihre ArjunaCRM-Instanz, um das v0.42.0-Image zu verwenden.
...
Aktualisieren Sie Ihre ArjunaCRM-Instanz, um das v0.41.0-Image zu verwenden.
...
Aktualisieren Sie Ihre ArjunaCRM-Instanz, um das v0.40.0-Image zu verwenden.
...
Aktualisieren Sie Ihre ArjunaCRM-Instanz, um das v0.35.0-Image zu verwenden.
...
Aktualisieren Sie Ihre ArjunaCRM-Instanz, um das v0.34.0-Image zu verwenden.
...
Aktualisieren Sie Ihre ArjunaCRM-Instanz, um das v0.33.0-Image zu verwenden.
...
Ab dieser Version wurde das arjuna-postgres-Image für DB veraltet und es wird stattdessen arjuna-postgres-spilo verwendet.
Wenn Sie weiterhin das arjuna-postgres-Image verwenden möchten, ersetzen Sie einfach `arjunacrm/arjuna-postgres:${TAG}` durch `arjunacrm/arjuna-postgres` in docker-compose.yml.

### v0.31.0 bis v0.32.0

Aktualisieren Sie Ihre ArjunaCRM-Instanz, um das v0.32.0-Image zu verwenden.
...
Aktualisieren Sie Ihre ArjunaCRM-Instanz, um das v0.31.0-Image zu verwenden.
...
Aktualisieren Sie Ihre ArjunaCRM-Instanz, um das v0.30.0-Image zu verwenden.

**Breaking change**:
To enhance performances, ArjunaCRM now requires redis cache to be configured. Wir haben unser [docker-compose.yml](https://raw.githubusercontent.com/vedpragyabharat/arjuna/main/packages/arjuna-docker/docker-compose.yml) aktualisiert, um dies zu reflektieren.
...
Aktualisieren Sie Ihre ArjunaCRM-Instanz, um das v0.24.0-Image zu verwenden.
...
Aktualisieren Sie Ihre ArjunaCRM-Instanz, um das v0.23.0-Image zu verwenden.
...
Aktualisieren Sie Ihre ArjunaCRM-Instanz, um das v0.22.0-Image zu verwenden.
