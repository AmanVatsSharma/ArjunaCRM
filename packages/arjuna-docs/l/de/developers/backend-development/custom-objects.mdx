...
Objekte sind Strukturen, die es Ihnen ermöglichen, Daten (Aufzeichnungen, Attribute und Werte) zu speichern, die spezifisch für eine Organisation sind. ArjunaCRM bietet sowohl Standard- als auch benutzerdefinierte Objekte.

Standard objects are in-built objects with a set of attributes available for all users. Beispiele für Standardobjekte in ArjunaCRM sind Unternehmen und Person. Standardobjekte verfügen über Standardfelder, die ebenfalls allen ArjunaCRM-Nutzern zur Verfügung stehen, wie z.B. Unternehmen.displayName.
