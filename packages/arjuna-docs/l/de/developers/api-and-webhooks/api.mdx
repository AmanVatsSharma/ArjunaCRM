...
Die ArjunaCRM API ermöglicht Entwicklern die programmgesteuerte Interaktion mit der ArjunaCRM CRM-Plattform. Mit der API können Sie ArjunaCRM mit anderen Systemen integrieren, die Datensynchronisation automatisieren und benutzerdefinierte Lösungen rund um Ihre Kundendaten entwickeln. Die API bietet Endpunkte zum **Erstellen, Lesen, Aktualisieren und Löschen** von zentralen CRM-Objekten (wie Personen und Unternehmen) sowie zum Zugriff auf Metadatenkonfigurationen.

**API-Spielplatz:** Sie können jetzt auf den API-Spielplatz in den Einstellungen der App zugreifen. To try out API calls in real-time, log in to your ArjunaCRM workspace and navigate to **Settings → APIs & Webhooks**. Dies öffnet den integrierten API-Spielplatz und die Einstellungen für API-Schlüssel.
**[Zu den API-Einstellungen gehen](https://app.vedpragya.com/settings)**

## Authentifizierung

Die API von ArjunaCRM verwendet API-Schlüssel zur Authentifizierung. Jede Anfrage an geschützte Endpunkte muss einen API-Schlüssel im Header enthalten.

- **API Keys:** You can generate a new API key from your ArjunaCRM app’s **API settings** page. Jeder API-Schlüssel ist ein geheimes Token, das Zugriff auf Ihre CRM-Daten gewährt, daher sollten Sie es sicher aufbewahren. Wenn ein Schlüssel kompromittiert ist, widerrufen Sie ihn in den Einstellungen und generieren Sie einen neuen.
...
- **Cloud:** `https://api.vedpragya.com/` oder Ihre benutzerdefinierte Domain/Subdomain
