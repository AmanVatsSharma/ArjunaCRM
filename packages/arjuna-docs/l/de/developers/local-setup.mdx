...
description: "Der Leitfaden für Mitwirkende (oder neugierige Entwickler), die ArjunaCRM lokal ausführen möchten."
...
Bevor Sie ArjunaCRM installieren und verwenden können, stellen Sie sicher, dass Sie Folgendes auf Ihrem Computer installiert haben:
...
ArjunaCRM benötigt einen Redis-Cache, um die beste Leistung zu bieten
...
Verwenden Sie Umgebungsvariablen oder `.env`-Dateien, um Ihr Projekt zu konfigurieren. Weitere Informationen [hier](https://docs.vedpragya.com/l/de/developers/self-hosting/setup)
...
Um den ArjunaCRM-Server zu bauen und einige Daten in Ihre Datenbank zu seeden, führen Sie den folgenden Befehl aus:
...
## Schritt 8: Verwenden Sie ArjunaCRM

**Frontend**

Das Frontend von ArjunaCRM läuft unter [http://localhost:3001](http://localhost:3001).
Sie können sich mit dem Standard-Demokonto anmelden: `tim@apple.dev` (Passwort: `tim@apple.dev`)

**Backend**

- Der Server von ArjunaCRM wird unter [http://localhost:3000](http://localhost:3000) verfügbar sein.
