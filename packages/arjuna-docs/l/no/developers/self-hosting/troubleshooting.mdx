...
Hvis du har en eksisterende ArjunaCRM-instans med produksjonsdata, **IKKE** følg disse trinnene da de vil slette databasen din permanent!

Når du installerer ArjunaCRM for første gang, vil du kanskje endre det forhåndsinnstilte databasepassordet.
...
Under ArjunaCRM-installasjonen må du forsyne postgres-databasen din med de riktige schemaene, utvidelsene og brukerne.
...
#### Kan ikke finne modul 'arjuna-emails' eller de tilsvarende typedeklarasjonene.

Du må bygge pakken `arjuna-emails` før du kjører initialiseringen av databasen med `npx nx run arjuna-emails:build`

#### Manglende arjuna-x-pakke

Forsikre deg om at du kjører yarn i rotkatalogen, og deretter `npx nx server:dev arjuna-server`. Hvis dette fortsatt ikke fungerer, prøv å bygge den manglende pakken manuelt.
...
#### Mens du kjører `npx nx start` eller `npx nx start arjuna-front`, kan det oppstå en minnefeil

I `packages/arjuna-front/.env` fjern kommentar fra `VITE_DISABLE_TYPESCRIPT_CHECKER=true` og `VITE_DISABLE_ESLINT_CHECKER=true` for å deaktivere bakgrunnssjekker og dermed redusere RAM-behovet.

**Hvis det ikke fungerer:**  
Kun kjør tjenestene du trenger, i stedet for `npx nx start`. For eksempel, hvis du jobber på serveren, kjør bare `npx nx worker arjuna-server`

**Hvis det ikke fungerer:**  
Hvis du prøvde å kjøre bare `npx nx run arjuna-server:start` på WSL og det feiler med feilmeldingen nedenfor om minne:
...
**Hvis det ikke fungerer:**  
Undersøk hvilke prosesser som bruker det meste av din maskins RAM. Hos ArjunaCRM merket vi at noen VScode-utvidelser brukte mye RAM, så vi deaktiverte dem midlertidig.
...
- Åpne din ArjunaCRM-app, naviger til `/settings`, og aktiver **Avansert** bryteren nederst til venstre på skjermen.
- Opprett en ny webhook.
- Lim inn **Din unike Webhook-URL** i **Endpoint Url**-feltet i ArjunaCRM. Sett **Filtre** til `Selskaper` og `Opprettet`.
<div style={{textAlign: 'center'}}>
...
Kommenter ut checker plugin i `packages/arjuna-ui/vite-config.ts` som i eksempelet nedenfor
...
Hvis du kjører ArjunaCRM bak en omvendt proxy og opplever tilkoblingsproblemer:
...
2. **Sjekk innstillinger for omvendt proxy:**
   ...
   - Kontroller at din omvendte proxy sender forespørslene riktig til ArjunaCRM-serveren.
   - Sørg for at headere som `X-Forwarded-For` og `X-Forwarded-Proto` er riktig satt.

3. **Start tjenester på nytt:**

   Etter å ha gjort endringer, start både den omvendte proxyen og ArjunaCRM-konteinerne på nytt.
...
- Fellesskapsstøtte:
  ...
  Ta kontakt med [ArjunaCRM-fellesskapet](https://github.com/vedpragyabharat/arjuna/issues) eller [støttekanaler](https://discord.gg/cx5n4Jzs57) for hjelp.