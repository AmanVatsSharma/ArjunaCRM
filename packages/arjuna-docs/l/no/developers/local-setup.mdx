...
Før du kan installere og bruke ArjunaCRM, sørg for at du installerer følgende på datamaskinen din:
...
ArjunaCRM krever et redis cache for å gi best mulig ytelse
...
Bruk miljøvariabler eller `.env` filer for å konfigurere prosjektet ditt. Mer info [her](https://docs.vedpragya.com/l/no/developers/self-hosting/setup)
...
For å bygge ArjunaCRM serveren og sette inn noen data i databasen din, kjør følgende kommando:
...
## Steg 8: Bruk ArjunaCRM

**Frontend**

ArjunaCRMs frontend kjører på [http://localhost:3001](http://localhost:3001).
Du kan logge inn med standard demokonto: `tim@apple.dev` (passord: `tim@apple.dev`)

**Backend**

- ArjunaCRM-serveren vil være i gang på [http://localhost:3000](http://localhost:3000)
