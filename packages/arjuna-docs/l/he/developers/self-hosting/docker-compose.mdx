...
<Warning>
Docker containers are for production hosting or self-hosting, for the contribution please check the [Local Setup](https://docs.vedpragya.com/l/he/developers/local-setup).
</Warning>

## סקירה כללית

מדריך זה מספק הוראות שלב-אחר-שלב להתקנה ולהגדרת אפליקציית ArjunaCRM באמצעות Docker Compose. המטרה היא להפוך את התהליך לפשוט למניעת תקלות נפוצות שעלולות לפגוע בהגדרתך.

**חשוב:** שנה הגדרות רק אם הן מצויינות במדריך זה. שינוי תצורות אחרות עלול להוביל לבעיות.

See docs [Setup Environment Variables](https://docs.vedpragya.com/l/he/developers/self-hosting/setup) for advanced configuration. All environment variables must be declared in the docker-compose.yml file at the server and / or worker level depending on the variable.
...
התקן את הגרסה היציבה האחרונה של ArjunaCRM באמצעות פקודה אחת:
...
### חשוף את ArjunaCRM לגישה חיצונית

ברירת המחדל, ArjunaCRM רץ על `localhost` ב-port `3000`. כדי לגשת באמצעות דומיין חיצוני או כתובת IP, יש להגדיר את `SERVER_URL` בקובץ `.env` שלך.
...
SSL (HTTPS) נדרש כדי שתכונות מסוימות בדפדפן יפעלו כראוי. בעוד שתכונות אלו עשויות לפעול במהלך פיתוח מקומי (כי דפדפנים מטפלים ב-localhost באופן שונה), יש להגדיר SSL בצורה נכונה כאשר מאחסנים ArjunaCRM על דומיין רגיל.

לדוגמה, ה-clipboard API עשוי לדרוש הקשר מאובטח - תכונות מסוימות כמו כפתורי העתקה בכל האפליקציה עשויות לא לעבוד ללא HTTPS.

אנו ממליצים בחום לאחסן את ArjunaCRM מאחורי פרוקסי הפוך עם ניתוק SSL להשגת אבטחה מיטבית ופונקציונליות.
...
אם אתה נתקל בכל בעיה, בדוק את [פתרון תקלות](https://docs.vedpragya.com/l/he/developers/self-hosting/troubleshooting) לפתרונות.
