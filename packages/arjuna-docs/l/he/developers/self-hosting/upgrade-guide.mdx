...
1. בטרמינל, על המחשב המארח בו ArjunaCRM פועלת, כבה את ArjunaCRM: `docker compose down`

2. שדרוג את הגרסה על ידי שינוי ערך ה-`TAG` בקובץ `.env` ליד הקובץ docker-compose שלך. ( מומלץ לצרוך גרסת `major.minor` כמו `v0.53` )

3. העלה את ArjunaCRM בחזרה עם `docker compose up -d`
...
אם אתה נתקל בבעיות בזמן ריצה לאחר השדרוג, ייתכן שתצטרך לנקות את המטמון שלך כדי לוודא שהוא מסונכרן עם השינויים האחרונים. הרץ את הפקודה במיכל השרת שלך ב-ArjunaCRM:
...
שדרג את מופע ה-ArjunaCRM שלך לשימוש בתמונת v0.55
...
במיכל השרת שלך ב-ArjunaCRM, הרץ:

```bash
yarn command:prod cache:flush
```

בעיה זו ספציפית לגרסה זו של ArjunaCRM ולא אמורה להידרש בשדרוגים עתידיים.
...
הוא תמיד צריך להיות בטווח של גרסת `major.minor` הנוכחית של מופע ArjunaCRM שלך, תוכל לבדוק את גרסת המופע שלך בפאנל הניהול (ב`/settings/admin-panel`, נגיש אם ההגדרה `canAccessFullAdminPanel` של המשתמש שלך מוגדרת כ-true בבסיס הנתונים) או על ידי הרצת `echo $APP_VERSION` במיכל השרת שלך.

כדי לתקן גרסת מרחב עבודה אונכתפית, תצטרך לשדרג מהגרסה הרלוונטית של ArjunaCRM על פי מדריך השדרוג המתאים ברציפות וכך הלאה עד שתגיע לגרסה הרצויה.
...
שדרג את מופע ה-ArjunaCRM שלך לשימוש בתמונת v0.52
...
יהיה עליך לעדכן ידנית את גרסת מרחב העבודה שלך ל-`0.51.0` בבסיס הנתונים ולשדרג בשימוש בגרסת arjuna `0.52.11` לפי מדריך השדרוג שלה הנמצא מעל.

### v0.50 ל-v0.51

שדרג את מופע ה-ArjunaCRM שלך לשימוש בתמונת v0.51
...
שדרג את מופע ה-ArjunaCRM שלך לשימוש בתמונת v0.50.0
...
אנא עדכן את ה`docker-compose.yml` המקומי שלך עם [v0.50.0 docker-compose.yml](https://github.com/vedpragyabharat/arjuna/blob/v0.50.0/packages/arjuna-docker/docker-compose.yml)

### v0.43.0 ל-v0.44.0

שדרג את מופע ה-ArjunaCRM שלך לשימוש בתמונת v0.44.0
...
שדרג את מופע ה-ArjunaCRM שלך לשימוש בתמונת v0.43.0
...
2. שדרג את ה-docker-compose.yml שלך לשימוש בתמונת postgres:16 כפי שבקובץ [docker-compose.yml](https://raw.githubusercontent.com/vedpragyabharat/arjuna/main/packages/arjuna-docker/docker-compose.yml).
...
שדרג את מופע ה-ArjunaCRM שלך לשימוש בתמונת v0.42.0
...
שדרג את מופע ה-ArjunaCRM שלך לשימוש בתמונת v0.41.0
...
שדרג את מופע ה-ArjunaCRM שלך לשימוש בתמונת v0.40.0
...
שדרג את מופע ה-ArjunaCRM שלך לשימוש בתמונת v0.35.0
...
שדרג את מופע ה-ArjunaCRM שלך לשימוש בתמונת v0.34.0
...
שדרג את מופע ה-ArjunaCRM שלך לשימוש בתמונת v0.33.0
...
החל מגרסה זו, התמונה arjuna-postgres עבור בסיס הנתונים נעשתה מיושנת ויש להשתמש בתמונה arjuna-postgres-spilo במקום.
אם אתה מעוניין להמשיך להשתמש בתמונה arjuna-postgres, פשוט החלף את `arjunacrm/arjuna-postgres:${TAG}` ב-arjunacrm/arjuna-postgres בתיקיית docker-compose.yml.

### v0.31.0 ל-v0.32.0

שדרג את מופע ה-ArjunaCRM שלך לשימוש בתמונת v0.32.0
...
שדרג את מופע ה-ArjunaCRM שלך לשימוש בתמונת v0.31.0
...
שדרג את מופע ה-ArjunaCRM שלך לשימוש בתמונת v0.30.0

**Breaking change**:
To enhance performances, ArjunaCRM now requires redis cache to be configured. עדכנו את [docker-compose.yml](https://raw.githubusercontent.com/vedpragyabharat/arjuna/main/packages/arjuna-docker/docker-compose.yml) כדי לשקף זאת.
...
שדרג את מופע ה-ArjunaCRM שלך לשימוש בתמונת v0.24.0
...
שדרג את מופע ה-ArjunaCRM שלך לשימוש בתמונת v0.23.0
...
שדרג את מופע ה-ArjunaCRM שלך לשימוש בתמונת v0.22.0
