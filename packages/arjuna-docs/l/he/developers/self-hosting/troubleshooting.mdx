...
אם יש לך כבר מקרה ArjunaCRM עם נתוני ייצור, **אל** תעקוב אחר שלבים אלו כי הם ימחקו לצמיתות את המאגר שלך!

כאשר אתה מתקין את ArjunaCRM בפעם הראשונה, ייתכן שתרצה לשנות את סיסמת המאגר המוגדרת כברירת מחדל.
...
במהלך התקנת ArjunaCRM, עליך לספק את מאגר postgres שלך עם הסכימות, התוספות והמשתמשים הנכונים.
...
#### לא ניתן למצוא את המודול 'arjuna-emails' או את ההצהרות הסוג המתאימות שלו.

עליך לבנות את החבילה `arjuna-emails` לפני הרצת ההכנה של המאגר עם `npx nx run arjuna-emails:build`.

#### חבילת arjuna-x חסרה

ודא להריץ yarn בתיקיית השורש ולאחר מכן להריץ `npx nx server:dev arjuna-server`.  אם זה עדיין לא עובד, נסה לבנות את החבילה החסרה באופן ידני.
...
#### בעת הרצת `npx nx start` או `npx nx start arjuna-front`, מופיעה שגיאה של מחסור בזיכרון
...
**אם זה לא עובד:**
הרץ רק את השירותים שאתה צריך, במקום `npx nx start`. למשל, אם אתה עובד על השרת, הרץ רק `npx nx worker arjuna-server`.

**אם זה לא עובד:**
אם ניסית להפעיל רק `npx nx run arjuna-server:start` ב-WSL ולא הצלחת בגלל שגיאת זיכרון להלן:
...
**אם זה לא עובד:**
בדוק אילו תהליכים לוקחים לך את מירב הזיכרון RAM במכונה שלך. ב־ArjunaCRM, שמנו לב שכמה תוספות של VScode לקחו הרבה RAM ולכן השבתנו אותן באופן זמני.
...
- פתח את האפליקציה של ArjunaCRM, נווט ל-`/settings`, והפעל את המתג **מתקדם** בפינה השמאלית התחתונה של המסך.
- Create a new webhook.
- הדבק את **כתובת האתר הייחודית של ה-Webhook שלך** בשדה **Endpoint Url** ב-ArjunaCRM. הגדר את ה-**מסננים** ל`Companies` ו`Created`.
<div style={{textAlign: 'center'}}>
...
אם אתם מריצים ArjunaCRM מאחורי פרוקסי הפוך ויש בעיות חיבור:
...
2. **בדוק את הגדרות הפרוקסי ההפוך:**
   ...
   - וודא שהפרוקסי ההפוך שלכם מפנה נכון את הבקשות לשרת של ArjunaCRM.
   - ודא ש-headers כמו `X-Forwarded-For` ו-`X-Forwarded-Proto` מוגדרים כראוי.

3. **הפעל מחדש את השירותים:**

   לאחר ביצוע שינוי, הפעל מחדש את שני הפרוקסי ההפוך ומיכלי ArjunaCRM.
...
- תמיכה קהילתית:
  ...
  פנו לקהילה של [ArjunaCRM](https://github.com/vedpragyabharat/arjuna/issues) או ערוצי התמיכה [support channels](https://discord.gg/cx5n4Jzs57) לקבלת עזרה.