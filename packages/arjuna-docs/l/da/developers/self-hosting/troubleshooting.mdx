...
Hvis du har en eksisterende ArjunaCRM-instans med produktionsdata, **FØLG IKKE** disse trin, da de permanent vil slette din database!

Mens du installerer ArjunaCRM for første gang, kan du skifte standarddatabaseadgangskoden.
...
Under ArjunaCRM-installationen skal du forsyne din postgres-database med de korrekte skemaer, udvidelser og brugere.
...
#### Kan ikke finde modul 'arjuna-emails' eller dets tilsvarende typedeklarationer.

Du skal bygge pakken `arjuna-emails` før du kører initialiseringen af databasen med `npx nx run arjuna-emails:build`

#### Mangler arjuna-x pakke

Sørg for at køre yarn i rodmappen og derefter køre `npx nx server:dev arjuna-server`. Hvis dette stadig ikke virker, prøv at bygge den manglende pakke manuelt.
...
#### Mens du kører `npx nx start` eller `npx nx start arjuna-front`, kastes der en 'Out of memory' fejl

I `packages/arjuna-front/.env` fjern kommenteringen af `VITE_DISABLE_TYPESCRIPT_CHECKER=true` og `VITE_DISABLE_ESLINT_CHECKER=true` for at deaktivere baggrundskontroller, hvilket reducerer behovet for RAM.

**Hvis det ikke virker:**  
Kør kun de tjenester, du har brug for, i stedet for `npx nx start`. For eksempel, hvis du arbejder på serveren, kør bare `npx nx worker arjuna-server`

**Hvis det ikke virker:**  
Hvis du forsøgte at køre kun `npx nx run arjuna-server:start` på WSL, og det fejler med nedenstående hukommelsesfejl:
...
**Hvis det ikke virker:**  
Undersøg hvilke processer, der tager mest af din maskins RAM. Hos ArjunaCRM bemærkede vi, at nogle VScode-udvidelser tog meget RAM, så vi deaktiverede dem midlertidigt.
...
- Åbn din ArjunaCRM-app, naviger til `/settings`, og aktiver **Avanceret** skiftet nederst til venstre på skærmen.
- Opret en ny webhook.
- Indsæt **Din Unikke Webhook-URL** i **Endpoint Url**-feltet i ArjunaCRM. Sæt **Filtre** til `Companies` og `Created`.
<div style={{textAlign: 'center'}}>
...
Hvis du kører ArjunaCRM bag en reverse proxy og oplever forbindelsesproblemer:
...
2. **Tjek indstillinger for reverse proxy:**
   ...
   - Bekræft, at din reverse proxy korrekt videresender forespørgsler til ArjunaCRM-serveren.
   - Sørg for at overskrifter som `X-Forwarded-For` og `X-Forwarded-Proto` er korrekt indstillet.

3. **Genstart tjenester:**

   Efter at have foretaget ændringer skal du genstarte både reverse proxy og ArjunaCRM containere.
...
- Fællesskabsstøtte:
  ...
  Kontakt [ArjunaCRM-fællesskabet](https://github.com/vedpragyabharat/arjuna/issues) eller [supportkanaler](https://discord.gg/cx5n4Jzs57) for hjælp.