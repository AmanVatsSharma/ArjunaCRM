...
1. Åbn en terminal på værtsmaskinen, hvor ArjunaCRM kører, og sluk for ArjunaCRM: `docker compose down`
...
3. Bring ArjunaCRM tilbage online med `docker compose up -d`
...
Hvis du støder på runtime-problemer efter opgradering, kan det være nødvendigt at tømme din cache for at sikre, at den er synkroniseret med de seneste ændringer. Kør denne kommando i din arjuna-server-konteiner:
...
Opgrader din ArjunaCRM-instans til at bruge v0.55-billedet
...
In your `arjuna-server` container, run:

```bash
yarn command:prod cache:flush
```

Dette problem er specifikt for denne ArjunaCRM-version og bør ikke være nødvendigt for fremtidige opgraderinger.
...
Den bør altid være inden for din aktuelle ArjunaCRMs instans `major.minor` version, du kan se din instans version i administrationspanelet (på `/settings/admin-panel`, tilgængelig hvis din brugers `canAccessFullAdminPanel` ejendom er indstillet til sand i databasen) eller ved at køre `echo $APP_VERSION` i din `arjuna-server` container.

For at rette en desynkroniseret arbejdsområdeversion skal du opgradere fra den tilsvarende ArjunaCRMs version ved at følge den relaterede opgraderingsvejledning sekventielt og så videre, indtil den når den ønskede version.
...
Opgrader din ArjunaCRM-instans for at bruge v0.52-billedet
...
Opgrader din ArjunaCRM-instans for at bruge v0.51-billedet
...
Opgrader din ArjunaCRM-instans for at bruge v0.50.0-billedet
...
Venligst opdater din lokale `docker-compose.yml` med [v0.50.0 docker-compose.yml](https://github.com/vedpragyabharat/arjuna/blob/v0.50.0/packages/arjuna-docker/docker-compose.yml)

### v0.43.0 til v0.44.0

Opgrader din ArjunaCRM-instans for at bruge v0.44.0-billedet
...
Opgrader din ArjunaCRM-instans for at bruge v0.43.0-billedet
...
2. Opgrader din docker-compose.yml for at bruge postgres:16-billedet som i [docker-compose.yml](https://raw.githubusercontent.com/vedpragyabharat/arjuna/main/packages/arjuna-docker/docker-compose.yml) filen.
...
Opgrader din ArjunaCRM-instans for at bruge v0.42.0-billedet
...
Opgrader din ArjunaCRM-instans for at bruge v0.41.0-billedet
...
Opgrader din ArjunaCRM-instans for at bruge v0.40.0-billedet
...
Opgrader din ArjunaCRM-instans for at bruge v0.35.0-billedet
...
Opgrader din ArjunaCRM-instans for at bruge v0.34.0-billedet
...
Opgrader din ArjunaCRM-instans for at bruge v0.33.0-billedet
...
Fra denne version blev arjuna-postgres-billedet til DB deprecieret, og arjuna-postgres-spilo bruges i stedet.
Hvis du vil fortsætte med at bruge arjuna-postgres-billedet, skal du blot erstatte `arjunacrm/arjuna-postgres:${TAG}` med `arjunacrm/arjuna-postgres` i docker-compose.yml.

### v0.31.0 til v0.32.0

Opgrader din ArjunaCRM-instans for at bruge v0.32.0-billedet
...
Opgrader din ArjunaCRM-instans for at bruge v0.31.0-billedet
...
Opgrader din ArjunaCRM-instans for at bruge v0.30.0-billedet

**Breaking change**:
To enhance performances, ArjunaCRM now requires redis cache to be configured. Vi har opdateret vores [docker-compose.yml](https://raw.githubusercontent.com/vedpragyabharat/arjuna/main/packages/arjuna-docker/docker-compose.yml) for at afspejle dette.
...
Opgrader din ArjunaCRM-instans for at bruge v0.24.0-billedet
...
Opgrader din ArjunaCRM-instans for at bruge v0.23.0-billedet
...
Opgrader din ArjunaCRM-instans for at bruge v0.22.0-billedet
