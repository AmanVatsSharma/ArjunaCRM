...
<Warning>
Docker containers are for production hosting or self-hosting, for the contribution please check the [Local Setup](https://docs.vedpragya.com/l/ca/developers/local-setup).
</Warning>

## Visió general

Aquesta guia proporciona instruccions pas a pas per instal·lar i configurar l'aplicació ArjunaCRM utilitzant Docker Compose. L'objectiu és fer que el procés sigui senzill i evitar errors comuns que podrien trencar la vostra configuració.
...
Consulteu la documentació [Configuració de Variables d'Entorn](https://docs.vedpragya.com/l/ca/developers/self-hosting/setup) per a configuracions avançades. Totes les variables d'entorn han de ser declarades al fitxer docker-compose.yml al nivell de servidor i / o de treballador, depenent de la variable.
...
Instal·leu la versió estable més recent de ArjunaCRM amb una sola comanda:

```bash
bash <(curl -sL https://raw.githubusercontent.com/vedpragyabharat/arjuna/main/packages/arjuna-docker/scripts/install.sh)
```

Per instal·lar una versió o branca específica:

```bash
VERSION=vx.y.z BRANCH=branch-name bash <(curl -sL https://raw.githubusercontent.com/vedpragyabharat/arjuna/main/packages/arjuna-docker/scripts/install.sh)
```
...
1. **Crear el Fitxer .env**
   ...
   ```bash
   curl -o .env https://raw.githubusercontent.com/vedpragyabharat/arjuna/refs/heads/main/packages/arjuna-docker/.env.example
   ```
...
```bash
curl -o docker-compose.yml https://raw.githubusercontent.com/vedpragyabharat/arjuna/refs/heads/main/packages/arjuna-docker/docker-compose.yml
```
...
Si hosteu arjunaCRM al vostre propi ordinador, obriu el navegador i navegueu a [http://localhost:3000](http://localhost:3000).
...
### Exposar ArjunaCRM a Accés Extern

Per defecte, ArjunaCRM s'executa a `localhost` al port `3000`. Per accedir-hi a través d'un domini extern o adreça IP, heu de configurar el `SERVER_URL` al vostre fitxer `.env`.
...
SSL (HTTPS) és necessari per a que certes característiques del navegador funcionin correctament. Mentre aquestes característiques poden funcionar durant el desenvolupament local (ja que els navegadors tracten localhost de manera diferent), una configuració SSL adequada és necessària quan es hostatja ArjunaCRM en un domini regular.
...
Recomanem fortament configurar ArjunaCRM darrere d'un proxy invers amb terminació SSL per a una seguretat i funcionalitat òptimes.
...
2. **Actualitza el Fitxer `.env`**
   ...
   **Exemples:**
   ...
   - Accés via domini amb SSL:
     ```ini
     SERVER_URL=https://myarjunaapp.com
     ```
...
Si trobeu algun problema, consulteu [Resolució de problemes](https://docs.vedpragya.com/l/ca/developers/self-hosting/troubleshooting) per trobar solucions.
