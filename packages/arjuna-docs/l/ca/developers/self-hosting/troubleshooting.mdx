...
Si ja teniu una instància de ArjunaCRM amb dades en producció, **NO** seguiu aquests passos ja que esborraran permanentment la vostra base de dades!

Durant la primera instal·lació de ArjunaCRM, potser voldreu canviar la contrasenya de la base de dades per defecte.
...
Durant la instal·lació de ArjunaCRM, has de proveir la teva base de dades postgres amb els esquemes, extensions i usuaris adequats.
...
#### No es pot trobar el mòdul 'arjuna-emails' o les seves declaracions de tipus corresponents.

Has de construir el paquet `arjuna-emails` abans d'executar la inicialització de la base de dades amb `npx nx run arjuna-emails:build`

#### Paquet arjuna-x faltant

Assegureu-vos d'executar yarn en el directori arrel i després executar `npx nx server:dev arjuna-server`. Si això encara no funciona, prova de construir manualment el paquet que falta.
...
#### Mentre s'executa `npx nx start` o `npx nx start arjuna-front`, es genera un error de falta de memòria

A `packages/arjuna-front/.env` descomenta `VITE_DISABLE_TYPESCRIPT_CHECKER=true` i `VITE_DISABLE_ESLINT_CHECKER=true` per desactivar les comprovacions en segon pla, reduint així la quantitat de RAM necessària.

**Si no funciona:**  
Executa només els serveis que necessites, en lloc de `npx nx start`. For instance, if you work on the server, run only `npx nx worker arjuna-server`

**Si no funciona:**  
Si heu provat d'executar només `npx nx run arjuna-server:start` en WSL i està fallant amb l'error de memòria següent:
...
**Si no funciona:**  
Investiga quins processos estan ocupant més la RAM de la teva màquina. A ArjunaCRM, vam notar que algunes extensions VScode ocupaven molta RAM, així que les deshabilitem temporalment.
...
- Obre la teva aplicació ArjunaCRM, ves a `/settings` i activa l'interruptor **Avançat** a la part inferior esquerra de la pantalla.
- Crea un nou webhook.
- Enganxa **L'URL únic del teu webhook** al camp **Endpoint Url** a ArjunaCRM. Defineix els **Filtres** a `Companies` i `Created`.
<div style={{textAlign: 'center'}}>
...
Comenti el connector del verificador a `packages/arjuna-ui/vite-config.ts` com en l'exemple següent
...
Si estàs executant ArjunaCRM darrere d'un proxy invers i experimentant problemes de connexió:
...
2. **Check Reverse Proxy Settings:**
   ...
   - Confirm that your reverse proxy is correctly forwarding requests to the ArjunaCRM server.
   - Assegura't que les capçaleres com `X-Forwarded-For` i `X-Forwarded-Proto` estiguin configurades correctament.

3. **Reiniciar Serveis:**

   Després de fer canvis, reinicia tant el proxy invers com els contenidors de ArjunaCRM.
...
- Suport Comunitari:
  ...
  Contacta amb la [comunitat ArjunaCRM](https://github.com/vedpragyabharat/arjuna/issues) o els [canals de suport](https://discord.gg/cx5n4Jzs57) per obtenir ajuda.