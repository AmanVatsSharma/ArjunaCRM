...
description: "La guia per a col·laboradors (o desenvolupadors curiosos) que volen executar ArjunaCRM localment."
...
Abans d'instal·lar i utilitzar ArjunaCRM, assegura't d'instal·lar el següent al teu ordinador:
...
ArjunaCRM requereix una caché de Redis per proporcionar el millor rendiment
...
Use environment variables or `.env` files to configure your project. Més informació [aquí](https://docs.vedpragya.com/l/ca/developers/self-hosting/setup)
...
Per construir el servidor ArjunaCRM i afegir algunes dades a la base de dades, executa la següent comanda:
...
## Pas 8: Utilitzar ArjunaCRM

**Frontend**

El frontend de ArjunaCRM estarà funcionant a [http://localhost:3001](http://localhost:3001).
Podeu iniciar sessió utilitzant el compte de demostració per defecte: `tim@apple.dev` (contrasenya: `tim@apple.dev`)

**Backend**

- El servidor de ArjunaCRM estarà en funcionament a [http://localhost:3000](http://localhost:3000)
