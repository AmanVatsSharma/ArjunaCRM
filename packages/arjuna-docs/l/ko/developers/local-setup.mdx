---
title: 로컬 설정
description: "콘트리뷰터(기여자)나 호기심 많은 개발자를 위한 가이드로서, ArjunaCRM를 로컬에서 실행하고자 하는 분들에게 드리는 안내입니다."
...
ArjunaCRM를 설치하고 사용하기 전에, 먼저 컴퓨터에 다음을 설치하세요:
...
ArjunaCRM는 최상의 성능을 제공하기 위해 redis 캐시가 필요합니다.
...
프로젝트를 구성하기 위해 환경 변수나 `.env` 파일을 사용하세요. 추가 정보는 [여기](https://docs.vedpragya.com/l/ko/developers/self-hosting/setup)에서 확인하십시오
...
ArjunaCRM 서버를 빌드하고 데이터베이스에 몇 가지 데이터를 시드하기 위해, 다음 명령어를 실행하세요:
...
ArjunaCRM의 프론트엔드는 [http://localhost:3001](http://localhost:3001)에서 실행됩니다.
기본 데모 계정을 사용하여 로그인할 수 있습니다: `tim@apple.dev` (비밀번호: `tim@apple.dev`)

**백엔드**

- ArjunaCRM의 서버는 [http://localhost:3000](http://localhost:3000)에서 실행됩니다.
