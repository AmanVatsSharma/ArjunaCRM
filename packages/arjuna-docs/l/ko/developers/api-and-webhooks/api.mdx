...
ArjunaCRM API는 개발자가 ArjunaCRM CRM 플랫폼과 프로그래밍적으로 상호작용하도록 허용합니다. API를 사용하여 ArjunaCRM와 다른 시스템을 통합하고, 데이터 동기화를 자동화하며, 고객 데이터를 중심으로 맞춤 솔루션을 구축할 수 있습니다. API는 핵심 CRM 객체(사람 및 회사와 같은)의 **생성, 읽기, 업데이트 및 삭제**뿐만 아니라 메타데이터 구성에 대한 접근을 제공합니다.

**API 플레이그라운드:** 이제 앱의 설정 내에 있는 API 플레이그라운드에 접근할 수 있습니다. 실시간으로 API 호출을 시도하려면 ArjunaCRM 작업 공간에 로그인하고 **설정 → API 및 Webhooks**로 이동하세요. 이렇게 하면 앱 내 API 플레이그라운드와 API 키 설정이 열립니다.
**[API 설정으로 이동](https://app.vedpragya.com/settings)**

## 인증

ArjunaCRM의 API는 인증을 위해 API 키를 사용합니다. 보호된 엔드포인트에 대한 모든 요청은 헤더에 API 키를 포함해야 합니다.

- **API 키:** ArjunaCRM 앱의 **API 설정** 페이지에서 새 API 키를 생성할 수 있습니다. 각 API 키는 CRM 데이터에 대한 액세스를 제공하는 비밀 토큰이므로 안전하게 보관하세요. 키가 손상된 경우 설정에서 해지하고 새 키를 생성하세요.
...
- **클라우드:** `https://api.vedpragya.com/` 또는 사용자 지정 도메인/서브도메인
