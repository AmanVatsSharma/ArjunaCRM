...
description: "Hướng dẫn cho người đóng góp (hoặc nhà phát triển tò mò) muốn chạy ArjunaCRM cục bộ."
...
Trước khi bạn có thể cài đặt và sử dụng ArjunaCRM, hãy đảm bảo bạn đã cài đặt các phần sau trên máy tính của mình:
...
ArjunaCRM yêu cầu một cache redis để cung cấp hiệu suất tốt nhất
...
Sử dụng biến môi trường hoặc tệp `.env` để cấu hình dự án của bạn. Thêm thông tin [tại đây](https://docs.vedpragya.com/l/vi/developers/self-hosting/setup)
...
## Bước 8: Sử dụng ArjunaCRM

**Frontend**

Giao diện người dùng của ArjunaCRM sẽ chạy tại [http://localhost:3001](http://localhost:3001).
Bạn có thể đăng nhập bằng tài khoản demo mặc định: `tim@apple.dev` (mật khẩu: `tim@apple.dev`)

**Backend**

- Máy chủ ArjunaCRM sẽ hoạt động tại [http://localhost:3000](http://localhost:3000)
...
Nếu bạn gặp bất kỳ vấn đề gì, hãy kiểm tra [Xử lý sự cố](https://docs.vedpragya.com/l/vi/developers/self-hosting/troubleshooting) để tìm giải pháp.