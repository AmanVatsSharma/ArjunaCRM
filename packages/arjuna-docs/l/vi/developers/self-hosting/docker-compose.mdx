...
<Warning>
Docker containers are for production hosting or self-hosting, for the contribution please check the [Local Setup](https://docs.vedpragya.com/l/vi/developers/local-setup).
</Warning>

## Tổng quan

Hướng dẫn này cung cấp các bước cài đặt và cấu hình ứng dụng ArjunaCRM sử dụng Docker Compose. Mục tiêu là làm cho quá trình này đơn giản và tránh những sự cố phổ biến có thể phá vỡ thiết lập của bạn.

**Quan trọng:** Chỉ sửa đổi các cài đặt được nêu cụ thể trong hướng dẫn này. Thay đổi các cấu hình khác có thể dẫn đến sự cố.

Xem tài liệu [Thiết lập Biến môi trường](https://docs.vedpragya.com/l/vi/developers/self-hosting/setup) để cấu hình nâng cao. Tất cả các biến môi trường phải được khai báo trong file docker-compose.yml tại cấp độ máy chủ và / hoặc công nhân tùy thuộc vào biến.
...
Cài đặt phiên bản ổn định mới nhất của ArjunaCRM với một lệnh duy nhất:
...
Nếu bạn lưu trữ arjunaCRM trên máy tính của mình, mở trình duyệt và điều hướng đến [http://localhost:3000](http://localhost:3000).
...
### Mở rộng ArjunaCRM để truy cập bên ngoài

Theo mặc định, ArjunaCRM chạy trên `localhost` tại cổng `3000`. Để truy cập qua tên miền hoặc địa chỉ IP bên ngoài, bạn cần cấu hình `SERVER_URL` trong tệp `.env` của bạn.
...
SSL (HTTPS) được yêu cầu cho một số tính năng trình duyệt hoạt động chính xác. Mặc dù các tính năng này có thể hoạt động trong quá trình phát triển cục bộ (vì trình duyệt xử lý localhost khác nhau), thiết lập SSL đầy đủ là cần thiết khi lưu trữ ArjunaCRM trên tên miền thông thường.
...
Chúng tôi khuyến nghị mạnh mẽ việc thiết lập ArjunaCRM sau proxy ngược với kết thúc SSL để bảo mật và chức năng tối ưu.
...
Nếu gặp bất kỳ vấn đề nào, hãy kiểm tra [Xử lý sự cố](https://docs.vedpragya.com/l/vi/developers/self-hosting/troubleshooting) để tìm giải pháp.
