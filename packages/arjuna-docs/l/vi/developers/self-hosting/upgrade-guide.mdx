...
1. Trong terminal, trên máy chủ nơi ArjunaCRM đang chạy, tắt ArjunaCRM: `docker compose down`
...
3. Kích hoạt lại ArjunaCRM với `docker compose up -d`
...
Nếu bạn gặp bất kỳ vấn đề thời gian chạy nào sau khi nâng cấp, bạn có thể cần làm trống bộ nhớ cache để đảm bảo nó được đồng bộ hóa với các thay đổi mới nhất. Chạy lệnh này trong container arjuna-server của bạn:
...
Nâng cấp phiên bản ArjunaCRM của bạn để sử dụng hình ảnh v0.55
...
Trong container `arjuna-server` của bạn, chạy:

```bash
yarn command:prod cache:flush
```

Vấn đề này chỉ đặc biệt về phiên bản ArjunaCRM này và không yêu cầu cho các nâng cấp trong tương lai.
...
Nó luôn nên nằm trong phạm vi của phiên bản `chính.phụ` hiện tại của ArjunaCRM của bạn, bạn có thể xem phiên bản của mình trong bảng quản trị (tại `/settings/admin-panel`, truy cập nếu người dùng của bạn có thuộc tính `canAccessFullAdminPanel` được đặt thành true trong cơ sở dữ liệu) hoặc bằng cách chạy `echo $APP_VERSION` trong container `arjuna-server` của bạn.

Để sửa một phiên bản không gian làm việc không đồng bộ, bạn sẽ phải nâng cấp từ phiên bản arjuna tương ứng theo hướng dẫn nâng cấp liên quan theo tuần tự và cứ tiếp tục đến khi đạt đến phiên bản mong muốn.
...
Nâng cấp phiên bản ArjunaCRM của bạn để sử dụng hình ảnh v0.52
...
Bạn sẽ phải cập nhật thủ công phiên bản không gian làm việc của mình lên `0.51.0` trong cơ sở dữ liệu và nâng cấp sử dụng phiên bản arjuna `0.52.11` theo hướng dẫn nâng cấp vừa trên.

### v0.50 đến v0.51

Nâng cấp phiên bản ArjunaCRM của bạn để sử dụng hình ảnh v0.51
...
Nâng cấp phiên bản ArjunaCRM của bạn để sử dụng hình ảnh v0.50.0
...
Vui lòng cập nhật `docker-compose.yml` của bạn với [docker-compose.yml v0.50.0](https://github.com/vedpragyabharat/arjuna/blob/v0.50.0/packages/arjuna-docker/docker-compose.yml)

### v0.43.0 đến v0.44.0

Nâng cấp phiên bản ArjunaCRM của bạn để sử dụng hình ảnh v0.44.0
...
Nâng cấp phiên bản ArjunaCRM của bạn để sử dụng hình ảnh v0.43.0
...
2. Nâng cấp docker-compose.yml của bạn để sử dụng hình ảnh postgres:16 như trong tệp [docker-compose.yml](https://raw.githubusercontent.com/vedpragyabharat/arjuna/main/packages/arjuna-docker/docker-compose.yml).
...
Nâng cấp phiên bản ArjunaCRM của bạn để sử dụng hình ảnh v0.42.0
...
Nâng cấp phiên bản ArjunaCRM của bạn để sử dụng hình ảnh v0.41.0
...
Nâng cấp phiên bản ArjunaCRM của bạn để sử dụng hình ảnh v0.40.0
...
Nâng cấp phiên bản ArjunaCRM của bạn để sử dụng hình ảnh v0.35.0
...
Nâng cấp phiên bản ArjunaCRM của bạn để sử dụng hình ảnh v0.34.0
...
Nâng cấp phiên bản ArjunaCRM của bạn để sử dụng hình ảnh v0.33.0
...
Kể từ phiên bản này, hình ảnh arjuna-postgres cho DB đã bị ngừng sử dụng và thay thế bằng arjuna-postgres-spilo.
Nếu bạn muốn tiếp tục sử dụng hình ảnh arjuna-postgres, chỉ cần thay thế `arjunacrm/arjuna-postgres:${TAG}` bằng `arjunacrm/arjuna-postgres` trong docker-compose.yml.

### v0.31.0 đến v0.32.0

Nâng cấp phiên bản ArjunaCRM của bạn để sử dụng hình ảnh v0.32.0
...
Nâng cấp phiên bản ArjunaCRM của bạn để sử dụng hình ảnh v0.31.0
...
Nâng cấp phiên bản ArjunaCRM của bạn để sử dụng hình ảnh v0.30.0

**Breaking change**:
To enhance performances, ArjunaCRM now requires redis cache to be configured. Chúng tôi đã cập nhật [docker-compose.yml](https://raw.githubusercontent.com/vedpragyabharat/arjuna/main/packages/arjuna-docker/docker-compose.yml) của chúng tôi để phản ánh điều này.
...
Nâng cấp phiên bản ArjunaCRM của bạn để sử dụng hình ảnh v0.24.0
...
Nâng cấp phiên bản ArjunaCRM của bạn để sử dụng hình ảnh v0.23.0
...
Nâng cấp phiên bản ArjunaCRM của bạn để sử dụng hình ảnh v0.22.0
