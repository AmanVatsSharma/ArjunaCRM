...
Webhooks trong ArjunaCRM bổ sung cho API bằng cách cho phép **thông báo theo thời gian thực** tới các ứng dụng của bạn khi có sự kiện xảy ra trong CRM của bạn. Thay vì liên tục hỏi API về các thay đổi, bạn có thể thiết lập webhooks để ArjunaCRM **đẩy** dữ liệu tới hệ thống của bạn bất cứ khi nào sự kiện cụ thể xảy ra (ví dụ: khi một bản ghi mới được tạo hoặc một bản ghi hiện có được cập nhật). Điều này giúp các hệ thống bên ngoài đồng bộ với ArjunaCRM ngay lập tức và hiệu quả.

Với webhooks, ArjunaCRM sẽ gửi một yêu cầu HTTP POST tới một URL mà bạn chỉ định, chứa thông tin chi tiết về sự kiện. You can then handle that data in your application (e.g., to update your external database, trigger workflows, or send alerts).

## Thiết lập một Webhook

Để tạo một webhook trong ArjunaCRM, sử dụng cài đặt **APIs & Webhooks** trong ứng dụng ArjunaCRM của bạn:

1. **Điều hướng đến Cài đặt:** Trong ứng dụng ArjunaCRM của bạn, vào **Cài đặt → APIs & Webhooks**.
2. **Tạo một Webhook:** Dưới **Webhooks** nhấp vào **+ Tạo webhook**.
3. **Nhập URL:** Cung cấp URL điểm cuối trên máy chủ của bạn nơi bạn muốn ArjunaCRM gửi yêu cầu webhook. Đây nên là một URL công khai có thể xử lý các yêu cầu POST.
4. **Lưu:** Nhấp **Lưu** để tạo webhook. Webhook mới sẽ hoạt động ngay lập tức.

Bạn có thể tạo nhiều webhooks nếu bạn cần gửi các sự kiện khác nhau tới các điểm cuối khác nhau. Mỗi webhook về cơ bản là một đăng ký cho tất cả các sự kiện liên quan (hiện tại, ArjunaCRM gửi tất cả các loại sự kiện tới URL đã cho; lọc các loại sự kiện cụ thể có thể được cấu hình qua giao diện người dùng). Nếu bạn cần xóa một webhook, bạn có thể xóa nó từ cùng trang cài đặt (chọn webhook và chọn xóa).

## Sự kiện và Payloads

Sau khi một webhook được thiết lập, ArjunaCRM sẽ gửi một yêu cầu HTTP POST tới URL bạn đã chỉ định mỗi khi có sự kiện kích hoạt xảy ra trong dữ liệu CRM của bạn. Những sự kiện phổ biến kích hoạt webhooks bao gồm:
...
**Lưu ý:** Điều quan trọng là đáp lại bằng trạng thái HTTP **2xx** từ điểm cuối webhook của bạn để xác nhận đã nhận thành công. Nếu người gửi webhook của ArjunaCRM không nhận được phản hồi 2xx, nó có thể xem xét việc gửi thất bại. (Trong tương lai, logic thử lại có thể cố gắng gửi lại webhooks bị thất bại, vì vậy luôn cố gắng trả về một 200 OK càng nhanh càng tốt sau khi xử lý dữ liệu.)

## Xác thực Webhook

Để đảm bảo an toàn cho các điểm cuối webhook của bạn, ArjunaCRM bao gồm một chữ ký trong tiêu đề `X-ArjunaCRM-Webhook-Signature`.
...
1. Kết hợp thời gian (từ tiêu đề `X-ArjunaCRM-Webhook-Timestamp`), một dấu hai chấm, và chuỗi JSON của payload
