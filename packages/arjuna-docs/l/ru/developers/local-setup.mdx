...
description: "Руководство для участников (или любопытных разработчиков), которые хотят запускать ArjunaCRM локально."
...
Прежде чем установить и использовать ArjunaCRM, убедитесь, что у вас установлено следующее:
...
ArjunaCRM требует кэша Redis для обеспечения наилучшей производительности
...
Используйте переменные окружения или файлы `.env` для настройки вашего проекта. Подробнее [здесь](https://docs.vedpragya.com/l/ru/developers/self-hosting/setup).
...
Чтобы собрать сервер ArjunaCRM и добавить данные в вашу базу данных, выполните следующую команду:
...
## Шаг 8: Использовать ArjunaCRM

**Frontend**

Фронтенд ArjunaCRM будет работать на [http://localhost:3001](http://localhost:3001).
Вы можете войти, используя учетную запись демо по умолчанию: `tim@apple.dev` (пароль: `tim@apple.dev`)

**Backend**

- Сервер ArjunaCRM будет работать на [http://localhost:3000](http://localhost:3000).
