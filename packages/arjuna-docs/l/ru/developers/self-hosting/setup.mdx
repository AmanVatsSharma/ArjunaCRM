...
<Warning>
**First time installing?** Follow the [Docker Compose installation guide](https://docs.vedpragya.com/l/ru/developers/self-hosting/docker-compose) to get ArjunaCRM running, then return here for configuration.
</Warning>

ArjunaCRM предлагает **два режима конфигурации**, чтобы соответствовать разным потребностям развертывания:
...
1. Получите доступ к вашему экземпляру ArjunaCRM (обычно `http://localhost:3000`)
...
соответствующий исходный код](https://github.com/vedpragyabharat/arjuna/blob/main/packages/arjuna-server/src/engine/core-modules/auth/utils/get-google-apis-oauth-scopes.ts#L4-L10)
...
<Warning>
Пользователи должны иметь [лицензию Microsoft 365](https://admin.microsoft.com/Adminportal/Home), чтобы иметь возможность использовать API Календаря и Сообщений. Они не смогут синхронизировать свою учетную запись в ArjunaCRM без нее.
</Warning>
...
соответствующий исходный код](https://github.com/vedpragyabharat/arjuna/blob/main/packages/arjuna-server/src/engine/core-modules/auth/utils/get-microsoft-apis-oauth-scopes.ts#L2-L9)
