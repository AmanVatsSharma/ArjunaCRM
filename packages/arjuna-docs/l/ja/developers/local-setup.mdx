---
title: ローカルセットアップ
description: "寄稿者（または好奇心旺盛な開発者）のためのガイドで、ローカルにArjunaCRMを実行したい人向けです。"
...
ArjunaCRMをインストールして使用する前に、以下をコンピュータにインストールしてください。
...
ArjunaCRMは、最良のパフォーマンスを提供するためにredisキャッシュを必要とします。
...
環境変数や`.env`ファイルを使ってプロジェクトを設定します。 詳細は[こちら](https://docs.vedpragya.com/l/ja/developers/self-hosting/setup)。 詳細は[こちら](https://docs.vedpragya.com/l/ja/developers/self-hosting/setup)。
...
ArjunaCRMサーバーを構築し、データベースにデータをシードするには、以下のコマンドを実行します：
...
## ステップ 8: ArjunaCRMを使用する

**フロントエンド**

ArjunaCRMのフロントエンドは[http://localhost:3001](http://localhost:3001)で稼働しています。
ArjunaCRMのフロントエンドは[http://localhost:3001](http://localhost:3001)で稼働しています。
デフォルトのデモアカウントを使用してログインできます: `tim@apple.dev` (パスワード: `tim@apple.dev`)

**バックエンド**

- ArjunaCRMのサーバーは[http://localhost:3000](http://localhost:3000)で稼働しています
