...
<Warning>
**First time installing?** Follow the [Docker Compose installation guide](https://docs.vedpragya.com/l/ja/developers/self-hosting/docker-compose) to get ArjunaCRM running, then return here for configuration.
</Warning>

ArjunaCRMは、異なる展開ニーズに合わせて**2つの構成モード**を提供します:
...
1. ArjunaCRMインスタンスにアクセスしてください (通常は `http://localhost:3000`)
2. **設定 / 管理パネル / 構成変数**に移動
...
**必要なスコープ** (自動的に構成される):
[関連ソースコードを見る](https://github.com/vedpragyabharat/arjuna/blob/main/packages/arjuna-server/src/engine/core-modules/auth/utils/get-google-apis-oauth-scopes.ts#L4-L10)
...
カレンダーおよびメッセージングAPIを使用するためには、[Microsoft 365 ライセンス](https://admin.microsoft.com/Adminportal/Home)が必要です。 They will not be able to sync their account on ArjunaCRM without one.
 They will not be able to sync their account on ArjunaCRM without one.
</Warning>
...
[関連ソースコードを見る](https://github.com/vedpragyabharat/arjuna/blob/main/packages/arjuna-server/src/engine/core-modules/auth/utils/get-microsoft-apis-oauth-scopes.ts#L2-L9)
