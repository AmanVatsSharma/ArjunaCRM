...
1. ArjunaCRMを実行しているホスト上のターミナルで、ArjunaCRMを停止します：`docker compose down`
...
3. `docker compose up -d` でArjunaCRMを再起動します。
...
アップグレード後にランタイムの問題が発生した場合は、キャッシュをフラッシュして最新の変更と同期させる必要があるかもしれません。 arjuna-serverコンテナ内で次のコマンドを実行します： arjuna-serverコンテナ内で次のコマンドを実行します：
...
ArjunaCRMインスタンスをv0.55イメージにアップグレードします。
...
あなたの `arjuna-server` コンテナ内で次のコマンドを実行します：

```bash
yarn command:prod cache:flush
```

この問題は特定のArjunaCRMバージョンに固有であり、今後のアップグレードでは不要なはずです。
...
それは常にあなたの現在のArjunaCRMのインスタンスの `major.minor` バージョンの範囲内である必要があります。あなたのインスタンスバージョンは、管理パネル（ `/settings/admin-panel` にあり、データベース内でユーザーが `canAccessFullAdminPanel` プロパティを持っている場合にアクセス可能）にまたは `arjuna-server` コンテナ内で `echo $APP_VERSION` を実行することで確認できます。

非同期化されたワークスペースバージョンを修正するには、対応するArjunaCRMのバージョンから関連するアップグレードガイドに従って順次アップグレードし、それが目的のバージョンに到達するまで続ける必要があります。
...
ArjunaCRMインスタンスをv0.52イメージにアップグレードします。
...
残念ながら、`0.52.0` と `0.52.6` はdockerHubから完全に削除されました。
データベースでワークスペースバージョンを手動で `0.51.0` に更新し、arjunaバージョン `0.52.11` を使用して、その直上のアップグレードガイドに従ってアップグレードする必要があります。

### v0.50からv0.51

ArjunaCRMインスタンスをv0.51イメージにアップグレードします。
...
ArjunaCRMインスタンスをv0.50.0イメージにアップグレードします。
...
ローカルの `docker-compose.yml` を[v0.50.0 docker-compose.yml](https://github.com/vedpragyabharat/arjuna/blob/v0.50.0/packages/arjuna-docker/docker-compose.yml)に更新してください。
ローカルの `docker-compose.yml` を[v0.50.0 docker-compose.yml](https://github.com/vedpragyabharat/arjuna/blob/v0.50.0/packages/arjuna-docker/docker-compose.yml)に更新してください。

### v0.43.0からv0.44.0

ArjunaCRMインスタンスをv0.44.0イメージにアップグレードします。
...
ArjunaCRMインスタンスをv0.43.0イメージにアップグレードします。
...
2. docker-compose.ymlを、新しいpostgres:16イメージを使用するようにアップグレードします。[docker-compose.yml](https://raw.githubusercontent.com/vedpragyabharat/arjuna/main/packages/arjuna-docker/docker-compose.yml)を参照してください。
...
ArjunaCRMインスタンスをv0.42.0イメージにアップグレードします。
...
ArjunaCRMインスタンスをv0.41.0イメージにアップグレードします。
...
ArjunaCRMインスタンスをv0.40.0イメージにアップグレードします。
...
ArjunaCRMインスタンスをv0.35.0イメージにアップグレードします。
...
ArjunaCRMインスタンスをv0.34.0イメージにアップグレードします。
...
ArjunaCRMインスタンスをv0.33.0イメージにアップグレードします。
...
このバージョンから、DB用のarjuna-postgresイメージは廃止され、arjuna-postgres-spiloが代わりに使用されるようになりました。
このバージョンから、DB用のarjuna-postgresイメージは廃止され、arjuna-postgres-spiloが代わりに使用されるようになりました。
arjuna-postgresイメージを使い続けたい場合は、docker-compose.yml内の`arjunacrm/arjuna-postgres:${TAG}`を`arjunacrm/arjuna-postgres`に置き換えるだけで済みます。

### v0.31.0からv0.32.0

ArjunaCRMインスタンスをv0.32.0イメージにアップグレードします。
...
ArjunaCRMインスタンスをv0.31.0イメージにアップグレードします。
...
ArjunaCRMインスタンスをv0.30.0イメージにアップグレードします。

**Breaking change**:
To enhance performances, ArjunaCRM now requires redis cache to be configured. これを反映した[docker-compose.yml](https://raw.githubusercontent.com/vedpragyabharat/arjuna/main/packages/arjuna-docker/docker-compose.yml)を更新しました。
...
ArjunaCRMインスタンスをv0.24.0イメージにアップグレードします。
...
ArjunaCRMインスタンスをv0.23.0イメージにアップグレードします。
...
ArjunaCRMインスタンスをv0.22.0イメージにアップグレードします。
