...
オブジェクトは、組織独自のデータ（記録、属性、値）を保存するための構造です。 ArjunaCRM provides both standard and custom objects. ArjunaCRM provides both standard and custom objects.

標準オブジェクトは、すべてのユーザーが利用可能な一連の属性を備えた組み込みのオブジェクトです。 標準オブジェクトは、すべてのユーザーが利用可能な一連の属性を備えた組み込みのオブジェクトです。 Examples of standard objects in ArjunaCRM include Company and Person. Standard objects have standard fields that are also available for all ArjunaCRM users, like Company.displayName.
