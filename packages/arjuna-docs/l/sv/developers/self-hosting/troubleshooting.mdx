...
Om du har en befintlig ArjunaCRM-instans med produktionsdata, **FÖLJ INTE** dessa steg då de permanent raderar din databas!

När du installerar ArjunaCRM för första gången kan du vilja ändra standardlösenordet för databasen.
...
Under installation av ArjunaCRM måste du förse din postgresdatabas med rätt scheman, tillägg och användare.
...
#### Kan inte hitta modulen 'arjuna-emails' eller dess motsvarande typdeklarationer.

Du måste bygga paketet `arjuna-emails` innan du kör initieringen av databasen med `npx nx run arjuna-emails:build`

#### Missing arjuna-x package

Se till att köra yarn i rotkatalogen och kör sedan `npx nx server:dev arjuna-server`. Om det fortfarande inte fungerar, försök att bygga det saknade paketet manuellt.
...
#### Vid körning av `npx nx start` eller `npx nx start arjuna-front`, får jag ett Minnesfel

I `packages/arjuna-front/.env` avkommentera `VITE_DISABLE_TYPESCRIPT_CHECKER=true` och `VITE_DISABLE_ESLINT_CHECKER=true` för att inaktivera bakgrundskontroller och således minska mängden behövt RAM.

**Om det inte fungerar:**  
Kör endast de tjänster du behöver, istället för `npx nx start`. Till exempel, om du arbetar på servern, kör endast `npx nx worker arjuna-server`

**Om det inte fungerar:**  
Om du försökte köra endast `npx nx run arjuna-server:start` i WSL och det misslyckades med minnesfel nedan:
...
**Om det inte fungerar:**  
Undersök vilka processer som tar upp mest av ditt systems RAM. Hos ArjunaCRM märkte vi att några VScode-tillägg tog mycket RAM, så vi inaktiverade dem tillfälligt.
...
- Öppna din ArjunaCRM-app, navigera till `/settings`, och aktivera **Avancerat**-växeln längst ner till vänster på skärmen.
- Skapa en ny webhook.
- Klistra in **Din unika Webhook-URL** i fältet **Endpoint URL** i ArjunaCRM. Ställ in **Filter** till `Companies` och `Created`.
<div style={{textAlign: 'center'}}>
...
Kommentera kontrollplugin i `packages/arjuna-ui/vite-config.ts` som i exemplet nedan
...
Om du kör ArjunaCRM bakom en omvänd proxy och upplever anslutningsproblem:
...
2. **Kontrollera omvända proxyinställningar:**
   ...
   - Bekräfta att din omvända proxy vidarebefordrar förfrågningar korrekt till ArjunaCRM-servern.
   - Säkerställ att rubriker som `X-Forwarded-For` och `X-Forwarded-Proto` är korrekt inställda.

3. **Starta om tjänster:**

   Efter att ha gjort förändringar, starta om både den omvända proxyn och ArjunaCRM-behållarna.
...
- Communitysupport:
  ...
  Kontakta [ArjunaCRM-gemenskapen](https://github.com/vedpragyabharat/arjuna/issues) eller [supportkanaler](https://discord.gg/cx5n4Jzs57) för hjälp.