...
1. I en terminal, på värden där ArjunaCRM körs, stäng av ArjunaCRM: `docker compose down`
...
3. Sätt igång ArjunaCRM igen med `docker compose up -d`
...
Om du stöter på några problem vid körning efter uppgradering kan du behöva rensa cachen för att säkerställa att den är synkroniserad med de senaste ändringarna. Kör detta kommando i din arjuna-server-container:
...
Uppgradera din ArjunaCRM-instans till att använda v0.55-bilden
...
I din `arjuna-server`-container, kör:

```bash
yarn command:prod cache:flush
```

Detta problem är specifikt för denna ArjunaCRM-version och bör inte behövas för framtida uppgraderingar.
...
Det bör alltid ligga inom intervallet för din nuvarande ArjunaCRM-instans `major.minor` version, du kan se din instansversion i administratörspanelen (på `/settings/admin-panel`, åtkomligt om din användare har `canAccessFullAdminPanel` egenskap satt till true i databasen) eller genom att köra `echo $APP_VERSION` i din `arjuna-server`-container.

För att åtgärda en osynkroniserad arbetsytaversion måste du uppgradera från motsvarande ArjunaCRMs version genom att följa den relaterade uppgraderingsguiden sekventiellt och så vidare tills önskad version uppnås.
...
Uppgradera din ArjunaCRM-instans till att använda v0.52-bilden
...
Du måste manuellt uppdatera din arbetsytaversion till `0.51.0` i databasen och uppgradera med arjuna version `0.52.11` genom att följa den just ovanstående uppgraderingsguiden.

### v0.50 till v0.51

Uppgradera din ArjunaCRM-instans till att använda v0.51-bilden
...
Uppgradera din ArjunaCRM-instans till att använda v0.50.0-bilden
...
Vänligen uppdatera din lokala `docker-compose.yml` med [v0.50.0 docker-compose.yml](https://github.com/vedpragyabharat/arjuna/blob/v0.50.0/packages/arjuna-docker/docker-compose.yml)

### v0.43.0 till v0.44.0

Uppgradera din ArjunaCRM-instans till att använda v0.44.0-bilden
...
Uppgradera din ArjunaCRM-instans till att använda v0.43.0-bilden
...
2. Uppgradera din docker-compose.yml för att använda postgres:16 bilden som i [docker-compose.yml](https://raw.githubusercontent.com/vedpragyabharat/arjuna/main/packages/arjuna-docker/docker-compose.yml) filen.
...
Uppgradera din ArjunaCRM-instans till att använda v0.42.0-bilden
...
Uppgradera din ArjunaCRM-instans till att använda v0.41.0-bilden
...
Uppgradera din ArjunaCRM-instans till att använda v0.40.0-bilden
...
Uppgradera din ArjunaCRM-instans till att använda v0.35.0-bilden
...
Uppgradera din ArjunaCRM-instans till att använda v0.34.0-bilden
...
Uppgradera din ArjunaCRM-instans till att använda v0.33.0-bilden
...
Uppgradera din ArjunaCRM-instans till att använda v0.32.0-bilden
...
Uppgradera din ArjunaCRM-instans till att använda v0.31.0-bilden
...
Uppgradera din ArjunaCRM-instans till att använda v0.30.0-bilden

**Breaking change**:
To enhance performances, ArjunaCRM now requires redis cache to be configured. Vi har uppdaterat vår [docker-compose.yml](https://raw.githubusercontent.com/vedpragyabharat/arjuna/main/packages/arjuna-docker/docker-compose.yml) för att återspegla detta.
...
Uppgradera din ArjunaCRM-instans till att använda v0.24.0-bilden
...
Uppgradera din ArjunaCRM-instans till att använda v0.23.0-bilden
...
Uppgradera din ArjunaCRM-instans till att använda v0.22.0-bilden
