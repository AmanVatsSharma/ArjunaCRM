...
Dessa kommandon ska köras från mappen packages/arjuna-server.
...
Notera: du kan köra `npx nx run arjuna-server:test:integration:with-db-reset` om du behöver återställa databasen innan du kör integrationstesterna.
...
ArjunaCRM använder främst NestJS för backend.
