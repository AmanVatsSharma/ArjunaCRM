...
1. W terminalu, na hoście, na którym działa ArjunaCRM, wyłącz ArjunaCRM: `docker compose down`
...
3. Przywróć pracę ArjunaCRM w trybie online za pomocą `docker compose up -d`
...
Jeśli napotkasz jakiekolwiek problemy z działaniem po aktualizacji, może być konieczne opróżnienie pamięci podręcznej, aby upewnić się, że jest zsynchronizowana z najnowszymi zmianami. Uruchom to polecenie w swoim kontenerze arjuna-server:
...
Zaktualizuj instancję ArjunaCRM, aby używać obrazu v0.55
...
W swoim kontenerze `arjuna-server` uruchom:

```bash
yarn command:prod cache:flush
```

Ten problem jest specyficzny dla tej wersji ArjunaCRM i nie powinien być wymagany w przyszłych aktualizacjach.
...
Powinna zawsze znajdować się w zakresie bieżącej wersji `major.minor` instancji ArjunaCRM. Możesz zobaczyć swoją wersję instancji na panelu administracyjnym (pod `/settings/admin-panel`, dostępnym, jeśli twój użytkownik ma ustawioną właściwość `canAccessFullAdminPanel` na true w bazie danych) lub uruchamiając `echo $APP_VERSION` w swoim kontenerze `arjuna-server`.

To fix a desynchronized workspace version, you will have to upgrade from the corresponding arjuna's version following related upgrade guide sequentially and so on until it reaches desired version.
...
Zaktualizuj instancję ArjunaCRM, aby używać obrazu v0.52
...
Zaktualizuj instancję ArjunaCRM, aby używać obrazu v0.51
...
Zaktualizuj instancję ArjunaCRM, aby używać obrazu v0.50.0
...
Zaktualizuj lokalny plik `docker-compose.yml` przy użyciu [v0.50.0 docker-compose.yml](https://github.com/vedpragyabharat/arjuna/blob/v0.50.0/packages/arjuna-docker/docker-compose.yml)

### v0.43.0 do v0.44.0

Zaktualizuj instancję ArjunaCRM, aby używać obrazu v0.44.0
...
Zaktualizuj instancję ArjunaCRM, aby używać obrazu v0.43.0
...
2. Zaktualizuj swój docker-compose.yml, aby używać obrazu postgres:16, zgodnie z plikiem [docker-compose.yml](https://raw.githubusercontent.com/vedpragyabharat/arjuna/main/packages/arjuna-docker/docker-compose.yml).
...
Zaktualizuj instancję ArjunaCRM, aby używać obrazu v0.42.0
...
Zaktualizuj instancję ArjunaCRM, aby używać obrazu v0.41.0
...
Zaktualizuj instancję ArjunaCRM, aby używać obrazu v0.40.0
...
Zaktualizuj instancję ArjunaCRM, aby używać obrazu v0.35.0
...
Zaktualizuj instancję ArjunaCRM, aby używać obrazu v0.34.0
...
Zaktualizuj instancję ArjunaCRM, aby używać obrazu v0.33.0
...
Począwszy od tej wersji, obraz arjuna-postgres dla DB stał się przestarzały, a zamiast niego używany jest arjuna-postgres-spilo.
Jeśli chcesz nadal używać obrazu arjuna-postgres, po prostu zamień `arjunacrm/arjuna-postgres:${TAG}` na `arjunacrm/arjuna-postgres` w docker-compose.yml.

### v0.31.0 do v0.32.0

Zaktualizuj instancję ArjunaCRM, aby używać obrazu v0.32.0
...
Zaktualizuj instancję ArjunaCRM, aby używać obrazu v0.31.0
...
Zaktualizuj instancję ArjunaCRM, aby używać obrazu v0.30.0

**Breaking change**:
To enhance performances, ArjunaCRM now requires redis cache to be configured. Zaktualizowaliśmy nasz [docker-compose.yml](https://raw.githubusercontent.com/vedpragyabharat/arjuna/main/packages/arjuna-docker/docker-compose.yml), aby to odzwierciedlać.
...
Zaktualizuj instancję ArjunaCRM, aby używać obrazu v0.24.0
...
Zaktualizuj instancję ArjunaCRM, aby używać obrazu v0.23.0
...
Zaktualizuj instancję ArjunaCRM, aby używać obrazu v0.22.0
