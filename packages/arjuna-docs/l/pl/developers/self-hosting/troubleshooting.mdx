...
Jeśli masz istniejącą instancję ArjunaCRM z danymi produkcyjnymi, **NIE** postępuj zgodnie z tymi krokami, ponieważ spowodują one trwałe usunięcie bazy danych!

Podczas pierwszej instalacji ArjunaCRM, możesz chcieć zmienić domyślne hasło do bazy danych.
...
Podczas instalacji ArjunaCRM musisz zaopatrzyć swoją bazę danych postgres w odpowiednie schematy, rozszerzenia i użytkowników.
...
#### Nie można znaleźć modułu 'arjuna-emails' lub jego odpowiednich deklaracji typów.

Musisz zbudować pakiet `arjuna-emails` przed uruchomieniem inicjalizacji bazy danych za pomocą `npx nx run arjuna-emails:build`

#### Brak pakietu arjuna-x

Upewnij się, że uruchomiony jest yarn w katalogu głównym, a następnie uruchom `npx nx server:dev arjuna-server`. Jeśli to nadal nie działa, spróbuj ręcznie zbudować brakujący pakiet.
...
W `packages/arjuna-front/.env` skomentuj `VITE_DISABLE_TYPESCRIPT_CHECKER=true` i `VITE_DISABLE_ESLINT_CHECKER=true`, aby wyłączyć sprawdzanie w tle, co zmniejsza ilość potrzebnego RAM-u.

**If it does not work:**  
Run only the services you need, instead of `npx nx start`. Na przykład, jeśli pracujesz nad serwerem, uruchom tylko `npx nx worker arjuna-server`

**Jeśli to nie działa:**  
Jeśli próbowałeś uruchomić tylko `npx nx run arjuna-server:start` na WSL i kończy się to błędem pamięci przedstawionym poniżej:
...
**Jeśli to nie działa:**  
Zbadaj, które procesy zużywają większość pamięci RAM twojego komputera. W ArjunaCRM zauważyliśmy, że niektóre rozszerzenia VScode zużywają dużo RAM, więc tymczasowo je wyłączamy.
...
- Otwórz aplikację ArjunaCRM, przejdź do `/settings`, i włącz przełącznik **Zaawansowane** w dolnym lewym rogu ekranu.
- Create a new webhook.
- Wklej **Your Unique Webhook URL** w polu **Endpoint Url** w ArjunaCRM. Ustaw **Filtry** na `Companies` i `Created`.
<div style={{textAlign: 'center'}}>
...
Skomentuj wtyczkę checker w `packages/arjuna-ui/vite-config.ts` jak w poniższym przykładzie
...
Jeśli uruchamiasz ArjunaCRM za odwrotnym proxy i występują problemy z połączeniem:
...
2. **Sprawdź ustawienia odwrotnego proxy:**
   ...
   - Upewnij się, że twoje odwrotne proxy prawidłowo przekazuje żądania do serwera ArjunaCRM.
   - Upewnij się, że nagłówki takie jak `X-Forwarded-For` i `X-Forwarded-Proto` są prawidłowo ustawione.

3. **Uruchom ponownie usługi:**

   Po dokonaniu zmian uruchom ponownie zarówno odwrotne proxy, jak i kontenery ArjunaCRM.
...
- Wsparcie społeczności:
  ...
  Skontaktuj się z [społecznością ArjunaCRM](https://github.com/vedpragyabharat/arjuna/issues) lub [kanałami wsparcia](https://discord.gg/cx5n4Jzs57) po pomoc.