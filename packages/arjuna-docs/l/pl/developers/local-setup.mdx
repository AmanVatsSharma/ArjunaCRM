...
description: "Przewodnik dla współautorów (lub ciekawych deweloperów), którzy chcą uruchomić ArjunaCRM lokalnie."
...
Zanim zainstalujesz i użyjesz ArjunaCRM, upewnij się, że na Twoim komputerze są zainstalowane następujące programy:
...
ArjunaCRM wymaga pamięci cache redis, aby zapewnić najlepszą wydajność
...
Użyj zmiennych środowiskowych lub plików `.env`, aby skonfigurować swój projekt. Więcej informacji [tutaj](https://docs.vedpragya.com/l/pl/developers/self-hosting/setup)
...
Aby zbudować serwer ArjunaCRM i wprowadzić pewne dane do swojej bazy danych, uruchom następujące polecenie:
...
## Krok 8: Użyj ArjunaCRM

**Frontend**

Frontend ArjunaCRM będzie działał pod adresem [http://localhost:3001](http://localhost:3001).
Możesz zalogować się, używając domyślnego konta demo: `tim@apple.dev` (hasło: `tim@apple.dev`)

**Backend**

- Serwer ArjunaCRM będzie działać pod adresem [http://localhost:3000](http://localhost:3000)
