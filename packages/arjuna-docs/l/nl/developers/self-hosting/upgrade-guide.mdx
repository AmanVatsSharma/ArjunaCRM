...
1. Zet in een terminal, op de host waar ArjunaCRM draait, ArjunaCRM uit: `docker compose down`
...
3. Breng ArjunaCRM weer online met `docker compose up -d`
...
**Zorg ervoor dat u na elke geüpgradede versie een niet-beschadigde back-up hebt.**
...
Als u runtime-problemen ondervindt na het upgraden, moet u mogelijk uw cache leegmaken om ervoor te zorgen dat deze is gesynchroniseerd met de laatste wijzigingen. Voer deze opdracht uit in uw arjuna-server container:
...
Upgrade je ArjunaCRM-instantie om v0.55 afbeelding te gebruiken
...
In je `arjuna-server`-container, voer uit:

```bash
yarn command:prod cache:flush
```

Dit probleem is specifiek voor deze ArjunaCRM-versie en zou niet nodig moeten zijn voor toekomstige upgrades.
...
Het moet altijd binnen het bereik van uw huidige ArjunaCRM-instantie `major.minor` versie zijn, u kunt uw instantieversie bekijken in het beheerderspaneel (op `/settings/admin-panel`, toegankelijk als uw gebruiker `canAccessFullAdminPanel` eigenschap heeft ingesteld op true in de database) of door `echo $APP_VERSION` uit te voeren in uw `arjuna-server` container.
...
Upgrade je ArjunaCRM-instantie om v0.52 afbeelding te gebruiken
...
Upgrade je ArjunaCRM-instantie om v0.51 afbeelding te gebruiken
...
Upgrade je ArjunaCRM-instantie naar v0.50.0 afbeelding
...
Werk uw lokale `docker-compose.yml` bij met [v0.50.0 docker-compose.yml](https://github.com/vedpragyabharat/arjuna/blob/v0.50.0/packages/arjuna-docker/docker-compose.yml)

### v0.43.0 naar v0.44.0

Upgrade je ArjunaCRM-instantie om v0.44.0 afbeelding te gebruiken
...
Upgrade je ArjunaCRM-instantie om v0.43.0 afbeelding te gebruiken
...
2. Upgrade uw docker-compose.yml om de postgres:16 afbeelding te gebruiken zoals in het [docker-compose.yml](https://raw.githubusercontent.com/vedpragyabharat/arjuna/main/packages/arjuna-docker/docker-compose.yml) bestand.
...
Upgrade je ArjunaCRM-instantie om v0.42.0 afbeelding te gebruiken
...
Upgrade je ArjunaCRM-instantie om v0.41.0 afbeelding te gebruiken
...
Upgrade je ArjunaCRM-instantie om v0.40.0 afbeelding te gebruiken
...
Upgrade je ArjunaCRM-instantie om v0.35.0 afbeelding te gebruiken
...
Upgrade je ArjunaCRM-instantie naar v0.34.0 afbeelding
...
Upgrade je ArjunaCRM-instantie om v0.33.0 afbeelding te gebruiken
...
Vanaf deze versie is de arjuna-postgres afbeelding voor DB verouderd en wordt er in plaats daarvan gebruikgemaakt van arjuna-postgres-spilo.
Als je de arjuna-postgres afbeelding wilt blijven gebruiken, vervang dan eenvoudig `arjunacrm/arjuna-postgres:${TAG}` door `arjunacrm/arjuna-postgres` in docker-compose.yml.

### v0.31.0 naar v0.32.0

Upgrade je ArjunaCRM-instantie naar v0.32.0 afbeelding
...
Upgrade je ArjunaCRM-instantie om v0.31.0 afbeelding te gebruiken
...
Upgrade je ArjunaCRM-instantie om v0.30.0 afbeelding te gebruiken

**Breaking change**:
To enhance performances, ArjunaCRM now requires redis cache to be configured. We hebben onze [docker-compose.yml](https://raw.githubusercontent.com/vedpragyabharat/arjuna/main/packages/arjuna-docker/docker-compose.yml) bijgewerkt om dit te weerspiegelen.
...
Upgrade je ArjunaCRM-instantie om v0.24.0 afbeelding te gebruiken
...
Upgrade je ArjunaCRM-instantie om v0.23.0 afbeelding te gebruiken
...
Upgrade je ArjunaCRM-instantie om v0.22.0 afbeelding te gebruiken
