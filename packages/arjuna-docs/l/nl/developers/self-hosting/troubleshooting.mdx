...
Als u een bestaande ArjunaCRM-instantie heeft met productiedata, gebruik deze stappen NIET, omdat deze permanent uw database verwijderen!

Tijdens de eerste installatie van ArjunaCRM wilt u misschien het standaarddatabasewachtwoord wijzigen.
...
Tijdens de ArjunaCRM-installatie moet u uw Postgres-database voorzien van de juiste schema's, extensies en gebruikers.
...
#### Kan module 'arjuna-emails' of de bijbehorende typeverklaringen niet vinden.

U moet het pakket `arjuna-emails` bouwen voordat u de database-initialisatie uitvoert met `npx nx run arjuna-emails:build`

#### Missing arjuna-x package

Zorg ervoor dat u in de hoofdmap `yarn` uitvoert en daarna `npx nx server:dev arjuna-server`. Als het nog steeds niet werkt, probeer dan het ontbrekende pakket handmatig te bouwen.
...
#### Tijdens uitvoeren van `npx nx start` of `npx nx start arjuna-front`, wordt er een geheugenfout gegenereerd

In `packages/arjuna-front/.env` uncomment `VITE_DISABLE_TYPESCRIPT_CHECKER=true` en `VITE_DISABLE_ESLINT_CHECKER=true` om achtergrondcontroles te stoppen en zo het benodigde RAM-gebruik te verminderen.

**Als dit niet werkt:**  
Voer alleen de services uit die u nodig heeft, in plaats van `npx nx start`. Bijvoorbeeld, als je aan de server werkt, voer alleen `npx nx worker arjuna-server` uit.

**Als dit niet werkt:**  
Als je geprobeerd hebt om alleen `npx nx run arjuna-server:start` op WSL uit te voeren en het niet lukt met de hieronder beschreven geheugenfout:
...
**Als dit niet werkt:**  
Onderzoek welke processen het meeste RAM van uw machine gebruiken. Bij ArjunaCRM merkten we dat sommige VScode-extensies veel RAM innamen, dus we schakelden ze tijdelijk uit.
...
- Open uw ArjunaCRM-app, ga naar `/settings` en schakel de **Geavanceerd** schakelaar in aan de onderkant links van het scherm.
- Maak een nieuwe webhook.
- Plak **Uw Unieke Webhook-URL** in het **Endpoint-URL** veld in ArjunaCRM. Stel de **Filters** in op `Bedrijven` en `Aangemaakt`.
<div style={{textAlign: 'center'}}>
...
Comment out checker plugin in `packages/arjuna-ui/vite-config.ts` like in example below
...
Als u ArjunaCRM draait achter een reverse proxy en verbindingsproblemen ondervindt:
...
2. **Controleer de instellingen van de reverse proxy:**
   ...
   - Bevestig dat uw reverse proxy verzoeken correct doorstuurt naar de ArjunaCRM-server.
   - Zorg ervoor dat headers zoals `X-Forwarded-For` en `X-Forwarded-Proto` correct zijn ingesteld.

3. **Herstart services:**

   Na het aanbrengen van wijzigingen, start zowel de reverse proxy als de ArjunaCRM-containers opnieuw.
...
- Ondersteuning vanuit de community:
  ...
  Neem contact op met de [ArjunaCRM-gemeenschap](https://github.com/vedpragyabharat/arjuna/issues) of [ondersteuningskanalen](https://discord.gg/cx5n4Jzs57) voor hulp.