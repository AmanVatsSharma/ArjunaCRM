...
description: "De gids voor bijdragers (of nieuwsgierige ontwikkelaars) die ArjunaCRM lokaal willen uitvoeren."
...
Voordat je ArjunaCRM kunt installeren en gebruiken, zorg ervoor dat je het volgende op je computer installeert:
...
ArjunaCRM vereist een redis-cache voor de beste prestaties.
...
Gebruik omgevingsvariabelen of `.env`-bestanden om je project te configureren. Meer info [hier](https://docs.vedpragya.com/l/nl/developers/self-hosting/setup).
...
Om de ArjunaCRM-server te bouwen en enkele gegevens in je database te vullen, voer het volgende commando uit:
...
## Stap 8: Gebruik ArjunaCRM

**Frontend**

De frontend van ArjunaCRM draait op [http://localhost:3001](http://localhost:3001).
Je kunt inloggen met het standaard demo-account: `tim@apple.dev` (wachtwoord: `tim@apple.dev`)

**Backend**

- De server van ArjunaCRM draait op [http://localhost:3000](http://localhost:3000).
