...
Deze opdrachten moeten vanuit de map packages/arjuna-server worden uitgevoerd.
...
Opmerking: je kunt `npx nx run arjuna-server:test:integration:with-db-reset` uitvoeren als je de database moet resetten voordat de integratietests worden uitgevoerd.
...
ArjunaCRM gebruikt voornamelijk NestJS voor de backend.
