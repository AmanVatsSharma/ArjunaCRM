---
title: Yerel Kurulum
description: "ArjunaCRM'i yerel olarak çalıştırmak isteyen katkıda bulunanlar (veya meraklı geliştiriciler) için kılavuz."
...
ArjunaCRM'i yüklemeden ve kullanmadan önce bilgisayarınıza aşağıdakileri yüklediğinizden emin olun:
...
ArjunaCRM, en iyi performansı sağlamak için bir redis önbelleğe ihtiyaç duyar
...
Projenizi yapılandırmak için çevresel değişkenler veya `.env` dosyaları kullanın. Daha fazla bilgi [burada](https://docs.vedpragya.com/l/tr/developers/self-hosting/setup)
...
ArjunaCRM server'ı oluşturup veritabanınıza bazı veriler yerleştirmek için aşağıdaki komutu çalıştırın:
...
## Step 8: Use ArjunaCRM

**Ön Uç**

ArjunaCRM's frontend will be running at [http://localhost:3001](http://localhost:3001).
Varsayılan demo hesabıyla giriş yapabilirsiniz: `tim@apple.dev` (şifre: `tim@apple.dev`)

**Arka Uç**

- ArjunaCRM's server will be up and running at [http://localhost:3000](http://localhost:3000)
