...
Mevcut bir ArjunaCRM örneğiniz varsa ve üretim verileri varsa, bu adımları **TAKİP ETMEYİN**, çünkü bu veritabanınızı kalıcı olarak silecektir!

ArjunaCRM'yi ilk kez kurarken, varsayılan veritabanı parolasını değiştirmek isteyebilirsiniz.
...
ArjunaCRM kurulumu sırasında, postgres veritabanınızı doğru şemalar, uzantılar ve kullanıcılarla hazırlamanız gerekir.
...
#### Modül 'arjuna-emails' veya buna karşılık gelen tür açıklamaları bulunamıyor.

`arjuna-emails` paketini veritabanını `npx nx run arjuna-emails:build` ile başlatmadan önce derlemeniz gerekiyor.

#### Eksik arjuna-x paketi

Kök dizinde yarn çalıştırdığınızdan ve ardından `npx nx server:dev arjuna-server` çalıştırdığınızdan emin olun. Bu hala çalışmıyorsa eksik paketi manuel olarak derlemeyi deneyin.
...
#### `npx nx start` veya `npx nx start arjuna-front` çalıştırırken, bellek hatası verildi

`packages/arjuna-front/.env` içinde `VITE_DISABLE_TYPESCRIPT_CHECKER=true` ve `VITE_DISABLE_ESLINT_CHECKER=true`'i açarak arka plan kontrollerini devre dışı bırakın, böylece gerekli RAM miktarını azaltın.

**Bu işe yaramazsa:**
Yalnızca ihtiyaç duyduğunuz hizmetleri çalıştırın, `npx nx start` yerine. Örneğin, sunucuda çalışıyorsanız yalnızca `npx nx worker arjuna-server` çalıştırın.

**Bu işe yaramazsa:**
WSL'de yalnızca `npx nx run arjuna-server:start` çalıştırmayı denediyseniz ve aşağıdaki bellek hatasıyla başarısız olduysanız:
...
**Bu işe yaramazsa:**
Hangi işlemlerin makinenizin RAM'inin çoğunu kullandığını araştırın. ArjunaCRM'de bazı VScode uzantılarının çok fazla RAM aldığını fark ettik, bu yüzden onları geçici olarak devre dışı bırakıyoruz.
...
- ArjunaCRM uygulamanızı açın, `/settings` sayfasına gidin ve ekranın sol alt köşesinde bulunan **Gelişmiş** seçeneğini etkinleştirin.
- Yeni bir webhook oluşturun.
- Paste **Your Unique Webhook URL** in the **Endpoint Url** field in ArjunaCRM. **Filtreleri** `Şirketler` ve `Oluşturuldu` olarak ayarlayın.
<div style={{textAlign: 'center'}}>
...
Eğer ArjunaCRM'yi ters proxy'nin arkasında çalıştırıyorsanız ve bağlantı sorunları yaşıyorsanız:
...
2. **Ters Proxy Ayarlarını Kontrol Edin:**
   ...
   - Ters proxy'nizin ArjunaCRM sunucusuna talepleri düzgün bir şekilde yönlendirdiğinden emin olun.
...
3. **Servisleri Yeniden Başlatın:**
   ...
   Değişiklik yaptıktan sonra hem ters proxy'yi hem de ArjunaCRM konteynerlerini yeniden başlatın.
...
- Topluluk Desteği:
  ...
  [ArjunaCRM topluluğuna](https://github.com/vedpragyabharat/arjuna/issues) veya [destek kanallarına](https://discord.gg/cx5n4Jzs57) yardım için ulaşın.