...
1. Yirmi'nin çalıştığı sunucuda bir terminalde, ArjunaCRM'yi kapatın: `docker compose down`

2. Sürümü yükseltmek için .env dosyanızdaki `TAG` değerini değiştirin. ( `v0.53` gibi `major.minor` sürümler tüketmenizi öneririz )

3. ArjunaCRM'yi `docker compose up -d` ile tekrar çevrimiçi hale getirin.
...
Yükseltme sonrasında herhangi bir çalıştırma sorunu yaşarsanız, önbelleğinizi temizlemeniz gerekebilir, böylece en son değişikliklerle senkronize olmuş olur. arjuna-server konteynerınızda bu komutu çalıştırın:
...
ArjunaCRM örneğinizi v0.55 görüntüsünü kullanacak şekilde yükseltin
...
`arjuna-server` konteynerinizde şu komutu çalıştırın:

```bash
yarn command:prod cache:flush
```

Bu sorun, bu ArjunaCRM sürümüne özgüdür ve gelecekteki yükseltmeler için gerekli olmamalıdır.
...
Her zaman, mevcut ArjunaCRM örneğinizin `major.minor` sürüm aralığında olması gerekir, örnek sürümünüzü yönetici panelinde (veritabanında `canAccessFullAdminPanel` özelliği true olarak ayarlandığında `settings/admin-panel` altında bulunabilir) veya `arjuna-server` konteynerinizde `echo $APP_VERSION` çalıştırarak görüntüleyebilirsiniz.
...
ArjunaCRM sürümünüzü v0.52 görüntüsünü kullanacak şekilde yükseltin
...
Veritabanında çalışma alanı sürümünüzü manuel olarak `0.51.0` olarak güncelleyip, yukarıdaki yükseltme kılavuzunu takip ederek ArjunaCRM sürümü `0.52.11` ile yükseltmeniz gerekecek.

### v0.50'den v0.51'e

ArjunaCRM sürümünüzü v0.51 görüntüsünü kullanacak şekilde yükseltin
...
ArjunaCRM sürümünüzü v0.50.0 görüntüsünü kullanacak şekilde yükseltin
...
Yerel `docker-compose.yml` dosyanızı [v0.50.0 docker-compose.yml](https://github.com/vedpragyabharat/arjuna/blob/v0.50.0/packages/arjuna-docker/docker-compose.yml) ile güncelleyin

### v0.43.0'dan v0.44.0'a

ArjunaCRM sürümünüzü v0.44.0 görüntüsünü kullanacak şekilde yükseltin
...
ArjunaCRM sürümünüzü v0.43.0 görüntüsünü kullanacak şekilde yükseltin
...
2. docker-compose.yml dosyanızı postgres:16 görüntüsü ile kullanacak şekilde yükseltin [docker-compose.yml](https://raw.githubusercontent.com/vedpragyabharat/arjuna/main/packages/arjuna-docker/docker-compose.yml) dosyasında belirtildiği gibi güncelleyin.
...
ArjunaCRM sürümünüzü v0.42.0 görüntüsünü kullanacak şekilde yükseltin
...
ArjunaCRM sürümünüzü v0.41.0 görüntüsünü kullanacak şekilde yükseltin
...
ArjunaCRM sürümünüzü v0.40.0 görüntüsünü kullanacak şekilde yükseltin
...
ArjunaCRM sürümünüzü v0.35.0 görüntüsünü kullanacak şekilde yükseltin
...
ArjunaCRM örneğinizi v0.34.0 görüntüsünü kullanacak şekilde yükseltin
...
ArjunaCRM örneğinizi v0.33.0 görüntüsünü kullanacak şekilde yükseltin
...
Bu sürümden itibaren, DB için arjuna-postgres görüntüsü kullanımdan kalktı ve yerine arjuna-postgres-spilo kullanılıyor.
ArjunaCRM-postgres görüntüsünü kullanmaya devam etmek istiyorsanız, docker-compose.yml dosyasında `arjunacrm/arjuna-postgres:${TAG}`i `arjunacrm/arjuna-postgres` ile değiştirin.

### v0.31.0'dan v0.32.0'ya

ArjunaCRM örneğinizi v0.32.0 görüntüsünü kullanacak şekilde yükseltin
...
ArjunaCRM örneğinizi v0.31.0 görüntüsünü kullanacak şekilde yükseltin
...
ArjunaCRM sürümünüzü v0.30.0 görüntüsünü kullanacak şekilde yükseltin

**Breaking change**:
To enhance performances, ArjunaCRM now requires redis cache to be configured. [docker-compose.yml](https://raw.githubusercontent.com/vedpragyabharat/arjuna/main/packages/arjuna-docker/docker-compose.yml) dosyamızı bu durumu yansıtacak şekilde güncelledik.
...
ArjunaCRM sürümünüzü v0.24.0 görüntüsünü kullanacak şekilde yükseltin
...
ArjunaCRM sürümünüzü v0.23.0 görüntüsünü kullanacak şekilde yükseltin
...
ArjunaCRM sürümünüzü v0.22.0 görüntüsünü kullanacak şekilde yükseltin
