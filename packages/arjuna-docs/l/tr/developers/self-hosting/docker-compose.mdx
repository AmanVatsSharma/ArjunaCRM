...
<Warning>
Docker containers are for production hosting or self-hosting, for the contribution please check the [Local Setup](https://docs.vedpragya.com/l/tr/developers/local-setup).
</Warning>
...
See docs [Setup Environment Variables](https://docs.vedpragya.com/l/tr/developers/self-hosting/setup) for advanced configuration. Tüm ortam değişkenleri, sunucu ve / veya işçi düzeyine bağlı olarak docker-compose.yml dosyasında ilan edilmelidir.
...
Tek bir komutla en son kararlı ArjunaCRM sürümünü yükleyin:
...
### ArjunaCRM'yi Dış Erişime Açma

Varsayılan olarak, ArjunaCRM `localhost` üzerinde `3000` portunda çalışır. Harici bir alan adı veya IP adresi aracılığıyla erişim sağlamak için `.env` dosyanızda `SERVER_URL`'i yapılandırmanız gerekir.
...
Belirli tarayıcı özelliklerinin düzgün çalışabilmesi için SSL (HTTPS) gereklidir. Bu özellikler yerel geliştirme sırasında çalışabilirken (tarayıcılar localhost'u farklı şekilde ele alır), ArjunaCRM'yi düzenli bir alanda barındırırken uygun bir SSL kurulumu gereklidir.
...
Optimum güvenlik ve işlevsellik için ArjunaCRM'nin SSL sonlandırma ile bir ters proxy arkasında ayarlanmasını şiddetle öneririz.
...
Herhangi bir sorunla karşılaşırsanız, çözümler için [Sorun Giderme](https://docs.vedpragya.com/l/tr/developers/self-hosting/troubleshooting) sayfasına bakın.
