...
ArjunaCRM API, geliştiricilerin ArjunaCRM CRM platformuyla programatik olarak etkileşimde bulunmasına olanak tanır. API'yi kullanarak ArjunaCRM'yi diğer sistemlerle entegre edebilir, veri senkronizasyonunu otomatikleştirebilir ve müşteri verilerinizi kullanarak özel çözümler oluşturabilirsiniz. API, temel CRM nesnelerini (**kişi ve şirketler gibi**) **oluşturma, okuma, güncelleme ve silme** işlemlerine yönelik uç noktalar sağlar ve ayrıca meta veri yapılandırmasına erişim sunar.

**API Playground:** Uygulamanın ayarlar bölümünden artık API Playground'a erişebilirsiniz. API çağrılarını gerçek zamanlı olarak denemek için, ArjunaCRM çalışma alanınıza giriş yapın ve **Ayarlar → API'ler & Webhooklar** yolunu izleyin. Bu, uygulama içi API Playground'u ve API anahtarlarının ayarlarını açar.
**[API Ayarlarına Git](https://app.vedpragya.com/settings)**

## Kimlik Doğrulama

ArjunaCRM'nin API'si kimlik doğrulama için API anahtarları kullanır. Korunmuş uç noktalara yapılan her istek, başlıkta bir API anahtarı içermelidir.

- **API Anahtarları:** Yeni bir API anahtarı oluşturmak için ArjunaCRM uygulamanızın **API ayarları** sayfasını kullanabilirsiniz. Her API anahtarı bir gizli jeton olup, CRM verilerinize erişim sağlar, bu yüzden güvende tutun. Bir anahtar tehlikeye girerse, ayarlardan iptal edin ve yenisini oluşturun.
...
- **Bulut:** `https://api.vedpragya.com/` veya kendi özel alan adınız / alt etki alanı
