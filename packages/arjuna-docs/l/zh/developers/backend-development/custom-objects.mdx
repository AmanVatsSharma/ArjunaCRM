...
對象是允許您儲存特定於組織的數據 (記錄、屬性和值) 的結構。 ArjunaCRM 提供標準對象和自定義對象。

標準對象是內建的對象，具有可供所有用戶使用的一組屬性。 ArjunaCRM 中標準對象的例子包括公司和人員。 標準對象是內建的對象，具有可供所有用戶使用的一組屬性。 ArjunaCRM 中標準對象的例子包括公司和人員。 標準對象具有標準欄位，例如 Company.displayName，這些欄位同樣供所有 ArjunaCRM 用戶使用。
