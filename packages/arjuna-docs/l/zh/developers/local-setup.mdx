---
title: 本地設定
description: "供貢獻者（或好奇的開發者）在本地運行 ArjunaCRM 的指南。"
...
在您可以安裝和使用 ArjunaCRM 之前，請確保您的電腦上安裝了以下內容：
...
ArjunaCRM 需要一個 redis 緩存來提供最佳性能
...
使用環境變數或 `.env` 文件配置您的項目。 使用環境變數或 `.env` 文件配置您的項目。 查看更多信息 [這裡](https://docs.vedpragya.com/l/zh/developers/self-hosting/setup)
...
要構建 ArjunaCRM 服務器並向您的數據庫種植一些數據，請運行以下命令：
...
ArjunaCRM 的前端將在 [http://localhost:3001](http://localhost:3001) 運行。
您可以使用默認的演示帳戶登錄：`tim@apple.dev`（密碼：`tim@apple.dev`）
您可以使用默認的演示帳戶登錄：`tim@apple.dev`（密碼：`tim@apple.dev`）

**後端**

- ArjunaCRM 的服務器將在 [http://localhost:3000](http://localhost:3000) 啟動並運行
