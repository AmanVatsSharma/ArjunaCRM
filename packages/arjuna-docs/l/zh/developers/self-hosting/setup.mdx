...
<Warning>
**First time installing?** Follow the [Docker Compose installation guide](https://docs.vedpragya.com/l/zh/developers/self-hosting/docker-compose) to get ArjunaCRM running, then return here for configuration.
</Warning>

ArjunaCRM 提供 **兩種配置模式** 以滿足不同的部署需求：
...
1. 訪問你的 ArjunaCRM 實例（通常是 `http://localhost:3000`）
...
**所需範圍**（自動配置）：
[查看相關源代碼](https://github.com/vedpragyabharat/arjuna/blob/main/packages/arjuna-server/src/engine/core-modules/auth/utils/get-google-apis-oauth-scopes.ts#L4-L10)
...
用戶必須擁有 [Microsoft 365 許可](https://admin.microsoft.com/Adminportal/Home) 才能使用日曆和消息 API。 沒有許可，將無法在 ArjunaCRM 上同步帳戶。
 沒有許可，將無法在 ArjunaCRM 上同步帳戶。
</Warning>
...
[查看相關源代碼](https://github.com/vedpragyabharat/arjuna/blob/main/packages/arjuna-server/src/engine/core-modules/auth/utils/get-microsoft-apis-oauth-scopes.ts#L2-L9)
