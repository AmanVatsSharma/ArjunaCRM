...
🚨 **重要提示：此解決方案僅適用於新安裝** 🚨
如果您已經有一個包含生產數據的ArjunaCRM實例，**請勿**遵循這些步驟，因為它們將永久刪除您的數據庫！

在首次安裝ArjunaCRM時，您可能想要更改默認的數據庫密碼。
...
在ArjunaCRM安裝期間，您需要用正確的架構、擴展和用戶來準備您的postgres數據庫。
如果您成功運行了這個配置，您應該在數據庫中擁有 `default` 和 `metadata` 架構。
如果沒有，請確保電腦上沒有運行多個postgres實例。

#### 無法找到模組 'arjuna-emails' 或其相應的類型聲明。

您需要先構建 `arjuna-emails` 包，然後再用 `npx nx run arjuna-emails:build` 初始化數據庫。

#### 缺少 arjuna-x 包

確保在根目錄運行 yarn，然後運行 `npx nx server:dev arjuna-server`。 如果還是無法運行，請嘗試手動構建缺失的包。 如果還是無法運行，請嘗試手動構建缺失的包。
...
#### 運行 `npx nx start` 或 `npx nx start arjuna-front` 時，內存不足錯誤被拋出

在 `packages/arjuna-front/.env` 中取消註釋 `VITE_DISABLE_TYPESCRIPT_CHECKER=true` 和 `VITE_DISABLE_ESLINT_CHECKER=true` 以禁用後台檢查，從而減少所需的RAM量。

**如果不工作：**  
僅運行您需要的服務，而不是 `npx nx start`。 例如，如果您在服務器上工作，僅運行 `npx nx worker arjuna-server` 例如，如果您在服務器上工作，僅運行 `npx nx worker arjuna-server`

**如果不工作：**  
如果您嘗試僅在 WSL 上運行 `npx nx run arjuna-server:start`，但失敗並顯示以下內存錯誤：
...
**如果不工作：**  
調查哪些進程佔用了您大部分的機器RAM。 在ArjunaCRM，我們注意到一些VScode擴展佔用了很多RAM，因此我們暫時禁用了它們。
...
- Paste **Your Unique Webhook URL** in the **Endpoint Url** field in ArjunaCRM. 將 **Filters** 設置為 `Companies` 和 `Created`。
<div style={{textAlign: 'center'}}>
...
註釋掉 `packages/arjuna-ui/vite-config.ts` 中的檢查器插件，參考下面的示例
...
2. **檢查反向代理設置：**
   ...
   - 確認您的反向代理正確轉發了對ArjunaCRM服務器的請求。
   - 確保標頭如 `X-Forwarded-For` 和 `X-Forwarded-Proto` 已正確設置。

3. **重啟服務：**

   進行更改後，重新啟動反向代理和ArjunaCRM容器。
...
- 社區支持：
  ...
  聯繫 [ArjunaCRM社區](https://github.com/vedpragyabharat/arjuna/issues) 或 [支持渠道](https://discord.gg/cx5n4Jzs57) 獲取幫助。