...
Les Webhooks dans ArjunaCRM complètent l'API en permettant des **notifications en temps réel** à vos propres applications lors de certains événements dans votre CRM. Au lieu de sonder continuellement l'API pour les changements, vous pouvez configurer des webhooks pour que ArjunaCRM **pousse** des données vers votre système chaque fois que des événements spécifiques se produisent (par exemple, lorsqu'un nouvel enregistrement est créé ou qu'un enregistrement existant est mis à jour). Cela aide à synchroniser instantanément et efficacement les systèmes externes avec ArjunaCRM.

Avec les webhooks, ArjunaCRM enverra une requête HTTP POST à une URL que vous spécifiez, contenant des détails sur l'événement. Vous pouvez ensuite traiter ces données dans votre application (par exemple, pour mettre à jour votre base de données externe, déclencher des flux de travail ou envoyer des alertes).

## Configuration d'un Webhook

Pour créer un webhook dans ArjunaCRM, utilisez les paramètres **APIs & Webhooks** dans votre application ArjunaCRM :

1. **Naviguer vers les Paramètres :** Dans votre application ArjunaCRM, allez dans **Paramètres → APIs & Webhooks**.
2. **Créer un Webhook :** Dans **Webhooks**, cliquez sur **+ Créer un webhook**.
3. **Entrez l'URL :** Fournissez l'URL de l'endpoint sur votre serveur où vous souhaitez que ArjunaCRM envoie les requêtes webhook. Cela doit être une URL publiquement accessible qui peut gérer les requêtes POST.
4. **Enregistrer :** Cliquez sur **Enregistrer** pour créer le webhook. Le nouveau webhook sera actif immédiatement.

Vous pouvez créer plusieurs webhooks si vous avez besoin d'envoyer différents événements vers différents endpoints. Chaque webhook est essentiellement un abonnement pour tous les événements pertinents (actuellement, ArjunaCRM envoie tous types d'événements à l'URL donnée ; filtrer des types d'événements spécifiques peut être configurable dans l'interface utilisateur). Si vous devez supprimer un webhook, vous pouvez le supprimer de la même page des paramètres (sélectionnez le webhook et choisissez supprimer).

## Événements et Charges utiles

Une fois qu'un webhook est configuré, ArjunaCRM enverra une requête HTTP POST à l'URL que vous avez spécifiée chaque fois qu'un événement déclencheur se produit dans vos données CRM. Les événements communs qui déclenchent les webhooks incluent :
...
**Remarque :** Il est important de répondre avec un **statut HTTP 2xx** depuis votre endpoint webhook pour accuser réception avec succès. Si l'expéditeur du webhook de ArjunaCRM ne reçoit pas une réponse 2xx, il peut considérer que la livraison a échoué. (À l'avenir, une logique de réessai pourrait tenter de renvoyer les webhooks échoués, donc essayez toujours de retourner un 200 OK aussi rapidement que possible après le traitement des données.)

## Validation des Webhooks

Pour garantir la sécurité de vos endpoints webhook, ArjunaCRM inclut une signature dans l'en-tête `X-ArjunaCRM-Webhook-Signature`.
...
1. Concaténer le timestamp (de l'en-tête `X-ArjunaCRM-Webhook-Timestamp`), un deux-points, et la chaîne JSON de la charge utile
