...
description: "Le guide pour les contributeurs (ou les développeurs curieux) qui souhaitent exécuter ArjunaCRM localement."
...
Avant de pouvoir installer et utiliser ArjunaCRM, assurez-vous d'installer ce qui suit sur votre ordinateur :
...
ArjunaCRM nécessite un cache Redis pour offrir la meilleure performance
...
Utilisez des variables d'environnement ou des fichiers `.env` pour configurer votre projet. Plus d'informations [ici](https://docs.vedpragya.com/l/fr/developers/self-hosting/setup)
...
Pour construire le serveur ArjunaCRM et injecter des données dans votre base de données, exécutez la commande suivante :
...
## Étape 8 : Utiliser ArjunaCRM

**Frontend**

Le frontend de ArjunaCRM sera en cours d'exécution à [http://localhost:3001](http://localhost:3001).
Vous pouvez vous connecter en utilisant le compte démo par défaut : `tim@apple.dev` (mot de passe : `tim@apple.dev`)

**Backend**

- Le serveur de ArjunaCRM sera en fonctionnement à [http://localhost:3000](http://localhost:3000)
