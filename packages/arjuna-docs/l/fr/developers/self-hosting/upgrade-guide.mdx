...
1. Dans un terminal, sur l'hôte où ArjunaCRM est en cours d'exécution, éteignez ArjunaCRM: `docker compose down`
...
3. Remettez ArjunaCRM en ligne avec `docker compose up -d`
...
Si vous rencontrez des problèmes d'exécution après la mise à niveau, vous devrez peut-être vider votre cache pour vous assurer qu'il est synchronisé avec les dernières modifications. Exécutez cette commande dans votre conteneur arjuna-server :
...
Mettez à niveau votre instance ArjunaCRM pour utiliser l'image v0.55
...
Dans votre conteneur `arjuna-server`, exécutez :

```bash
yarn command:prod cache:flush
```

Ce problème est spécifique à cette version de ArjunaCRM et ne devrait pas être nécessaire pour les futures mises à niveau.
...
Il doit toujours être dans la plage de la version `major.minor` de votre instance ArjunaCRM actuelle, vous pouvez consulter la version de votre instance dans le panneau d'administration (à l'adresse `/settings/admin-panel`, accessible si votre utilisateur a la propriété `canAccessFullAdminPanel` définie comme vraie dans la base de données) ou en exécutant `echo $APP_VERSION` dans votre conteneur `arjuna-server`.

Pour corriger une version d'espace de travail désynchronisée, vous devrez effectuer une mise à niveau à partir de la version correspondante de ArjunaCRM en suivant le guide de mise à niveau correspondant séquentiellement, et ainsi de suite jusqu'à atteindre la version souhaitée.
...
Mettez à jour votre instance ArjunaCRM pour utiliser l'image v0.52
...
Vous devrez mettre à jour manuellement la version de votre espace de travail à `0.51.0` dans la base de données et mettre à niveau en utilisant la version arjuna `0.52.11` en suivant son guide de mise à niveau juste au-dessus.

### v0.50 à v0.51

Mettez à jour votre instance ArjunaCRM pour utiliser l'image v0.51
...
Mettez à jour votre instance ArjunaCRM pour utiliser l'image v0.50.0
...
Veuillez mettre à jour votre `docker-compose.yml` local avec [docker-compose.yml v0.50.0](https://github.com/vedpragyabharat/arjuna/blob/v0.50.0/packages/arjuna-docker/docker-compose.yml)

### v0.43.0 à v0.44.0

Mettez à jour votre instance ArjunaCRM pour utiliser l'image v0.44.0
...
Mettez à jour votre instance ArjunaCRM pour utiliser l'image v0.43.0
...
2. Mettez à jour votre docker-compose.yml pour utiliser l'image postgres:16 comme dans le fichier [docker-compose.yml](https://raw.githubusercontent.com/vedpragyabharat/arjuna/main/packages/arjuna-docker/docker-compose.yml).
...
Mettez à jour votre instance ArjunaCRM pour utiliser l'image v0.42.0
...
Mettez à jour votre instance ArjunaCRM pour utiliser l'image v0.41.0
...
Mettez à jour votre instance ArjunaCRM pour utiliser l'image v0.40.0
...
Mettez à jour votre instance ArjunaCRM pour utiliser l'image v0.35.0
...
Mettez à jour votre instance ArjunaCRM pour utiliser l'image v0.34.0
...
Mettez à jour votre instance ArjunaCRM pour utiliser l'image v0.33.0
...
À partir de cette version, l'image arjuna-postgres pour la DB est devenue obsolète et arjuna-postgres-spilo est utilisé à la place.
Si vous souhaitez continuer à utiliser l'image arjuna-postgres, remplacez simplement `arjunacrm/arjuna-postgres:${TAG}` par `arjunacrm/arjuna-postgres` dans docker-compose.yml.

### v0.31.0 à v0.32.0

Mettez à jour votre instance ArjunaCRM pour utiliser l'image v0.32.0
...
Mettez à jour votre instance ArjunaCRM pour utiliser l'image v0.31.0
...
Mettez à jour votre instance ArjunaCRM pour utiliser l'image v0.30.0

**Changement significatif**:
Pour améliorer les performances, ArjunaCRM nécessite désormais la configuration de cache redis. Nous avons mis à jour notre [docker-compose.yml](https://raw.githubusercontent.com/vedpragyabharat/arjuna/main/packages/arjuna-docker/docker-compose.yml) pour refléter cela.
...
Mettez à jour votre instance ArjunaCRM pour utiliser l'image v0.24.0
...
Mettez à jour votre instance ArjunaCRM pour utiliser l'image v0.23.0
...
Mettez à jour votre instance ArjunaCRM pour utiliser l'image v0.22.0
