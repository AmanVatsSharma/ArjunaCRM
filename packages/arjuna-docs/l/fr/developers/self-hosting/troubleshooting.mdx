...
Si vous avez une instance ArjunaCRM existante avec des données de production, **NE SUIVEZ PAS** ces étapes, car elles supprimeront définitivement votre base de données !

Lors de l'installation de ArjunaCRM pour la première fois, vous pouvez vouloir changer le mot de passe par défaut de la base de données.
...
Pendant l'installation de ArjunaCRM, vous devez approvisionner votre base de données postgres avec les bons schémas, extensions et utilisateurs.
...
#### Impossible de trouver le module 'arjuna-emails' ou les déclarations de type correspondantes.

Vous devez construire le package `arjuna-emails` avant de lancer l'initialisation de la base de données avec `npx nx run arjuna-emails:build`

#### Package arjuna-x manquant

Assurez-vous d'exécuter yarn dans le répertoire racine, puis d'exécuter `npx nx server:dev arjuna-server`. Si cela ne fonctionne toujours pas, essayez de construire le package manquant manuellement.
...
#### Pendant l'exécution de `npx nx start` ou `npx nx start arjuna-front`, une erreur de mémoire est renvoyée

Dans `packages/arjuna-front/.env`, décommentez `VITE_DISABLE_TYPESCRIPT_CHECKER=true` et `VITE_DISABLE_ESLINT_CHECKER=true` pour désactiver les vérifications en arrière-plan, réduisant ainsi la quantité de RAM nécessaire.

**Si cela ne fonctionne pas :**
Lancez uniquement les services dont vous avez besoin, au lieu de `npx nx start`. Par exemple, si vous travaillez sur le serveur, lancez seulement `npx nx worker arjuna-server`

**Si cela ne fonctionne pas :**
Si vous avez essayé de lancer uniquement `npx nx run arjuna-server:start` sur WSL et que cela échoue avec l'erreur de mémoire ci-dessous :
...
**Si cela ne fonctionne pas :**
Enquêter sur les processus qui consomment le plus de RAM sur votre machine. Chez ArjunaCRM, nous avons remarqué que certaines extensions de VScode consommaient beaucoup de RAM, nous les désactivons donc temporairement.
...
- Ouvrez votre application ArjunaCRM, naviguez jusqu'à `/settings`, et activez le bouton **Avancé** en bas à gauche de l'écran.
- Créez un nouveau webhook.
- Collez **Votre URL Webhook Unique** dans le champ **URL de l'Endpoint** de ArjunaCRM. Réglez les **Filtres** sur `Sociétés` et `Créé`.
<div style={{textAlign: 'center'}}>
...
Commentez le plugin de vérification dans `packages/arjuna-ui/vite-config.ts` comme dans l'exemple ci-dessous
...
1. Exécutez les commandes suivantes :
   ```bash
   docker exec -it arjuna-server-1 yarn
   docker exec -it arjuna-server-1 npx nx database:reset  --configuration=no-seed
   ```
...
Si vous exécutez ArjunaCRM derrière un proxy inverse et rencontrez des problèmes de connexion :
...
2. **Vérifiez les paramètres du proxy inverse :**
   ...
   - Confirmez que votre proxy inverse transfère correctement les requêtes au serveur ArjunaCRM.
   - Assurez-vous que les en-têtes comme `X-Forwarded-For` et `X-Forwarded-Proto` sont correctement configurés.

3. **Redémarrez les services :**

   Après avoir apporté des modifications, redémarrez à la fois le proxy inverse et les conteneurs ArjunaCRM.
...
- Support communautaire :
  ...
  Contactez la [communauté ArjunaCRM](https://github.com/vedpragyabharat/arjuna/issues) ou les [canaux de support](https://discord.gg/cx5n4Jzs57) pour obtenir de l'aide.