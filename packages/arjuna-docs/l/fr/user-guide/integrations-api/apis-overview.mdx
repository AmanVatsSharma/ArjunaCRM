...
sectionInfo: Découvrez comment connecter ArjunaCRM à vos autres outils.
---
...
ArjunaCRM a été conçu pour être convivial pour les développeurs, offrant des API puissantes qui s'adaptent à votre modèle de données personnalisé. Nous fournissons quatre types d'API distincts pour répondre à différents besoins d'intégration.

## Approche axée sur les développeurs

ArjunaCRM génère des API spécifiquement pour votre modèle de données, ce qui signifie :
...
ArjunaCRM propose des API aux formats **REST** et **GraphQL** :
