...
Aceste comenzi ar trebui să fie executate din dosarul packages/arjuna-server.
...
Notă: poți rula `npx nx run arjuna-server:test:integration:with-db-reset` în cazul în care trebuie să resetezi baza de date înainte de a rula testele de integrare.
...
ArjunaCRM folosește în principal NestJS pentru backend.
