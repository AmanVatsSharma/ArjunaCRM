...
1. Într-un terminal, pe găzduitorul unde rulează ArjunaCRM, opriți ArjunaCRM: `docker compose down`.
...
3. Repuneți ArjunaCRM în funcțiune cu comanda `docker compose up -d`.
...
Dacă întâmpinați probleme de runtime după actualizare, s-ar putea să fie nevoie să goliți cache-ul pentru a vă asigura că este sincronizat cu ultimele schimbări. Rulați această comandă în containerul dvs. arjuna-server:
...
Actualizați instanța ArjunaCRM pentru a utiliza imaginea v0.55
...
În containerul dvs. `arjuna-server`, rulați:

```bash
yarn command:prod cache:flush
```

Această problemă este specifică versiunii ArjunaCRM și nu ar trebui să fie necesară pentru actualizările viitoare.
...
Ar trebui să fie mereu în intervalul versiunii ArjunaCRM curente `major.minor`, puteți vizualiza versiunea instanței dvs. în panoul de administrare (la `/settings/admin-panel`, accesibil dacă utilizatorul dvs. are proprietatea `canAccessFullAdminPanel` setată la adevărat în baza de date) sau executând `echo $APP_VERSION` în containerul dvs. `arjuna-server`.

Pentru a remedia o desincronizare a versiunii spațiului de lucru, va trebui să actualizați din versiunea corespunzătoare ArjunaCRM urmând ghidul de actualizare relevant secvențial și așa mai departe până când ajungeți la versiunea dorită.
...
Actualizați instanța ArjunaCRM pentru a utiliza imaginea v0.52
...
Va trebui să actualizați manual versiunea spațiului de lucru la `0.51.0` în baza de date și să actualizați utilizând versiunea arjuna `0.52.11` urmând ghidul de actualizare de mai sus.

### v0.50 la v0.51

Actualizați instanța ArjunaCRM pentru a utiliza imaginea v0.51
...
Actualizați instanța ArjunaCRM pentru a utiliza imaginea v0.50.0
...
Vă rugăm să actualizați docker-compose.yml-ul dvs. local cu [v0.50.0 docker-compose.yml](https://github.com/vedpragyabharat/arjuna/blob/v0.50.0/packages/arjuna-docker/docker-compose.yml)

### v0.43.0 la v0.44.0

Actualizați instanța ArjunaCRM pentru a utiliza imaginea v0.44.0
...
Actualizați instanța ArjunaCRM pentru a utiliza imaginea v0.43.0
...
2. Actualizați docker-compose.yml-ul pentru a utiliza imaginea postgres:16 din fișierul [docker-compose.yml](https://raw.githubusercontent.com/vedpragyabharat/arjuna/main/packages/arjuna-docker/docker-compose.yml).
...
Actualizați instanța ArjunaCRM pentru a utiliza imaginea v0.42.0
...
Actualizați instanța ArjunaCRM pentru a utiliza imaginea v0.41.0
...
Actualizați instanța ArjunaCRM pentru a utiliza imaginea v0.40.0
...
Actualizați instanța ArjunaCRM pentru a utiliza imaginea v0.35.0
...
Actualizați instanța ArjunaCRM pentru a utiliza imaginea v0.34.0
...
Actualizați instanța ArjunaCRM pentru a utiliza imaginea v0.33.0
...
Începând cu această versiune, imaginea arjuna-postgres pentru DB a devenit depășită și se utilizează arjuna-postgres-spilo.
Dacă doriți să păstrați utilizarea imaginii arjuna-postgres, înlocuiți pur și simplu `arjunacrm/arjuna-postgres:${TAG}` cu `arjunacrm/arjuna-postgres` în docker-compose.yml.

### v0.31.0 la v0.32.0

Actualizați instanța ArjunaCRM pentru a utiliza imaginea v0.32.0
...
Actualizați instanța ArjunaCRM pentru a utiliza imaginea v0.31.0
...
Actualizați instanța ArjunaCRM pentru a utiliza imaginea v0.30.0

**Breaking change**:
To enhance performances, ArjunaCRM now requires redis cache to be configured. Am actualizat [docker-compose.yml](https://raw.githubusercontent.com/vedpragyabharat/arjuna/main/packages/arjuna-docker/docker-compose.yml) pentru a reflecta acest lucru.
...
Actualizați instanța ArjunaCRM pentru a utiliza imaginea v0.24.0
...
Actualizați instanța ArjunaCRM pentru a utiliza imaginea v0.23.0
...
Actualizați instanța ArjunaCRM pentru a utiliza imaginea v0.22.0
