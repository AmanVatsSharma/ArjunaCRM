...
Dacă aveți deja o instanță ArjunaCRM cu date de producție, **NU** urmați acești pași deoarece vor șterge permanent baza dvs. de date!

În timp ce instalați ArjunaCRM pentru prima dată, este posibil să doriți să schimbați parola implicită a bazei de date.
...
În timpul instalării ArjunaCRM, trebuie să aprovizionați baza de date postgres cu schemele, extensiile și utilizatorii corecți.
...
#### Nu se poate găsi modulul 'arjuna-emails' sau declarațiile de tip corespunzătoare.

Trebuie să construiți pachetul `arjuna-emails` înainte de a rula inițializarea bazei de date cu `npx nx run arjuna-emails:build`

#### Pachetul arjuna-x lipsă

Asigurați-vă că rulați yarn în directorul rădăcină și apoi rulați `npx nx server:dev arjuna-server`. Dacă acest lucru nu funcționează, încercați să construiți manual pachetul lipsă.
...
#### În timpul rulării `npx nx start` sau `npx nx start arjuna-front`, apare o eroare Out of memory

În `packages/arjuna-front/.env` decomentați `VITE_DISABLE_TYPESCRIPT_CHECKER=true` și `VITE_DISABLE_ESLINT_CHECKER=true` pentru a dezactiva verificările de fundal, astfel reducând cantitatea de RAM necesară.

**Dacă nu funcționează:**  
Rulați doar serviciile de care aveți nevoie, în loc de `npx nx start`. De exemplu, dacă lucrați la server, rulați doar `npx nx worker arjuna-server`

**Dacă nu funcționează:**  
Dacă ați încercat să rulați doar `npx nx run arjuna-server:start` pe WSL și nu reușește cu eroarea de memorie de mai jos:
...
**Dacă nu funcționează:**  
Investigați care procese consumă cea mai mare parte din RAM-ul mașinii dvs. La ArjunaCRM, am observat că unele extensii VScode consumau multă RAM, așa că le dezactivăm temporar.
...
- Deschideți aplicația ArjunaCRM, navigați la `/settings` și activați comutatorul **Avansat** în colțul din stânga jos al ecranului.
- Creați un nou webhook.
- Lipiți **Your Unique Webhook URL** în câmpul **Endpoint Url** din ArjunaCRM. Setați **Filtrele** pe `Companii` și `Create`.
<div style={{textAlign: 'center'}}>
...
Dacă rulați ArjunaCRM în spatele unui proxy invers și întâmpinați probleme de conexiune:
...
2. **Verificați Setările Proxy-ului Invers:**
   ...
   - Confirmați că proxy-ul invers transmite corect cererile către serverul ArjunaCRM.
   - Asigurați-vă că headerele precum `X-Forwarded-For` și `X-Forwarded-Proto` sunt setate corect.

3. **Reporniți Serviciile:**

   După ce faceți schimbări, reporniți atât proxy-ul invers, cât și containerele ArjunaCRM.
...
- Suport Comunitar:
  ...
  Contactați [comunitatea ArjunaCRM](https://github.com/vedpragyabharat/arjuna/issues) sau [canalele de suport](https://discord.gg/cx5n4Jzs57) pentru asistență.