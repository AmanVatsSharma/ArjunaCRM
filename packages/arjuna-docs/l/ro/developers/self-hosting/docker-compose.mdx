...
<Warning>
Docker containers are for production hosting or self-hosting, for the contribution please check the [Local Setup](https://docs.vedpragya.com/l/ro/developers/local-setup).
</Warning>

## Prezentare generală

Acest ghid oferă instrucțiuni pas cu pas pentru a instala și configura aplicația ArjunaCRM folosind Docker Compose. Scopul este de a face procesul direct și de a preveni capcanele comune care ar putea deteriora configurarea dvs.
...
Consultați documentația [Setup Environment Variables](https://docs.vedpragya.com/l/ro/developers/self-hosting/setup) pentru configurare avansată. Toate variabilele de mediu trebuie declarate în fișierul docker-compose.yml la nivel de server și/sau de lucru, în funcție de variabilă.
...
Dacă găzduiți arjunaCRM pe propriul computer, deschideți browserul și navigați la [http://localhost:3000](http://localhost:3000).
...
### Expuneți ArjunaCRM pentru Acces Extern

Implicit, ArjunaCRM rulează pe `localhost` la portul `3000`. Pentru a-l accesa printr-un domeniu extern sau adresă IP, trebuie să configurați `SERVER_URL` în fișierul dvs. `.env`.
...
SSL (HTTPS) este necesar pentru funcționarea corespunzătoare a anumitor funcții ale browserului. Deși aceste funcții pot funcționa în timpul dezvoltării locale (deoarece browserele tratează localhost diferit), o configurare SSL corespunzătoare este necesară când găzduiți ArjunaCRM pe un domeniu obișnuit.
...
Recomandăm cu tărie configurarea ArjunaCRM din spatele unui proxy revers cu terminare SSL pentru securitate și funcționalitate optimă.
...
Dacă întâmpinați vreo problemă, verificați [Depanare](https://docs.vedpragya.com/l/ro/developers/self-hosting/troubleshooting) pentru soluții.
