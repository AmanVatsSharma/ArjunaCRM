...
1. In 'n terminal op die gasheer waar ArjunaCRM loop, skakel ArjunaCRM af met: `docker compose down`

2. Opgradeer die weergawe deur die `TAG`-waarde in die .env-lêer naby jou docker-compose te verander. (Ons beveel aan dat jy `major.minor` weergawes gebruik soos `v0.53`)

3. Bring ArjunaCRM weer aanlyn met `docker compose up -d`
...
As jy na opgradering enige looptydfoute ervaar, moet jy dalk jou kas spoel om te verseker dat dit gesinkroniseer is met die nuutste veranderinge. Voer hierdie opdrag in jou arjuna-dienshouer uit:
...
Opgradeer jou ArjunaCRM-instansie om die v0.55 prent te gebruik.
...
As jy magtigingsfoute op die meeste versoeke ervaar ná opgradering, moet jy dalk jou kas spoel om die nuutste toestemmings te hersien.

In jou `arjuna-diens` houer, voer uit:

```bash
yarn command:prod cache:flush
```

Hierdie kwessie is spesifiek vir hierdie ArjunaCRM-weergawe en moet nie benodig word vir toekomstige opgraderings nie.
...
Ons het die `migrate` opdrag stap binne-in die `upgrade` opdrag geïntegreer. Ons beveel nie aan om `migrate` handmatig binne enige van jou diens/werknemer houers te loop nie.
...
Dit moet altyd in die reeks van jou huidige ArjunaCRM-instansie `major.minor` weergawe wees, jy kan jou instansie-weergawe in die admin-paneel sien (by `/settings/admin-panel`, toeganklik as jou gebruiker die `canAccessFullAdminPanel` eienskap gestel het op waar in die databasis) of deur `echo $APP_VERSION` in jou `arjuna-diens`-houer uit te voer.

Om 'n desinchroniseerde werksgebied-weergawe reg te stel, sal jy van die ooreenstemmende ArjunaCRM-weergawe aflaiend moet opgradeer volgens verwante opgraderingsgids stapsgewys en verder totdat dit die gewenste weergawe bereik.
...
Opgradeer jou ArjunaCRM-instansie om die v0.52 beeld te gebruik.
...
Jy sal jou werksgebied weergawe handmatig moet opdateer na `0.51.0` in die databasis en dan met ArjunaCRM weergawe `0.52.11` verstel, deur sy net bokant opgraderingsgids te volg.

### v0.50 tot v0.51

Opgradeer jou ArjunaCRM-instansie om die v0.51 beeld te gebruik.
...
Opgradeer jou ArjunaCRM-instansie om die v0.50.0 beeld te gebruik.
...
Werk asseblief jou plaaslike `docker-compose.yml` op met [v0.50.0 docker-compose.yml](https://github.com/vedpragyabharat/arjuna/blob/v0.50.0/packages/arjuna-docker/docker-compose.yml)

### v0.43.0 tot v0.44.0

Opgradeer jou ArjunaCRM-instansie om die v0.44.0 beeld te gebruik.
...
Opgradeer jou ArjunaCRM-instansie om die v0.43.0 beeld te gebruik.
...
Maak seker dat jou dump-lêer nie leeg is nie.

2. Opgradeer jou docker-compose.yml om die postgres:16 beeld te gebruik soos in die [docker-compose.yml](https://raw.githubusercontent.com/vedpragyabharat/arjuna/main/packages/arjuna-docker/docker-compose.yml) lêer.
...
Opgradeer jou ArjunaCRM-instansie om die v0.42.0 beeld te gebruik.
...
Opgradeer jou ArjunaCRM-instansie om die v0.41.0 beeld te gebruik.
...
Opgradeer jou ArjunaCRM-instansie om die v0.40.0 beeld te gebruik.
...
Opgradeer jou ArjunaCRM-instansie om die v0.35.0 beeld te gebruik.
...
Opgradeer jou ArjunaCRM-instansie om die v0.34.0 beeld te gebruik.
...
Opgradeer jou ArjunaCRM-instansie om die v0.33.0 beeld te gebruik.
...
Starting from this version, arjuna-postgres image for DB became deprecated and arjuna-postgres-spilo is used instead.
If you want to keep using arjuna-postgres image, simply replace `arjunacrm/arjuna-postgres:${TAG}` with `arjunacrm/arjuna-postgres` in docker-compose.yml.

### v0.31.0 tot v0.32.0

Opgradeer jou ArjunaCRM-instansie om die v0.32.0 beeld te gebruik.
...
Werk jou `.env`-lêer op om die nuwe `REDIS_URL` veranderlike te gebruik in plaas van die individuele Redis-verbindingparameters.
...
Werk jou `.env`-lêer op om die nuwe `APP_SECRET` veranderlike te gebruik in plaas van die individuele tekens geheime (jy kan dieselfde geheim as voorheen gebruik of 'n nuwe ewekansige string genereer)
...
Opgradeer jou ArjunaCRM-instansie om die v0.31.0 beeld te gebruik.
...
Opgradeer jou ArjunaCRM-instansie om die v0.30.0 beeld te gebruik.

**Breaking change**:
To enhance performances, ArjunaCRM now requires redis cache to be configured. Ons het ons [docker-compose.yml](https://raw.githubusercontent.com/vedpragyabharat/arjuna/main/packages/arjuna-docker/docker-compose.yml) opgedateer om dit te reflekteer.
...
Opgradeer jou ArjunaCRM-instansie om die v0.24.0 beeld te gebruik.
...
Opgradeer jou ArjunaCRM-instansie om die v0.23.0 beeld te gebruik.
...
Opgradeer jou ArjunaCRM-instansie om die v0.22.0 beeld te gebruik.
