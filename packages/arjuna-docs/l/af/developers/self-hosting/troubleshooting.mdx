...
As jy 'n bestaande ArjunaCRM-instansie met produksiedata het, **MOENIE** hierdie stappe volg nie, aangesien dit jou databasis permanent sal verwyder!

Terwyl jy ArjunaCRM vir die eerste keer installeer, wil jy dalk die standaard databasis-wagwoord verander.
...
Tydens die ArjunaCRM-installasie moet jy jou postgres-databasis met die regte skemas, uitbreidings en gebruikers voorsien.
...
#### Kan nie module 'arjuna-emails' of sy ooreenstemmende tipesverklarings vind nie.

Jy moet die pakket `arjuna-emails` bou voordat jy die initialisering van die databasis uitvoer met `npx nx run arjuna-emails:build`

#### Ontbrekende arjuna-x pakket

Maak seker dat jy `yarn` in die wortelgids uitvoer en dan `npx nx server:dev arjuna-server`. As dit steeds nie werk nie, probeer om die ontbrekende pakket handmatig te bou.
...
#### Terwyl `npx nx start` of `npx nx start arjuna-front` uitgevoer word, word 'n geheue-fout gewerp.

In `packages/arjuna-front/.env` ontmerk `VITE_DISABLE_TYPESCRIPT_CHECKER=true` en `VITE_DISABLE_ESLINT_CHECKER=true` om agtergrondkontrole uit te skakel, sodoende die hoeveelheid benodige RAM te verminder.

**As dit nie werk nie:**  
Voer slegs die dienste wat jy nodig het, uit in plaas van `npx nx start`. Byvoorbeeld, as jy aan die bediener werk, voer slegs `npx nx worker arjuna-server` uit.

**As dit nie werk nie:**  
As jy probeer het om slegs `npx nx run arjuna-server:start` op WSL uit te voer en dit misluk met die onderstaande geheue-fout:
...
**As dit nie werk nie:**  
Ondersoek watter prosesse die meeste van jou rekenaar-RAM neem. By ArjunaCRM het ons opgemerk dat sommige VScode-uitbreidings baie RAM geneem het, so ons deaktiveer dit tydelik.
...
- Maak jou ArjunaCRM-app oop, navigeer na `/settings`, en skakel die **Gevorderde** skakel onderaan links in.
- Skep 'n nuwe webhook.
- Plak **Jou Unieke Webhook URL** in die **Eindpunt-URL**-veld in ArjunaCRM. Stel die **Filters** tot `Companies` en `Created`.
<div style={{textAlign: 'center'}}>
...
As jy ArjunaCRM agter 'n omgekeerde volmag bedryf en verbindingsprobleme ondervind:
...
2. **Kontroleer Omgekeerde Volmag Instellings:**
   ...
   - Bevestig dat jou omgekeerde volmag versoeke korrek na die ArjunaCRM-bediener deurstuur.
   - Verseker dat kopstukke soos `X-Forwarded-For` en `X-Forwarded-Proto` behoorlik ingestel is.

3. **Herbegin Dienste:**

   Na veranderinge, herbegin beide die omgekeerde volmag en ArjunaCRM-houers.
...
- Gemeenskapsondersteuning:
  ...
  Neem kontak op met die [ArjunaCRM-gemeenskap](https://github.com/vedpragyabharat/arjuna/issues) of [ondersteuningskanale](https://discord.gg/cx5n4Jzs57) vir hulp.