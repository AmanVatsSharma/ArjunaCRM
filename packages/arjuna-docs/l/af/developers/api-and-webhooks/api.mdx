...
Die ArjunaCRM API stel ontwikkelaars in staat om programmaties met die ArjunaCRM CRM-platform te kommunikeer. Met behulp van die API kan jy ArjunaCRM integreer met ander stelsels, data-sinchronisasie outomatiseer en pasgemaakte oplossings om jou kliëntedata bou. Die API voorsien eindpunte om **te skep, lees, opdateer, en uitvee** kern CRM voorwerpe (soos mense en maatskappye) asook toegang tot metadata-konfigurasie.

**API Speelgrond:** Jy kan nou toegang tot die API Speelgrond binne die toep-instellings verkry. Om API-oproepe in reële tyd te toets, teken in by jou ArjunaCRM-werksruimte en navigeer na **Instellings → API's & Webhooks**. Dit open die in-app API Speelgrond en die instellings vir API-sleutels.
**[Gaan na API Instellings](https://app.vedpragya.com/settings)**

## Magtiging

ArjunaCRM se API gebruik API-sleutels vir verifikasie. Elke versoek aan beskermde eindpunte moet 'n API-sleutel in die kopstuk insluit.

- **API-sleutels:** Jy kan 'n nuwe API-sleutel genereer vanaf jou ArjunaCRM-toep se **API-instellings** bladsy. Elke API-sleutel is 'n geheime teken wat toegang tot jou CRM-data verleen, dus hou dit veilig. As 'n sleutel gekompromitteer is, herroep dit vanaf die instellings en genereer 'n nuwe een.
...
- **Wolk:** `https://api.vedpragya.com/` of jou pasgemaakte domein/sub-domein
