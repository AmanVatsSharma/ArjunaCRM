...
Ha már van egy meglévő ArjunaCRM példányod termelési adatokkal, **NE** kövesd ezeket a lépéseket, mert ezek véglegesen törlik az adatbázisod!
...
During ArjunaCRM installation, you need to provision your postgres database with the right schemas, extensions, and users.
...
Make sure to run yarn in the root directory and then run `npx nx server:dev arjuna-server`. If this still doesn't work try building the missing package manually.
...
#### A `npx nx start` vagy `npx nx start arjuna-front` futtatása közben memóriahiány hibát dob
...
**Ha ez nem működik:**  
Csak azokat a szolgáltatásokat futtasd, amelyeket szükséges, ahelyett, hogy a `npx nx start` parancsot használnád. Például, ha a szerverrel dolgozol, csak a `npx nx worker arjuna-server` parancsot futtasd.

**If it does not work:**  
If you tried to run only `npx nx run arjuna-server:start` on WSL and it's failing with the below memory error:
...
**Ha nem működik:**  
Vizsgáld meg mely folyamatok foglalnak el a legtöbb RAM-ot a gépeden. A ArjunaCRM-nél észrevettük, hogy néhány VScode kiterjesztés sok RAM-ot foglal, így ideiglenesen letiltottuk őket.
...
- Nyisd meg a ArjunaCRM alkalmazásod, navigálj a `/settings` oldalra, és engedélyezd az **Advanced** kapcsolót a képernyő bal alsó részén.
- Create a new webhook.
- Paste **Your Unique Webhook URL** in the **Endpoint Url** field in ArjunaCRM. Állítsd be a **Szűrőket** `Companies` és `Created` értékekre.
<div style={{textAlign: 'center'}}>
...
Kommentálj ki ellenőrző bővítményt a `packages/arjuna-ui/vite-config.ts` fájlban, mint az alábbi példában
...
Ha a ArjunaCRM-t egy fordított proxy mögött futtatod, és kapcsolódási hibák merülnek fel:
...
2. **Fordított Proxy Beállítások Ellenőrzése:**
   ...
   - Bizonyosodj meg arról, hogy a fordított proxy rendesen továbbítja a kéréseket a ArjunaCRM szerver felé.
...
3. **Szolgáltatások Újraindítása:**
   ...
   A változtatások után indítsd újra mind a fordított proxy-t, mind a ArjunaCRM konténereket.
...
- Közösségi Támogatás:
  ...
  Fordulj a [ArjunaCRM közösséghez](https://github.com/vedpragyabharat/arjuna/issues) vagy [támogatási csatornákhoz](https://discord.gg/cx5n4Jzs57) segítségért.