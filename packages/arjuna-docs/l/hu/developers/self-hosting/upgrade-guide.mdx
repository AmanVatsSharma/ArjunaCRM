...
1. A Terminálban, azon a gazdagépen, ahol a ArjunaCRM fut, kapcsolja ki a ArjunaCRM-t: `docker compose down`
...
3. Hozza vissza a ArjunaCRM-t online állapotba: `docker compose up -d`
...
Ha bármilyen futásidejű problémát tapasztal frissítés után, előfordulhat, hogy ki kell ürítenie a gyorsítótárát, hogy szinkronban legyen a legutóbbi változásokkal. Futtassa ezt a parancsot a arjuna-server tárolójában:
...
Frissítse ArjunaCRM példányát a v0.55 kép használatára
...
A `arjuna-server` tárolójában futtassa:

```bash
yarn command:prod cache:flush
```

This issue is specific to this ArjunaCRM version and should not be required for future upgrades.
...
A ArjunaCRM példányának aktuális `major.minor` verziójának tartományában kell lennie, megtekintheti példányának verzióját az admin panelen (az `/settings/admin-panel` alatt érhető el, ha a felhasználónak a `canAccessFullAdminPanel` tulajdonság be van állítva az adatbázisban) vagy a `echo $APP_VERSION` futtatásával a `arjuna-server` tárolóban.

Egy deszinkronizált munkaterület verziójának javításához frissítenie kell a megfelelő ArjunaCRM verzióját követve a kapcsolódó frissítési útmutatókat egymás után, amíg el nem éri a kívánt verziót.
...
Frissítse ArjunaCRM példányát a v0.52 kép használatára
...
Frissítse ArjunaCRM példányát a v0.51 kép használatára
...
Frissítse ArjunaCRM példányát a v0.50.0 kép használatára
...
Kérjük, frissítse helyi `docker-compose.yml` fájlját a [v0.50.0 docker-compose.yml](https://github.com/vedpragyabharat/arjuna/blob/v0.50.0/packages/arjuna-docker/docker-compose.yml) alapján

### v0.43.0-tól v0.44.0-ig

Frissítse ArjunaCRM példányát a v0.44.0 kép használatára
...
Frissítse ArjunaCRM példányát a v0.43.0 kép használatára
...
2. Frissítse docker-compose.yml fájlját a postgres:16 kép használatára a [docker-compose.yml](https://raw.githubusercontent.com/vedpragyabharat/arjuna/main/packages/arjuna-docker/docker-compose.yml) fájl szerint.
...
Frissítse ArjunaCRM példányát a v0.42.0 kép használatára
...
Frissítse ArjunaCRM példányát a v0.41.0 kép használatára
...
Frissítse ArjunaCRM példányát a v0.40.0 képre
...
Frissítse ArjunaCRM példányát a v0.35.0 kép használatára
...
Frissítse ArjunaCRM példányát a v0.34.0 kép használatára
...
Frissítse ArjunaCRM példányát a v0.33.0 kép használatára
...
Ettől a verziótól kezdve, a arjuna-postgres kép az adatbázishoz elavulttá vált, és a arjuna-postgres-spilo-t használják helyette.
Ha továbbra is szeretné használni a arjuna-postgres képet, egyszerűen cserélje ki a `arjunacrm/arjuna-postgres:${TAG}`-t `arjunacrm/arjuna-postgres`-re a docker-compose.yml-ben.

### v0.31.0-től v0.32.0-ig

Frissítse ArjunaCRM példányát a v0.32.0 kép használatára
...
Frissítse ArjunaCRM példányát a v0.31.0 kép használatára
...
Frissítse ArjunaCRM példányát a v0.30.0 kép használatára

**Breaking change**:
To enhance performances, ArjunaCRM now requires redis cache to be configured. Frissítettük a [docker-compose.yml](https://raw.githubusercontent.com/vedpragyabharat/arjuna/main/packages/arjuna-docker/docker-compose.yml) hogy tükrözze ezt.
...
Frissítse ArjunaCRM példányát a v0.24.0 kép használatára
...
Frissítse ArjunaCRM példányát a v0.23.0 kép használatára
...
Frissítse ArjunaCRM példányát a v0.22.0 kép használatára
