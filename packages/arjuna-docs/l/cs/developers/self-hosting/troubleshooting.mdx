...
Pokud máte existující instanci ArjunaCRM s produkčními daty, **NEPROVÁDĚJTE** tyto kroky, protože trvale smažou vaši databázi!

Při první instalaci ArjunaCRM můžete chtít změnit výchozí heslo pro databázi.
...
Během instalace ArjunaCRM musíte nastavit svou postgres databázi s správnými schématy, rozšířeními a uživateli.
...
Před spuštěním inicializace databáze musíte sestavit balíček `arjuna-emails` s `npx nx run arjuna-emails:build`

#### Chybí balíček arjuna-x

Ujistěte se, že v kořenovém adresáři spouštíte `yarn` a poté spusťte `npx nx server:dev arjuna-server`. Pokud stále nefunguje, zkuste chybějící balíček sestavit ručně.
...
#### Při spuštění `npx nx start` nebo `npx nx start arjuna-front` došlo k chybě nedostatku paměti
...
**Pokud to nefunguje:**  
Spusťte pouze služby, které potřebujete, místo `npx nx start`. Například pokud pracujete na serveru, spusťte pouze `npx nx worker arjuna-server`

**Pokud to nefunguje:**  
Pokud jste se pokusili pouze spustit `npx nx run arjuna-server:start` na WSL a došlo k chybě nízké paměti:
...
**Pokud to nefunguje:**  
Analyzujte, které procesy vám zabírají většinu paměti na stroji. V ArjunaCRM jsme si všimli, že některá rozšíření VScode zabírala hodně paměti RAM, takže jsme je dočasně deaktivovali.

**Pokud to nefunguje:**  
Restartování stroje pomáhá vyčistit „ghost“ procesy.
...
- Vložte **Svou unikátní webhook URL** do pole **Endpoint Url** ve ArjunaCRM. Nastavte **Filtry** na `Companies` a `Created`.
<div style={{textAlign: 'center'}}>
...
Pokud provozujete ArjunaCRM za reverzní proxy a máte problémy s připojením:
...
2. **Zkontrolujte nastavení reverzní proxy:**
   ...
   - Potvrďte, že vaše reverzní proxy správně směruje požadavky na server ArjunaCRM.
   - Ujistěte se, že záhlaví jako `X-Forwarded-For` a `X-Forwarded-Proto` jsou správně nastaveny.

3. **Restartujte služby:**

   Po provedení změn restartujte jak reverzní proxy, tak kontejnery ArjunaCRM.
...
- Podpora komunity:
  ...
  Obraťte se na [komunitu ArjunaCRM](https://github.com/vedpragyabharat/arjuna/issues) nebo [podpůrné kanály](https://discord.gg/cx5n4Jzs57) pro pomoc.