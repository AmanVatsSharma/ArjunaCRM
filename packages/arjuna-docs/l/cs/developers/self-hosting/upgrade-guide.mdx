...
1. V terminálu, na hostiteli, kde ArjunaCRM běží, vypněte ArjunaCRM: `docker compose down`
...
3. Opětovně zapněte ArjunaCRM pomocí `docker compose up -d`
...
Pokud po upgradu narazíte na problémy s výkonem, může být nutné vyprázdnit cache, aby bylo zajištěno její sladění s nejnovějšími změnami. Spusťte tento příkaz v kontejneru arjuna-server:
...
Upgradujte svou instanci ArjunaCRM pro použití v0.55 image
...
Ve svém kontejneru `arjuna-server` spusťte:

```bash
yarn command:prod cache:flush
```

Tento problém je specifický pro tuto verzi ArjunaCRM a neměl by být vyžadován pro budoucí upgrady.
...
Měla by se vždy nacházet v rozmezí vaší aktuální instance ArjunaCRM `major.minor` verze. Můžete zkontrolovat verzi instance na ovládacím panelu administrátora (na `/settings/admin-panel`, přístupné, pokud máte v databázi nastaveno uživatelské vlastnosti `canAccessFullAdminPanel` na true) nebo spuštěním `echo $APP_VERSION` ve vašem kontejneru `arjuna-server`.

Chcete-li opravit asynchronizaci verzí pracovního prostoru, budete muset upgradovat z odpovídající verze arjuna podle souvisejícího průvodce upgradem po jednotlivých krocích, až do dosažení požadované verze.
...
Upgradujte svou instanci ArjunaCRM pro použití v0.52 image
...
Budete muset ručně změnit verzi pracovního prostoru na `0.51.0` v databázi a upgradovat pomocí arjuna verze `0.52.11` podle jejího výše uvedeného průvodce upgradem.

### v0.50 až v0.51

Upgradujte svou instanci ArjunaCRM pro použití v0.51 image
...
Upgradujte svou instanci ArjunaCRM pro použití v0.50.0 image
...
Aktualizujte svůj místní `docker-compose.yml` pomocí [docker-compose.yml v0.50.0](https://github.com/vedpragyabharat/arjuna/blob/v0.50.0/packages/arjuna-docker/docker-compose.yml)

### v0.43.0 až v0.44.0

Upgradujte svou instanci ArjunaCRM pro použití v0.44.0 image
...
Upgradujte svou instanci ArjunaCRM pro použití v0.43.0 image
...
2. Upgradujte svůj `docker-compose.yml` na použití obrazu postgres:16 podle [docker-compose.yml](https://raw.githubusercontent.com/vedpragyabharat/arjuna/main/packages/arjuna-docker/docker-compose.yml) soubor.
...
Upgradujte svou instanci ArjunaCRM pro použití v0.42.0 image
...
Upgradujte svou instanci ArjunaCRM pro použití v0.41.0 image
...
Upgradujte svou instanci ArjunaCRM pro použití v0.40.0 image
...
Upgradujte svou instanci ArjunaCRM pro použití v0.35.0 image
...
Upgradujte svou instanci ArjunaCRM pro použití v0.34.0 image
...
Upgradujte svou instanci ArjunaCRM pro použití v0.33.0 image
...
Od této verze se obraz arjuna-postgres pro DB stal zastaralým a místo něj se používá arjuna-postgres-spilo.
Pokud chcete pokračovat v používání obrazu arjuna-postgres, jednoduše nahraďte `arjunacrm/arjuna-postgres:${TAG}` za `arjunacrm/arjuna-postgres` v docker-compose.yml.

### v0.31.0 až v0.32.0

Upgradujte svou instanci ArjunaCRM pro použití v0.32.0 image
...
Upgradujte svou instanci ArjunaCRM pro použití v0.31.0 image
...
Upgradujte svou instanci ArjunaCRM pro použití v0.30.0 image

**Breaking change**:
To enhance performances, ArjunaCRM now requires redis cache to be configured. Aktualizovali jsme náš [docker-compose.yml](https://raw.githubusercontent.com/vedpragyabharat/arjuna/main/packages/arjuna-docker/docker-compose.yml), aby to odrážel.
...
Upgradujte svou instanci ArjunaCRM pro použití v0.24.0 image
...
Upgradujte svou instanci ArjunaCRM pro použití v0.23.0 image
...
Upgradujte svou instanci ArjunaCRM pro použití v0.22.0 image
