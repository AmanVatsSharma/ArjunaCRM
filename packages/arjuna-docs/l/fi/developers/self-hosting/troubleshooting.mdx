...
Jos sinulla on olemassa oleva ArjunaCRM-Instanssi tuotantodatalla, **ÄLÄ** seuraa näitä ohjeita, koska ne poistavat pysyvästi tietokantasi!

Kun asennat ArjunaCRMä ensimmäistä kertaa, saatat haluta muuttaa oletustietokannan salasanaa.
...
ArjunaCRM-asennuksen aikana sinun on provisioitava postgres-tietokanta oikeilla schemoilla, laajennuksilla ja käyttäjillä.
...
#### Moduulia 'arjuna-emails' tai sen vastaavia tyypin ilmoituksia ei löydy.

Sinun on rakennettava paketti `arjuna-emails` ennen tietokannan alustamisen suorittamista komennolla `npx nx run arjuna-emails:build`

#### Puuttuva arjuna-x-paketti

Varmista, että suoritat yarn juurihakemistossa ja sitten käynnistät `npx nx server:dev arjuna-server`. Jos tämä ei edelleenkään toimi, yritä rakentaa puuttuva paketti manuaalisesti.
...
#### Kun `npx nx start` tai `npx nx start arjuna-front` suoritetaan, muistivirhe ilmoitetaan

In `packages/arjuna-front/.env` uncomment `VITE_DISABLE_TYPESCRIPT_CHECKER=true` and `VITE_DISABLE_ESLINT_CHECKER=true` to disable background checks thus reducing amount of needed RAM.

**Jos tämä ei toimi:**  
Suorita vain tarvitsemasi palvelut sen sijaan, että käyttäisit `npx nx start`. Esimerkiksi, jos työskentelet palvelimen kanssa, suorita vain `npx nx worker arjuna-server`

**Jos tämä ei toimi:**  
Jos yritit ajaa vain `npx nx run arjuna-server:start` WSL:ssä ja se epäonnistuu seuraavalla muistivirheellä:
...
**Jos tämä ei toimi:**  
Tutki, mitkä prosessit kuluttavat eniten koneesi RAM-muistia. Huomasimme ArjunaCRMssä, että jotkut VScode-laajennukset kuluttivat paljon RAM-muistia, joten poistamme ne tilapäisesti käytöstä.
...
- Avaa ArjunaCRM-sovellus, siirry kohtaan `/settings`, ja ota **Edistynyt** kytkin käyttöön näytön vasemmasta alakulmasta.
- Luo uusi webhook.
- Liitä **Sinun ainutlaatuinen Webhook-URL** **Endpoint Url** -kenttään ArjunaCRMssä. Aseta **Suodattimet** arvoiksi `Yritykset` ja `Luotu`.
<div style={{textAlign: 'center'}}>
...
Kommentoi checker-liitin tiedostossa `packages/arjuna-ui/vite-config.ts` kuten alla olevassa esimerkissä
...
Jos käytät ArjunaCRMa käänteisen proxyn takana ja kohtaat yhteysongelmia:
...
2. **Tarkista käänteisen proxyn asetukset:**
   ...
   - Varmista, että käänteinen proxy välittää pyynnöt oikein ArjunaCRM-palvelimelle.
   - Varmista, että otsikot, kuten `X-Forwarded-For` ja `X-Forwarded-Proto`, ovat asetettu oikein.

3. **Käynnistä palvelut uudelleen:**

   Muokkausten jälkeen käynnistä sekä käänteinen proxy että ArjunaCRM-kontit uudelleen.
...
- Yhteisön tuki:
  ...
  Ota yhteyttä [ArjunaCRM-yhteisöön](https://github.com/vedpragyabharat/arjuna/issues) tai [tukikanaviin](https://discord.gg/cx5n4Jzs57) saadaksesi apua.