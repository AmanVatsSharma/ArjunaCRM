...
<Warning>
Docker containers are for production hosting or self-hosting, for the contribution please check the [Local Setup](https://docs.vedpragya.com/l/fi/developers/local-setup).
</Warning>

## Yleiskatsaus

Tämä opas tarjoaa vaiheittaiset ohjeet, kuinka asentaa ja määrittää ArjunaCRM-sovellus käyttämällä Docker Composea. Tavoitteena on tehdä prosessista yksinkertainen ja välttää yleisiä sudenkuoppia, jotka voivat rikkoa asennuksen.

**Tärkeää:** Muuta vain asetuksia, jotka on erikseen mainittu tässä oppaassa. Altering other configurations may lead to issues.

See docs [Setup Environment Variables](https://docs.vedpragya.com/l/fi/developers/self-hosting/setup) for advanced configuration. Kaikki ympäristömuuttujat on määritettävä docker-compose.yml-tiedostossa palvelin- ja/tai työntekijätasolla muuttujasta riippuen.
...
Jos isännöit arjunaCRM:ää omalla tietokoneellasi, avaa selaimesi ja siirry osoitteeseen [http://localhost:3000](http://localhost:3000).
...
### Avaa ArjunaCRM ulkoiseen käyttöön

Oletuksena, ArjunaCRM toimii `localhost` portissa `3000`. Päästäksesi siihen ulkoisen domainin tai IP-osoitteen kautta, sinun tulee konfiguroida `SERVER_URL` `.env` tiedostossasi.
...
SSL (HTTPS) vaaditaan tiettyjen selaintoimintojen toimimiseksi oikein. Vaikka nämä toiminnot saattavat toimia paikallisessa kehityksessä (koska selaimet käsittelevät localhostia eri tavalla), oikea SSL-asetus vaaditaan, kun isännöit ArjunaCRMä normaalilla domainilla.
...
Suosittelemme vahvasti sijoittamaan ArjunaCRMn SSL-terminaalin taakse optimaalisen turvallisuuden ja toimivuuden varmistamiseksi.
...
Jos kohtaat ongelmia, tarkista [Vianmääritys](https://docs.vedpragya.com/l/fi/developers/self-hosting/troubleshooting) ratkaisujen saamiseksi.
