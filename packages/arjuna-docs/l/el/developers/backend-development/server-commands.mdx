...
Αυτές οι εντολές θα πρέπει να εκτελούνται από το φάκελο packages/arjuna-server.
...
Note: you can run `npx nx run arjuna-server:test:integration:with-db-reset` in case you need to reset the database before running the integration tests.
...
Το ArjunaCRM χρησιμοποιεί κυρίως το NestJS για το backend.
