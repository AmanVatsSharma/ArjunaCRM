...
describe('Successful user and workspace creation', () => {
  ...
  it('should sign up a new user and create a new workspace successfully', async () => {
    ...
    const arjunaStandardApp = findManyApplicationsData.find(
      (application) =>
        application.universalIdentifier ===
        ARJUNA_STANDARD_APPLICATION.universalIdentifier,
    );

    jestExpectToBeDefined(arjunaStandardApp);
    const {
      sourcePath: _sourcePath,
      sourceType: _sourceType,
      ...expectedStandardArjunaCRMApplication
    } = ARJUNA_STANDARD_APPLICATION;

    expect(arjunaStandardApp).toMatchObject(expectedStandardArjunaCRMApplication);
