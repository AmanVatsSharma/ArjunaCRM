...
export class AiModelRegistryService {
  ...
  constructor(private arjunaConfigService: ArjunaCRMConfigService) {
  ...
  private buildModelRegistry(): void {
    ...
    const openaiApiKey = this.arjunaConfigService.get('OPENAI_API_KEY');

    if (openaiApiKey) {
      this.registerOpenAIModels();
    }

    const anthropicApiKey = this.arjunaConfigService.get('ANTHROPIC_API_KEY');

    if (anthropicApiKey) {
      this.registerAnthropicModels();
    }

    const xaiApiKey = this.arjunaConfigService.get('XAI_API_KEY');

    if (xaiApiKey) {
      this.registerXaiModels();
    }

    const openaiCompatibleBaseUrl = this.arjunaConfigService.get(
      'OPENAI_COMPATIBLE_BASE_URL',
    );
    const openaiCompatibleModelNames = this.arjunaConfigService.get(
  ...
  private registerOpenAICompatibleModels(
    ...
    const apiKey = this.arjunaConfigService.get('OPENAI_COMPATIBLE_API_KEY');
  ...
  getDefaultSpeedModel(): RegisteredAIModel {
    const defaultModelId = this.arjunaConfigService.get(
  ...
  async validateApiKey(provider: ModelProvider): Promise<void> {
    ...
    switch (provider) {
      case ModelProvider.OPENAI:
        apiKey = this.arjunaConfigService.get('OPENAI_API_KEY');
        break;
      case ModelProvider.ANTHROPIC:
        apiKey = this.arjunaConfigService.get('ANTHROPIC_API_KEY');
        break;
      case ModelProvider.XAI:
        apiKey = this.arjunaConfigService.get('XAI_API_KEY');
        break;
      case ModelProvider.OPENAI_COMPATIBLE:
        apiKey = this.arjunaConfigService.get('OPENAI_COMPATIBLE_API_KEY');
        break;
      default:
        return;
    }
