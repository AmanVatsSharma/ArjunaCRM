...
import { Sources } from 'arjuna-shared/types';
...
export class ServerlessFunctionService {
  ...
  async executeOneServerlessFunction(
    ...
    if (this.arjunaConfigService.get('SERVERLESS_LOGS_ENABLED')) {
  ...
  async publishOneServerlessFunctionOrFail(id: string, workspaceId: string) {
    ...
    // See https://github.com/vedpragyabharat/arjuna/issues/13058
  ...
  private async throttleExecution(workspaceId: string) {
    try {
      await this.throttlerService.tokenBucketThrottleOrThrow(
        ...
        1,
        this.arjunaConfigService.get('SERVERLESS_FUNCTION_EXEC_THROTTLE_LIMIT'),
        this.arjunaConfigService.get('SERVERLESS_FUNCTION_EXEC_THROTTLE_TTL'),
      );
    } catch {
      throw new ServerlessFunctionException(
