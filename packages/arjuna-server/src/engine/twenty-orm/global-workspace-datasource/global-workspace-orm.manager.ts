...
import { GlobalWorkspaceDataSourceService } from 'src/engine/arjuna-orm/global-workspace-datasource/global-workspace-datasource.service';
import type { WorkspaceRepository } from 'src/engine/arjuna-orm/repository/workspace.repository';
import {
  type ORMWorkspaceContext,
  withWorkspaceContext,
} from 'src/engine/arjuna-orm/storage/orm-workspace-context.storage';
...
export class GlobalWorkspaceOrmManager {
  constructor(
    ...
    private readonly arjunaORMGlobalManager: ArjunaCRMORMGlobalManager,
  ) {}
  ...
  async getRepository<T extends ObjectLiteral>(
    ...
    if (typeof workspaceEntityOrObjectMetadataName === 'string') {
      return this.arjunaORMGlobalManager.getRepositoryForWorkspace<T>(
        workspaceId,
  ...
  async getDataSourceForWorkspace(
    ...
    return this.arjunaORMGlobalManager.getDataSourceForWorkspace({
      workspaceId,
    });
  }
  ...
  async destroyDataSourceForWorkspace(workspaceId: string): Promise<void> {
    ...
    await this.arjunaORMGlobalManager.destroyDataSourceForWorkspace(
      workspaceId,
    );
  }
