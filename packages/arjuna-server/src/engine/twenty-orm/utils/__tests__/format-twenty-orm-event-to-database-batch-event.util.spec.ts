...
describe('formatArjunaCRMOrmEventToDatabaseBatchEvent', () => {
  ...
  describe('UPDATED action', () => {
    it('should throw ArjunaCRMORMException when no matching before entity is found in array of beforeEntities', () => {
      const afterEntities = [
        {
          id: 'record-1',
      ...
      try {
        ...
        expect(error).toBeInstanceOf(ArjunaCRMORMException);
        expect((error as ArjunaCRMORMException).code).toBe(
          ArjunaCRMORMExceptionCode.ORM_EVENT_DATA_CORRUPTED,
        );
        expect((error as ArjunaCRMORMException).message).toBe(
    ...
    it('should successfully create update events when matching before entities are found and in the right order', () => {
      ...
      const result = formatArjunaCRMOrmEventToDatabaseBatchEvent({
        action: DatabaseEventAction.UPDATED,
        objectMetadataItem: flatObjectMetadata,
        flatFieldMetadataMaps,
        workspaceId: mockWorkspaceId,
