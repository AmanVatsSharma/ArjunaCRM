...
import { RELATION_NESTED_QUERY_KEYWORDS } from 'arjuna-shared/constants';
import {
  type EntityTarget,
  type ObjectLiteral,
...
} from 'src/engine/arjuna-orm/entity-manager/types/relation-nested-query-fields-by-entity-index.type';
import {
  ArjunaCRMORMException,
  ArjunaCRMORMExceptionCode,
} from 'src/engine/arjuna-orm/exceptions/arjuna-orm.exception';
import { formatConnectRecordNotFoundErrorMessage } from 'src/engine/arjuna-orm/relation-nested-queries/utils/formatConnectRecordNotFoundErrorMessage.util';
import { type WorkspaceSelectQueryBuilder } from 'src/engine/arjuna-orm/repository/workspace-select-query-builder';
import { computeRelationConnectQueryConfigs } from 'src/engine/arjuna-orm/utils/compute-relation-connect-query-configs.util';
import { createSqlWhereTupleInClause } from 'src/engine/arjuna-orm/utils/create-sql-where-tuple-in-clause.utils';
import { extractNestedRelationFieldsByEntityIndex } from 'src/engine/arjuna-orm/utils/extract-nested-relation-fields-by-entity-index.util';
import { getAssociatedRelationFieldName } from 'src/engine/arjuna-orm/utils/get-associated-relation-field-name.util';
import { getObjectMetadataFromEntityTarget } from 'src/engine/arjuna-orm/utils/get-object-metadata-from-entity-target.util';
import { getRecordToConnectFields } from 'src/engine/arjuna-orm/utils/get-record-to-connect-fields.util';
        ...
        if (
          ...
          if (recordToConnect.length !== 1) {
            ...
            throw new ArjunaCRMORMException(
              errorMessage,
              ArjunaCRMORMExceptionCode.CONNECT_RECORD_NOT_FOUND,
              {
                userFriendlyMessage,
              },
            );
