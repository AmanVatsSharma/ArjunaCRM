...
import { getAbsoluteUrl } from 'arjuna-shared/utils';
...
export class CallWebhookJob {
  ...
  private async callWebhook(data: CallWebhookJobData): Promise<void> {
    ...
    try {
      ...
      if (secret) {
        headers['X-ArjunaCRM-Webhook-Timestamp'] = Date.now().toString();
        headers['X-ArjunaCRM-Webhook-Signature'] = this.generateSignature(
          payloadWithoutSecret,
          secret,
          headers['X-ArjunaCRM-Webhook-Timestamp'],
        );
        headers['X-ArjunaCRM-Webhook-Nonce'] = crypto
          .randomBytes(16)
          .toString('hex');
      }
