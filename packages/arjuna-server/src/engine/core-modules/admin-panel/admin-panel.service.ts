...
import { CONFIG_VARIABLES_GROUP_METADATA } from 'src/engine/core-modules/arjuna-config/constants/config-variables-group-metadata';
import { type ConfigVariablesGroup } from 'src/engine/core-modules/arjuna-config/enums/config-variables-group.enum';
...
export class AdminPanelService {
  ...
  getConfigVariablesGrouped(): ConfigVariablesOutput {
    const rawEnvVars = this.arjunaConfigService.getAll();
  ...
  getConfigVariable(key: string): ConfigVariableDTO {
    const variableWithMetadata =
      this.arjunaConfigService.getVariableWithMetadata(
  ...
  async getVersionInfo(): Promise<VersionInfoDTO> {
    const currentVersion = this.arjunaConfigService.get('APP_VERSION');

    try {
      const rawResponse = await axios.get<unknown>(
        'https://hub.docker.com/v2/repositories/arjunacrm/arjuna/tags?page_size=100',
      );
      const response = z
        .object({
          data: z.object({
