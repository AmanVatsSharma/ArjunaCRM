...
export class ApplicationService {
  ...
  async findWorkspaceArjunaCRMStandardAndCustomApplicationOrThrow({
    ...
    const arjunaStandardApplicationId =
      flatApplicationMaps.idByUniversalIdentifier[
        ARJUNA_STANDARD_APPLICATION.universalIdentifier
      ];

    if (!isDefined(arjunaStandardApplicationId)) {
      throw new ApplicationException(
        `Could not find workspace arjuna standard applicationId in cache ${workspaceId}`,
        ApplicationExceptionCode.APPLICATION_NOT_FOUND,
      );
    }

    const arjunaStandardFlatApplication =
      flatApplicationMaps.byId[arjunaStandardApplicationId];
    const workspaceCustomFlatApplication =
      flatApplicationMaps.byId[workspace.workspaceCustomApplicationId];

    if (
      !isDefined(arjunaStandardFlatApplication) ||
  ...
  async createArjunaCRMStandardApplication(
    {
      workspaceId,
    ...
    const arjunaStandardApplication = await this.create(
      {
        ...ARJUNA_STANDARD_APPLICATION,
    ...
    return arjunaStandardApplication;
  }
