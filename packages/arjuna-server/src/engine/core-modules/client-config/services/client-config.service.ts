...
import { SupportDriver } from 'src/engine/core-modules/arjuna-config/interfaces/support.interface';
...
export class ClientConfigService {
  constructor(
    private arjunaConfigService: ArjunaCRMConfigService,
    private domainServerConfigService: DomainServerConfigService,
    private aiModelRegistryService: AiModelRegistryService,
  ) {}

  async getClientConfig(): Promise<ClientConfig> {
    const captchaProvider = this.arjunaConfigService.get('CAPTCHA_DRIVER');
    const supportDriver = this.arjunaConfigService.get('SUPPORT_DRIVER');
    const calendarBookingPageId = this.arjunaConfigService.get(
    ...
    const clientConfig: ClientConfig = {
      appVersion: this.arjunaConfigService.get('APP_VERSION'),
      billing: {
        isBillingEnabled: this.arjunaConfigService.get('IS_BILLING_ENABLED'),
        billingUrl: this.arjunaConfigService.get('BILLING_PLAN_REQUIRED_LINK'),
        trialPeriods: [
          {
            duration: this.arjunaConfigService.get(
      ...
      aiModels,
      authProviders: {
        google: this.arjunaConfigService.get('AUTH_GOOGLE_ENABLED'),
        magicLink: false,
        password: this.arjunaConfigService.get('AUTH_PASSWORD_ENABLED'),
        microsoft: this.arjunaConfigService.get('AUTH_MICROSOFT_ENABLED'),
        sso: [],
      },
      signInPrefilled: this.arjunaConfigService.get('SIGN_IN_PREFILLED'),
      isMultiWorkspaceEnabled: this.arjunaConfigService.get(
        'IS_MULTIWORKSPACE_ENABLED',
      ),
      isEmailVerificationRequired: this.arjunaConfigService.get(
        'IS_EMAIL_VERIFICATION_REQUIRED',
      ),
      defaultSubdomain: this.arjunaConfigService.get('DEFAULT_SUBDOMAIN'),
      ...
      support: {
        ...
        supportFrontChatId: this.arjunaConfigService.get(
          'SUPPORT_FRONT_CHAT_ID',
        ),
      },
      sentry: {
        environment: this.arjunaConfigService.get('SENTRY_ENVIRONMENT'),
        release: this.arjunaConfigService.get('APP_VERSION'),
        dsn: this.arjunaConfigService.get('SENTRY_FRONT_DSN'),
      },
      captcha: {
        provider: captchaProvider ? captchaProvider : undefined,
        siteKey: this.arjunaConfigService.get('CAPTCHA_SITE_KEY'),
      },
      chromeExtensionId: this.arjunaConfigService.get('CHROME_EXTENSION_ID'),
      api: {
        mutationMaximumAffectedRecords: this.arjunaConfigService.get(
          'MUTATION_MAXIMUM_AFFECTED_RECORDS',
        ),
      },
      isAttachmentPreviewEnabled: this.arjunaConfigService.get(
        'IS_ATTACHMENT_PREVIEW_ENABLED',
      ),
      analyticsEnabled: this.arjunaConfigService.get('ANALYTICS_ENABLED'),
      canManageFeatureFlags:
        this.arjunaConfigService.get('NODE_ENV') ===
          NodeEnvironment.DEVELOPMENT ||
        this.arjunaConfigService.get('IS_BILLING_ENABLED'),
      publicFeatureFlags: PUBLIC_FEATURE_FLAGS,
      isMicrosoftMessagingEnabled: this.arjunaConfigService.get(
        'MESSAGING_PROVIDER_MICROSOFT_ENABLED',
      ),
      isMicrosoftCalendarEnabled: this.arjunaConfigService.get(
        'CALENDAR_PROVIDER_MICROSOFT_ENABLED',
      ),
      isGoogleMessagingEnabled: this.arjunaConfigService.get(
        'MESSAGING_PROVIDER_GMAIL_ENABLED',
      ),
      isGoogleCalendarEnabled: this.arjunaConfigService.get(
        'CALENDAR_PROVIDER_GOOGLE_ENABLED',
      ),
      isConfigVariablesInDbEnabled: this.arjunaConfigService.get(
        'IS_CONFIG_VARIABLES_IN_DB_ENABLED',
      ),
      isImapSmtpCaldavEnabled: this.arjunaConfigService.get(
