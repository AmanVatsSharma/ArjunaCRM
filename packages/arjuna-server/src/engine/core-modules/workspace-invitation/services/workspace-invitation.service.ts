...
import ms from 'ms';
import { SendInviteLinkEmail } from 'arjuna-emails';
import { AppPath } from 'arjuna-shared/types';
import { getAppPath, isDefined } from 'arjuna-shared/utils';
...
export class WorkspaceInvitationService {
    ...
    for (const invitation of invitationsPr) {
      if (invitation.status === 'fulfilled') {
        ...
        const emailData = {
          ...
          serverUrl: this.arjunaConfigService.get('SERVER_URL'),
        ...
        const joinTeamMsg = msg`Join your team on ArjunaCRM`;
        const i18n = this.i18nService.getI18nInstance(sender.locale);
        const subject = i18n._(joinTeamMsg);

        await this.emailService.send({
          from: `${sender.name.firstName} ${sender.name.lastName} (via ArjunaCRM) <${this.arjunaConfigService.get('EMAIL_FROM_ADDRESS')}>`,
  ...
  async generateInvitationToken(workspaceId: string, email: string) {
    const expiresIn = this.arjunaConfigService.get(
