...
export class SearchArticlesTool implements Tool {
  description =
    'Search ArjunaCRM documentation and help articles to find information about features, setup, usage, and troubleshooting.';
  ...
  async execute(parameters: ToolInput): Promise<ToolOutput> {
    ...
    try {
      const MINTLIFY_API_KEY = this.arjunaConfigService.get('MINTLIFY_API_KEY');
      const MINTLIFY_SUBDOMAIN =
        this.arjunaConfigService.get('MINTLIFY_SUBDOMAIN');
      ...
      const endpoint = useDirectApi
        ...
        : 'https://arjuna-help-search.com/search/arjuna';
