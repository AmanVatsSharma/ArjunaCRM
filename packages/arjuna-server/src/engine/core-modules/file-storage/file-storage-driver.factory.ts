...
export class FileStorageDriverFactory extends DriverFactoryBase<StorageDriver> {
  ...
  protected buildConfigKey(): string {
    const storageType = this.arjunaConfigService.get('STORAGE_TYPE');

    if (storageType === StorageDriverType.LOCAL) {
      const storagePath = this.arjunaConfigService.get('STORAGE_LOCAL_PATH');
  ...
  protected createDriver(): StorageDriver {
    ...
    switch (storageType) {
      ...
      case StorageDriverType.S_3: {
        const bucketName = this.arjunaConfigService.get('STORAGE_S3_NAME');
        const endpoint = this.arjunaConfigService.get('STORAGE_S3_ENDPOINT');
        const region = this.arjunaConfigService.get('STORAGE_S3_REGION');
        const accessKeyId = this.arjunaConfigService.get(
