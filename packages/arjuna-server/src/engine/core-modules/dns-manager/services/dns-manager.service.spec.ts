...
describe('DnsManagerService', () => {
  ...
  it('should initialize cloudflareClient when CLOUDFLARE_API_KEY is defined', () => {
    ...
    jest.spyOn(arjunaConfigService, 'get').mockReturnValue(mockApiKey);

    const instance = new DnsManagerService(arjunaConfigService, {} as any);

    expect(arjunaConfigService.get).toHaveBeenCalledWith('CLOUDFLARE_API_KEY');
  ...
  describe('registerHostname', () => {
    ...
    it('should register a custom domain successfully', async () => {
      ...
      jest.spyOn(arjunaConfigService, 'get').mockReturnValue('test-zone-id');
