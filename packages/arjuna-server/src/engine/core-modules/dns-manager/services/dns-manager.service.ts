...
export class DnsManagerService {
  ...
  constructor(
    ...
    if (this.arjunaConfigService.get('CLOUDFLARE_API_KEY')) {
      this.cloudflareClient = new Cloudflare({
        apiToken: this.arjunaConfigService.get('CLOUDFLARE_API_KEY'),
      });
    }
  }
      ...
      records: [
        ...
        },
        {
          ...
          value:
            ...
            `${hostname}.${this.arjunaConfigService.get('CLOUDFLARE_DCV_DELEGATION_ID')}.dcv.cloudflare.com`,
        },
      ],
    };
  }
  ...
  private getZoneId(options?: DnsManagerOptions): string {
    return options?.isPublicDomain
      ? this.arjunaConfigService.get('CLOUDFLARE_PUBLIC_DOMAIN_ZONE_ID')
      : this.arjunaConfigService.get('CLOUDFLARE_ZONE_ID');
  }
