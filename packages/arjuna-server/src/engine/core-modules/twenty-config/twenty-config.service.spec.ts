...
import { type ConfigVariables } from 'src/engine/core-modules/arjuna-config/config-variables';
...
jest.mock(
  'src/engine/core-modules/arjuna-config/constants/config-variables-masking-config',
  () => ({
...
jest.mock(
  'src/engine/core-modules/arjuna-config/utils/is-env-only-config-var.util',
  () => ({
    isEnvOnlyConfigVar: jest.fn(),
  }),
);

type ArjunaCRMConfigServicePrivateProps = {
...
const setupTestModule = async (isDatabaseConfigEnabled = true) => {
  ...
  return {
    service: module.get<ArjunaCRMConfigService>(ArjunaCRMConfigService),
...
const setPrivateProps = (
  service: ArjunaCRMConfigService,
  props: Partial<ArjunaCRMConfigServicePrivateProps>,
) => {
...
describe('ArjunaCRMConfigService', () => {
  let service: ArjunaCRMConfigService;
