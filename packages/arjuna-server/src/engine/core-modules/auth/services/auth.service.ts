...
import ms from 'ms';
import { PasswordUpdateNotifyEmail } from 'arjuna-emails';
...
export class AuthService {
  ...
  async checkIsEmailVerified(isEmailVerified: boolean) {
    const isEmailVerificationRequired = this.arjunaConfigService.get(
    ...
    const apps = [
      {
        ...
        redirectUrl:
          ...
          NodeEnvironment.DEVELOPMENT
            ...
            : `https://${this.arjunaConfigService.get(
                'CHROME_EXTENSION_ID',
              )}.chromiumapp.org/`,
      },
    ];
  ...
  async updatePassword(
    ...
    await this.emailService.send({
      from: `${this.arjunaConfigService.get(
        'EMAIL_FROM_NAME',
      )} <${this.arjunaConfigService.get('EMAIL_FROM_ADDRESS')}>`,
      to: user.email,
      subject,
      text,
      html,
