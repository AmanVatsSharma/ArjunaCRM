  ...
  implements GqlOptionsFactory<YogaDriverConfig<'express'>>
{
  ...
  createGqlOptions(): YogaDriverConfig {
    const isDebugMode =
      this.arjunaConfigService.get('NODE_ENV') === NodeEnvironment.DEVELOPMENT;
    const plugins = [
      useGraphQLErrorHandlerHook({
        metricsService: this.metricsService,
        exceptionHandlerService: this.exceptionHandlerService,
        i18nService: this.i18nService,
        arjunaConfigService: this.arjunaConfigService,
      }),
      useDisableIntrospectionAndSuggestionsForUnauthenticatedUsers(
        this.arjunaConfigService.get('NODE_ENV') === NodeEnvironment.PRODUCTION,
      ),
      useValidateGraphqlQueryComplexity({
        maximumAllowedFields:
          this.arjunaConfigService.get('GRAPHQL_MAX_FIELDS'),
        maximumAllowedRootResolvers: this.arjunaConfigService.get(
