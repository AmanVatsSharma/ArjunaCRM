...
export class MigrateAttachmentAuthorToCreatedByCommand extends ActiveOrSuspendedWorkspacesMigrationCommandRunner {
  ...
  override async runOnWorkspace({
    ...
    const attachmentRepository =
      await this.arjunaORMGlobalManager.getRepository<AttachmentWorkspaceEntity>(
        workspaceId,
        'attachment',
      );

    const workspaceMemberRepository =
      await this.arjunaORMGlobalManager.getRepository<WorkspaceMemberWorkspaceEntity>(
        workspaceId,
        'workspaceMember',
      );
