...
export class AuthCommand {
  ...
  getCommand(): Command {
    ...
    authCommand
      .command('login')
      .description('Authenticate with ArjunaCRM')
      .option('--api-key <key>', 'API key for authentication')
      .option('--api-url <url>', 'ArjunaCRM API URL')
  ...
  private async login(options: {
    ...
    try {
      ...
      if (isValid) {
        ...
        console.log(
          chalk.green(
            `✓ Successfully authenticated with ArjunaCRM (workspace: ${activeWorkspace})`,
          ),
        );
      } else {
        console.log(
