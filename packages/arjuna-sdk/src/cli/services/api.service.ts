...
} from 'arjuna-shared/application';
...
export class ApiService {
  ...
  constructor() {
    ...
    this.client.interceptors.request.use(async (config) => {
      const arjunaConfig = await this.configService.getConfig();

      config.baseURL = arjunaConfig.apiUrl;

      if (arjunaConfig.apiKey) {
        config.headers.Authorization = `Bearer ${arjunaConfig.apiKey}`;
      }
    ...
    this.client.interceptors.response.use(
      ...
      (error) => {
        if (error.response?.status === 401) {
          console.error(
            chalk.red(
              'Authentication failed. Please run `arjuna auth login` first.',
            ),
          );
          ...
          console.error(
            chalk.red('Cannot connect to ArjunaCRM server. Is it running?'),
          );
        }
        throw error;
      },
  ...
  async subscribeToLogs({
    ...
    const wsClient = createClient({
      url: arjunaConfig.apiUrl + '/graphql',
      headers: {
        Authorization: `Bearer ${arjunaConfig.apiKey}`,
        'Content-Type': 'application/json',
