...
import { type Sources } from 'arjuna-shared/types';
import {
  type Application,
  type PackageJson,
...
export const extractArjunaCRMAppConfig = (program: Program): Application => {
  for (const sf of program.getSourceFiles()) {
    ...
    const visit = (node: any): void => {
      // Look for "export default arjunaAppConfig"
...
export const loadManifest = async (
  ...
  const [objects, serverlessFunctions, application, sources] = [
    ...
    extractArjunaCRMAppConfig(program),
