...
function extractImports(sourceFile: ts.SourceFile): string[] {
  ...
  function visit(node: ts.Node) {
    ...
    // Quite static
    if (modulePath !== `'arjuna-ui'` && modulePath !== '"arjuna-ui"') {
      return ts.forEachChild(node, visit);
    }
 ...
 * @param importSpecifier The module to import from (e.g., 'arjuna-ui/utils')
...
const migrateImports = (mappedResolutions: MappedResolution[]) => {
  for (const { file, newImports } of mappedResolutions) {
    for (const { barrel, modules } of Object.values(newImports)) {
      ...
      removeSpecificImports({
        filePath: file,
        moduleSpecifier: 'arjuna-ui',
      });
      insertImportAtTop({
        filePath: file,
        importSpecifier: `arjuna-ui/${barrel}`,
...
const main = () => {
  const packageSrcPath = 'packages/arjuna-ui/src';
  const exportsPerModule = retrievePackageExportsPerModule(packageSrcPath);

  const packagesToMigrate = ['arjuna-front'];
