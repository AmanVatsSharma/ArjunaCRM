# How to deploy arjuna on podman
...
2. Start arjuna by running `podman-compose up -d`.

If you need to stop arjuna, you can do so by running `podman-compose down`.


### Install systemd service (optional)

If you want to install a systemd service to run arjuna, you can use the provided systemd service. 

Edit `arjunacrm.service` and change these two variables:


	WorkingDirectory=/opt/apps/arjuna
	EnvironmentFile=/opt/apps/arjuna/.env
...
Note: this script will enable the service and also start it. So it will assume that arjuna is not currently running.
