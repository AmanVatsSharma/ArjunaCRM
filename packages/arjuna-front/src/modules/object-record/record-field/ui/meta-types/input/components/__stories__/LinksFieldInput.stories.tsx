...
import { getCanvasElementForDropdownTesting } from 'arjuna-ui/testing';
  ...
  args: {
    value: {
      ...
      secondaryLinks: [
        ...
        {
          url: 'https://github.com/vedpragyabharat/arjuna',
          label: 'GitHub',
        },
      ],
    },
...
export const CreatePrimaryLink: Story = {
  play: async ({ canvasElement }) => {
    ...
    const input = await canvas.findByPlaceholderText('URL');
    await userEvent.type(input, 'https://www.vedpragya.com{enter}');

    const linkDisplay = await canvas.findByText('vedpragya.com');
...
export const AddSecondaryLink: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const input = await canvas.findByPlaceholderText('URL');
    await userEvent.type(input, 'https://docs.vedpragya.com{enter}');

    const secondaryLink = await canvas.findByText('docs.vedpragya.com');
...
export const DeletePrimaryLink: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const listItemToDelete = await canvas.findByText('ArjunaCRM Website');
...
export const DeletePrimaryLinkAndUseSecondaryLinkAsTheNewPrimaryLink: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const oldPrimaryLink = canvas.queryByText('ArjunaCRM Website');
