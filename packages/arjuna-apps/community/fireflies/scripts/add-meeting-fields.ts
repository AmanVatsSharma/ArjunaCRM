 ...
 * Run this after: npx arjuna-cli app sync packages/arjuna-apps/fireflies
 *
...
if (!API_KEY) {
  console.error('❌ Error: ARJUNA_API_KEY not found in .env file');
  process.exit(1);
}
...
const findMeetingObject = async () => {
  ...
  if (!meetingEdge) {
    throw new Error('Meeting object not found. Please run "npx arjuna-cli app sync" first.');
  }
