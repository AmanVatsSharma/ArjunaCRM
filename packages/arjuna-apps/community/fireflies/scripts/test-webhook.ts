...
/**
 * Test script for Fireflies webhook against local ArjunaCRM instance
 *
 * Usage:
 *   yarn test:webhook
 *   # or
 ...
 *
 * Prerequisites:
 *   1. ArjunaCRM server running on http://localhost:3000
 *   2. Fireflies app synced: npx arjuna-cli app sync
...
const _mockFirefliesFetch = async (url: string, options?: RequestInit) => {
  ...
  // For ArjunaCRM API calls, use real fetch
  return fetch(url, options);
};

const main = async () => {
  console.log('🧪 Testing Fireflies Webhook Against Local ArjunaCRM Instance\n');
  console.log(`📍 Server URL: ${SERVER_URL}`);
  console.log(`🔑 API Key: ${ARJUNA_API_KEY ? '✅ Configured' : '❌ Missing'}`);
  console.log(`🔐 Webhook Secret: ${FIREFLIES_WEBHOOK_SECRET ? '✅ Configured' : '⚠️  Using test secret'}\n`);

  // Validation
  if (!ARJUNA_API_KEY) {
    console.error('❌ Error: ARJUNA_API_KEY is required');
  ...
  try {
    ...
    console.error('   Make sure ArjunaCRM is running: cd arjuna && yarn dev');
    process.exit(1);
  }
  ...
  if (signatureErrors) {
    ...
    if (response.ok) {
      ...
      console.log('\n📋 Next steps:');
      console.log('   1. Check ArjunaCRM CRM for new Meeting/Note records');
    ...
    console.error('\n💡 Troubleshooting:');
    console.error('   1. Ensure ArjunaCRM server is running');
    console.error('   2. Ensure app is synced: npx arjuna-cli app sync');
