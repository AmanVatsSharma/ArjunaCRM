...
Automatically captures meeting notes with AI-generated summaries and insights from Fireflies.ai into your ArjunaCRM CRM.

### Current Status
- Doesn't work with Fireflies webhook yet due to missing headers forwarding in arjuna serverless func
...
**Fireflies webhook → Fireflies API → ArjunaCRM CRM with summary-focused insights**
...
```bash
# Step 1: Authenticate with ArjunaCRM
npx arjuna-cli auth login

# Step 2: Sync the app to create Meeting object
npx arjuna-cli app sync packages/arjuna-apps/fireflies
...
ARJUNA_API_KEY=your_api_key             # From ArjunaCRM CRM settings
SERVER_URL=https://your-domain.vedpragya.com
```
...
```
https://your-arjuna-instance.com/s/webhook/fireflies
```
...
- ArjunaCRM verifies signature using your webhook secret
- Invalid signatures are rejected immediately

### Current Platform Limitation (Headers)

- ArjunaCRM serverless route triggers currently do **not forward HTTP headers** to functions. Fireflies signatures sent in headers are stripped, so header-based verification does not work in production.
...
- Ingest a specific Fireflies meeting into ArjunaCRM:
`yarn meeting:ingest <meetingId>` or `MEETING_ID=... yarn meeting:ingest`

- Fetch all/historical Fireflies meetings into ArjunaCRM:
`yarn meeting:all [--from 2024-01-01] [--to 2024-02-01] [--organizer a@x.com] [--participant b@x.com] [--channel <channelId>] [--mine] [--dry-run]`
  ...
  - Controls:
    - `--dry-run`: list and transform without writing to ArjunaCRM
...
**Integration**: Fireflies webhook → AI processing layer → Enhanced ArjunaCRM records
