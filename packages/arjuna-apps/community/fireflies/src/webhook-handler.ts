...
import { ArjunaCRMCrmService } from './arjuna-crm-service';
...
export class WebhookHandler {
  ...
  async handle(params: unknown, headers?: Record<string, string>): Promise<ProcessResult> {
    ...
    try {
      ...
      const arjunaApiKey = process.env.ARJUNA_API_KEY || '';
      ...
      if (!arjunaApiKey) {
        this.logger.critical('ARJUNA_API_KEY not configured - this is a critical configuration error');
        throw new Error('ARJUNA_API_KEY environment variable is required');
      }
      ...
      const arjunaService = new ArjunaCRMCrmService(
        arjunaApiKey,
        getApiUrl()
      );

      const existingMeetingById = await arjunaService.findMeetingByFirefliesId(meetingData.id);
      ...
      const existingMeetingByTitle = await arjunaService.findExistingMeeting(meetingData.title);
      ...
      const { matchedContacts, unmatchedParticipants } = await arjunaService.matchParticipantsToContacts(
      ...
      const newContactIds = autoCreate
        ? await arjunaService.createContactsForUnmatched(unmatchedParticipants)
        : [];
      ...
      const noteId = await arjunaService.createNoteOnly(
      ...
      result.meetingId = await arjunaService.createMeeting(meetingInput);
  ...
  private async linkNoteToParticipants(
    arjunaService: ArjunaCRMCrmService,
    noteId: string,
    contactIds: string[]
  ): Promise<void> {
    for (const contactId of contactIds) {
      try {
        await arjunaService.createNoteTarget(noteId, contactId);
  ...
  private async createFailedMeetingRecord(params: unknown, error: string): Promise<void> {
    try {
      const arjunaApiKey = process.env.ARJUNA_API_KEY || '';
      if (!arjunaApiKey) {
        this.logger.debug('Cannot create failed meeting record: ARJUNA_API_KEY not configured');
        return;
      }
      ...
      const arjunaService = new ArjunaCRMCrmService(arjunaApiKey, getApiUrl());
      ...
      const failedMeetingId = await arjunaService.createFailedMeeting(failedMeetingData);
