...
export class ArjunaCRMCrmService {
  ...
  constructor(apiKey: string, apiUrl: string) {
    if (!apiKey) {
      logger.critical('ARJUNA_API_KEY is required but not provided - this is a critical configuration error');
      throw new Error('ARJUNA_API_KEY is required');
    }
    this.apiKey = apiKey;
    this.apiUrl = apiUrl;
