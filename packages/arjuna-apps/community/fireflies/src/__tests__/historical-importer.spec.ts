...
describe('HistoricalImporter', () => {
  const buildImporter = () => {
    ...
    return { firefliesClient, arjunaService };
  };

  it('skips meetings that already exist by firefliesMeetingId', async () => {
    const { firefliesClient, arjunaService } = buildImporter();

    firefliesClient.listTranscripts.mockResolvedValue([{ id: 'existing' }]);
    arjunaService.findMeetingByFirefliesId.mockResolvedValue({ id: 'arjuna-id' });

    const importer = new HistoricalImporter(firefliesClient, arjunaService);
    ...
    expect(arjunaService.createMeeting).not.toHaveBeenCalled();
    expect(result.statuses[0].status).toBe('skipped_existing');
  });

  it('supports dry-run without writing to ArjunaCRM', async () => {
    const { firefliesClient, arjunaService } = buildImporter();
    ...
    arjunaService.findMeetingByFirefliesId.mockResolvedValue(undefined);
    ...
    expect(arjunaService.createMeeting).not.toHaveBeenCalled();
    expect(arjunaService.createNoteOnly).not.toHaveBeenCalled();
