...
### Added
- **Meeting ingest utility**: New `yarn meeting:ingest <meetingId>` script to manually fetch and import specific Fireflies meetings into ArjunaCRM
...
- **Documentation**: Added README sections on current arjuna headers forward limitations and utility scripts
...
- **Major refactoring**: Split monolithic `receive-fireflies-notes.ts` into modular architecture:
  ...
  - `arjuna-crm-service.ts` - ArjunaCRM CRM operations (contacts, notes, meetings)
...
- Added TypeScript path mappings for `arjuna-sdk` in workspace configuration
