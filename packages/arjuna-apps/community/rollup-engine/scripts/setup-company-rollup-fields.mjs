...
const envCandidates = [
  process.env.ARJUNA_ENV_PATH ? path.resolve(process.env.ARJUNA_ENV_PATH) : null,
...
envCandidates.forEach((candidate) => {
  ...
  if (!result.error && process.env.ARJUNA_API_KEY) {
    return;
  }
});

const METADATA_BASE_URL = (
  process.env.ARJUNA_METADATA_BASE_URL ??
  (process.env.ARJUNA_API_BASE_URL
    ? `${process.env.ARJUNA_API_BASE_URL.replace(/\/+$/, '')}/metadata`
    : null) ??
  'http://localhost:3000/rest/metadata'
).replace(/\/+$/, '');

const API_KEY = process.env.ARJUNA_API_KEY;

if (!API_KEY) {
  console.error('ARJUNA_API_KEY is required in the environment to call the Metadata API.');
  process.exit(1);
}
