...
import { buildChildRecordIndex, ArjunaCRMClient } from './client';
...
const getApiCredentials = () => {
  const apiKey =
    process.env.ARJUNA_API_KEY ??
    process.env.ARJUNA_API_TOKEN ??
    process.env.API_KEY;
  ...
  const baseUrlRaw =
    process.env.ARJUNA_API_BASE_URL ??
    process.env.ARJUNA_REST_BASE_URL ??
...
export const main = async (params: unknown): Promise<any> => {
  ...
  try {
    ...
    if (!credentials) {
      console.warn(
        '[rollup] skipping execution because ARJUNA_API_KEY is not set',
      );
      return { status: 'noop', reason: 'ARJUNA_API_KEY not configured' };
    }

    const { apiKey, baseUrl } = credentials;
    const client = new ArjunaCRMClient(apiKey, baseUrl);
    const now = new Date();
