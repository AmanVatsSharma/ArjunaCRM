...
General-purpose rollup engine for ArjunaCRM workspaces. The bundle ships a
...
- an API key with access to your workspace. Generate one at
  `https://vedpragya.com/settings/api-webhooks`.
...
- To script the setup, export `ARJUNA_API_KEY` (and `ARJUNA_METADATA_BASE_URL` if you are not targeting `http://localhost:3000/rest/metadata`) and run `yarn setup:metadata`. The helper script calls the Metadata API to ensure each field exists on the Company object, skipping anything already provisioned.
...
2. **Configure environment variables**
   ...
   - Provide values for the manifest-defined keys:
     - `ARJUNA_API_KEY` (required secret; create it in ArjunaCRM → Settings → API & Webhooks).
     - `ARJUNA_API_BASE_URL` (optional; leave blank to use `https://app.vedpragya.com/rest`).
...
- Opportunities expose `companyId`, `amount.amountMicros`, `amount.currencyCode`,
  ...
  config.
- Amount totals are stored in micros (integers) to align with ArjunaCRM’s composite
  currency fields.
...
Opportunity data, and asserts the PATCH payload that would be sent to ArjunaCRM.

## Troubleshooting
- **`Test your function` reports `mainFile.main is not a function`:** delete the app
  (`arjuna app delete`) and sync it again (`arjuna app sync`) to clear any stale bundle.
