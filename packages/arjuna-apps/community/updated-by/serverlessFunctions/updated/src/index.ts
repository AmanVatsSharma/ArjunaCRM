...
const ARJUNA_API_URL: string =
  process.env.ARJUNA_API_URL ?? 'https://api.vedpragya.com';
...
const returnWorkspaceMemberObjectId = async (): Promise<string> => {
  const options = {
    ...
    url: `${ARJUNA_API_URL}/rest/metadata/objects`,
  };
...
const updateUpdatedByFieldValue = async (
  ...
  const options = {
    ...
    url: `${ARJUNA_API_URL}/rest/${objectName}/${recordId}`,
    data: {
...
export const main = async (params: {
  ...
  if (ARJUNA_API_KEY === '') {
    console.error('You must specify a valid ARJUNA_API_KEY');
    return {};
  }
  try {
