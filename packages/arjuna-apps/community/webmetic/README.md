...
Automatically sync B2B website visitor data into ArjunaCRM CRM. Identify anonymous companies visiting your website and track their engagement without forms or manual entry. Every hour, Webmetic enriches your CRM with actionable sales intelligence about who's researching your product before they ever fill out a contact form.
...
- A ArjunaCRM API key (create one at `https://vedpragya.com/settings/api-webhooks` and name it **"Webmetic"**)
...
```bash
arjuna auth login
cd packages/arjuna-apps/webmetic
arjuna app sync
```

### 2. Configure environment variables

- **First, create a ArjunaCRM API key**:
...
- **Then configure the app**:
  ...
  - Provide values for the required keys:
    - `ARJUNA_API_KEY` (required secret; paste the API key you just created)
    - `ARJUNA_API_URL` (optional; defaults to `http://localhost:3000` for local dev, set to your production URL)
...
```
Webmetic API ─[hourly]→ sync-visitor-data ─[create/update]→ ArjunaCRM CRM
                              │
...
|----------|----------|-------------|
| `ARJUNA_API_KEY` | ✅ Yes | Your ArjunaCRM API key for authentication |
| `ARJUNA_API_URL` | ❌ No | Base URL of your ArjunaCRM instance (defaults to `http://localhost:3000`) |
...
This app uses multiple ArjunaCRM CRM APIs:
...
- **Solution**: This occurs if you previously deleted a company. ArjunaCRM maintains unique constraints on soft-deleted records. Either restore the company from trash or contact support.
...
- **ArjunaCRM API**: 800ms delay between requests (75 requests/minute)
...
# Sync to local ArjunaCRM instance
npx arjuna-cli app sync

# Watch for changes
npx arjuna-cli app dev
```

### Manual trigger

Use the ArjunaCRM UI test panel or trigger via API:
...
Built with 🍺 and ❤️ in Munich by [Team Webmetic](https://webmetic.de) for ArjunaCRM CRM Hacktoberfest 2025.

For issues or questions:
- Webmetic API: [webmetic.de](https://webmetic.de)
- ArjunaCRM CRM: [vedpragya.com/developers](https://vedpragya.com/developers)
