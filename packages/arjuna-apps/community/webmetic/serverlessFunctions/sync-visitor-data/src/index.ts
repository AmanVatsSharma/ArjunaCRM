...
export const main = async (): Promise<object> => {
  ...
  log(
    'Available environment variables: ' +
      Object.keys(process.env)
        .filter((k) => k.includes('ARJUNA') || k.includes('WEBMETIC'))
        .join(', '),
  );

  const WEBMETIC_API_KEY = process.env.WEBMETIC_API_KEY;
  const WEBMETIC_DOMAIN = process.env.WEBMETIC_DOMAIN;
  const ARJUNA_API_KEY = process.env.ARJUNA_API_KEY;

  // Base URL without /rest for metadata operations
  const ARJUNA_API_BASE_URL =
    process.env.ARJUNA_API_URL !== '' &&
    process.env.ARJUNA_API_URL !== undefined
      ? process.env.ARJUNA_API_URL
      : 'http://localhost:3000';

  // REST URL for data operations
  const ARJUNA_API_URL = `${ARJUNA_API_BASE_URL}/rest`;

  log('WEBMETIC_API_KEY: ' + (WEBMETIC_API_KEY ? '***SET***' : 'MISSING'));
  log('WEBMETIC_DOMAIN: ' + (WEBMETIC_DOMAIN || 'MISSING'));
  log('ARJUNA_API_KEY: ' + (ARJUNA_API_KEY ? '***SET***' : 'MISSING'));
  log('ARJUNA_API_BASE_URL: ' + ARJUNA_API_BASE_URL);
  log('ARJUNA_API_URL (REST): ' + ARJUNA_API_URL);

  if (!WEBMETIC_API_KEY || !WEBMETIC_DOMAIN || !ARJUNA_API_KEY) {
    return {
      success: false,
      error: 'Missing required environment variables',
      WEBMETIC_API_KEY: !!WEBMETIC_API_KEY,
      WEBMETIC_DOMAIN: !!WEBMETIC_DOMAIN,
      ARJUNA_API_KEY: !!ARJUNA_API_KEY,
  ...
  try {
    ...
    await ensureWebsiteLeadObjectExists(ARJUNA_API_KEY, ARJUNA_API_BASE_URL);

    // Then, ensure all fields exist (create via GraphQL if missing)
    await ensureWebsiteLeadFieldsExist(ARJUNA_API_KEY, ARJUNA_API_BASE_URL);
  } catch (error: any) {
  ...
  try {
    ...
    for (const company of companies) {
      try {
        ...
        // 2a. Find or create Company in ArjunaCRM
        ...
        const existingCompaniesOptions = {
          ...
          url: `${ARJUNA_API_URL}/companies?filter=domainName.primaryLinkUrl%5Beq%5D%3A%22${encodeURIComponent(domain)}%22`,
        };
        ...
        // If you need these fields, you'll need to create custom fields in ArjunaCRM first
        ...
        for (const session of company.sessions) {
          try {
            ...
            const existingLeadsOptions = {
              ...
              url: `${ARJUNA_API_URL}/websiteLeads?filter=name%5Beq%5D%3A%22${encodeURIComponent(leadName)}%22`,
            };
            ...
            if (!existingLeads || existingLeads.length === 0) {
              ...
              const createOptions = {
                ...
                url: `${ARJUNA_API_URL}/websiteLeads`,
                data: leadData,
              };
