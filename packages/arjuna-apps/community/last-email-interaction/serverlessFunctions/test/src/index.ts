...
const create_last_interaction = (id: string) => {
  return {
    method: 'POST',
    url: `${ARJUNA_URL}/metadata/fields`,
    headers: {
...
export const main = async (params: {
  ...
  const options = {
    method: 'GET',
    url: `${ARJUNA_URL}/metadata/objects`,
    headers: {
      Authorization: `Bearer ${ARJUNA_API_KEY}`,
    },
  };
  try {
    ...
    const messageOptions = {
      method: 'GET',
      url: `${ARJUNA_URL}/messages/${recordId}?depth=1`,
      headers: {
        Authorization: `Bearer ${ARJUNA_API_KEY}`,
      },
    };
