...
type ArjunaCRMApiResponse = {
  id: string;
};

const AI_PROVIDER_API_KEY = process.env.AI_PROVIDER_API_KEY;
const WEBHOOK_SECRET_TOKEN = process.env.WEBHOOK_SECRET_TOKEN;
const ARJUNA_API_URL = process.env.ARJUNA_API_URL;
...
const getArjunaCRMApiConfig = () => {
  const apiKey = process.env.ARJUNA_API_KEY;
  if (!apiKey) {
    throw new Error('ARJUNA_API_KEY environment variable is not set');
  }

  const baseUrl = ARJUNA_API_URL;
  if (!baseUrl) {
    throw new Error('ARJUNA_API_URL environment variable is not set');
  }
...
const lookupWorkspaceMemberByName = async (
  ...
  const { apiKey, baseUrl } = getArjunaCRMApiConfig();
...
const createNoteInArjunaCRM = async (
  summary: string,
  keyPoints: string[],
  relatedPersonId: string,
  meetingTitle?: string,
  meetingDate?: string,
): Promise<ArjunaCRMApiResponse> => {
  const { apiKey, baseUrl } = getArjunaCRMApiConfig();
  const noteTitle =
    meetingTitle ||
...
const createTaskInArjunaCRM = async (
...
const createTasksFromActionItems = async (
  ...
  for (const actionItem of actionItems) {
    try {
      ...
      const task = await createTaskInArjunaCRM({
        ...actionItem,
...
export const main = async (
  ...
  try {
    ...
    log('📄 Creating note in ArjunaCRM CRM...');
    const note = await createNoteInArjunaCRM(
      analysis.summary,
      analysis.keyPoints,
      relatedPersonId,
      meetingTitle,
