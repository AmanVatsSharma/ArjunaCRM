...
export default defineBackground(async () => {
    ...
    onMessage('createPerson', async ({data}) => {
        const response = await fetch(`${import.meta.env.WXT_ARJUNA_API_URL}/s/create/person`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${import.meta.env.WXT_ARJUNA_API_KEY}`,
          },
          body: JSON.stringify({
    ...
    onMessage('createCompany', async ({data}) => {
      const response = await fetch(`${import.meta.env.WXT_ARJUNA_API_URL}/s/create/company`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${import.meta.env.WXT_ARJUNA_API_KEY}`,
