# ArjunaCRM Browser Extension

A Chrome browser extension for capturing LinkedIn profiles (people and companies) directly into ArjunaCRM CRM. This is a basic **v0** focused mostly on establishing a solid architectural foundation.

## Overview

This extension integrates with LinkedIn to extract profile information and create records in ArjunaCRM CRM. It uses **WXT** as the framework - initially tried Plasmo, but found WXT to be significantly better due to its extensibility and closer alignment with the Chrome extension APIs, providing more control and flexibility.
...
1. **`arjuna-browser-extension`** - The main extension package (WXT + React)
2. **`arjuna-apps/browser-extension`** - Serverless functions for API interactions
...
- **Popup** (`src/entrypoints/popup/`)
  ...
  - Provides buttons to save person/company to ArjunaCRM
...
Located in `packages/arjuna-apps/browser-extension/serverlessFunctions/`:

- **`/s/create/person`** - Creates a new person record in ArjunaCRM
- **`/s/create/company`** - Creates a new company record in ArjunaCRM
...
- ArjunaCRM CLI installed globally: `npm install -g arjuna-cli`
- API key from ArjunaCRM: https://vedpragya.com/settings/api-webhooks

### Setup
   ```
1. **Configure environment variables:**
   - Set `ARJUNA_API_URL` in the serverless function configuration
   - Set `ARJUNA_API_KEY` (marked as secret) in the serverless function configuration
...
npx nx run dev arjuna-browser-extension

# Build for production
npx nx run build arjuna-browser-extension

# Package extension for distribution
npx nx run package arjuna-browser-extension
```
...
2. **Load extension in Chrome:**
   ...
   - Click "Load unpacked"
   - Select `packages/arjuna-browser-extension/dist/chrome-mv3-dev/`
...
3. **Test on LinkedIn:**
   ...
   - The "Add to ArjunaCRM" button should appear in the profile header
   - Click the button to open the popup and save to ArjunaCRM

### Project Structure

```
packages/arjuna-browser-extension/
├── src/
│   ├── common/
...
✅ Create person records in ArjunaCRM
✅ Create company records in ArjunaCRM

## Planned Features

- [ ] Provide a way to have API key and custom remote URLs.
- [ ] Detect if record already exists and prevent duplicates
- [ ] Open existing ArjunaCRM record when clicked (instead of creating duplicate)
